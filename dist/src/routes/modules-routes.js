...
modulesRouter.get("/modules", requireAuth(), asyncHandler(async (request, response) => {
    if (!request.authorizationSummary) {
        sendError(response, "Se requiere autenticación.", 401);
        return;
    }
    const modules = await moduleRegistryService.getEnabledModules(request.authorizationSummary.permissions);
