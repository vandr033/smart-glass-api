...
const WIN_ANSI_SPECIAL_CHARACTERS = {
    Á: 0xc1,
    É: 0xc9,
    Í: 0xcd,
    Ó: 0xd3,
...
const toWinAnsi = (value) => {
    ...
    for (const character of value) {
        ...
        bytes.push(WIN_ANSI_SPECIAL_CHARACTERS[character] ??
            (codePoint >= 32 && codePoint <= 126 ? codePoint : 32));
    }
    return Buffer.from(bytes);
};
const escapePdfBytes = (value) => {
...
const createTextStream = (page, pageNumber, totalPages) => {
    const chunks = [];
    const push = (value) => {
...
const createPdfObject = (body) => {
    return typeof body === "string" ? Buffer.from(body, "ascii") : body;
};
export const buildSimplePdf = (pages) => {
    const objects = [];
    ...
    objects[pagesId - 1] = createPdfObject(`<< /Type /Pages /Kids [${pageIds.map((id) => `${id} 0 R`).join(" ")}] /Count ${pageIds.length} >>`);
    ...
    bodyChunks.push(Buffer.from(`trailer\n<< /Size ${objects.length + 1} /Root ${catalogId} 0 R >>\nstartxref\n${xrefOffset}\n%%EOF\n`, "ascii"));
    return Buffer.concat(bodyChunks);
};
export const wrapPdfText = (value, width) => {
...
export const paginatePdfLines = (lines, options) => {
...
//# sourceMappingURL=simple-pdf.js.map