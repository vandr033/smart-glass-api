...
import { createProductSchema, productIdParamSchema, listProductsQuerySchema, updateProductSchema, } from "./products.validators.js";
...
const getRequiredProductId = (value) => {
...
export const productsController = {
    ...
    async getById(request, response) {
        const record = await productsService.getProductById(getRequiredProductId(request.params.id));
    ...
    async update(request, response) {
        ...
        const actorContext = getRequestLogActorContext(request);
        const result = await productsService.updateProduct(getRequiredProductId(request.params.id), payload);
        await Promise.all([
            activityLogService.logUserAction({
                ...actorContext,
    ...
    async remove(request, response) {
        const actorContext = getRequestLogActorContext(request);
        const record = await productsService.deleteProduct(getRequiredProductId(request.params.id));
        await Promise.all([
            activityLogService.logUserAction({
                ...actorContext,
