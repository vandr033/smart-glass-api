...
import { approveCuttingPlanSchema, cuttingOptimizationRunIdParamSchema, cuttingPlanIdParamSchema, listCuttingOptimizationsQuerySchema, listCuttingPlansQuerySchema, quotationGlassOptimizationSchema, quotationIdParamSchema, runGlassOptimizationSchema, } from "./cutting.validators.js";
...
export const cuttingController = {
    async listCuttingOptimizations(request, response) {
    ...
    async runGlassOptimization(request, response) {
        const payload = runGlassOptimizationSchema.parse(request.body);
        const actorContext = getRequestLogActorContext(request);
        const run = await cuttingService.runGlassOptimization(payload, actorContext.userId ?? null);
        sendSuccess(response, run, 201);
    },
    async getCuttingOptimizationById(request, response) {
        const run = await cuttingService.getCuttingOptimizationRunById(getRequiredRunId(request.params.id));
        sendSuccess(response, run);
    },
    async generateCuttingPlanFromRun(request, response) {
        const actorContext = getRequestLogActorContext(request);
        const plans = await cuttingService.generateCuttingPlanFromOptimizationRun(getRequiredRunId(request.params.id), actorContext.userId ?? null);
        sendSuccess(response, plans, 201);
    },
    async cancelCuttingOptimizationRun(request, response) {
        const actorContext = getRequestLogActorContext(request);
        const run = await cuttingService.cancelOptimizationRun(getRequiredRunId(request.params.id), actorContext.userId ?? null);
        sendSuccess(response, run);
    },
    async getQuotationGlassRequirements(request, response) {
        const requirements = await cuttingService.collectGlassRequirementsFromQuotation(getRequiredQuotationId(request.params.id));
        sendSuccess(response, requirements);
    },
    async runQuotationGlassOptimization(request, response) {
        ...
        const requirements = await cuttingService.collectGlassRequirementsFromQuotation(quotationId);
        if (requirements.pieces.length === 0) {
            throw new AppError("No glass sheet requirements could be extracted from this quotation.", 400);
        }
        const actorContext = getRequestLogActorContext(request);
    ...
    async listCuttingPlans(request, response) {
    ...
    async getCuttingPlanById(request, response) {
        const plan = await cuttingService.getCuttingPlanById(getRequiredPlanId(request.params.id));
        sendSuccess(response, plan);
    },
    async approveCuttingPlan(request, response) {
        const payload = approveCuttingPlanSchema.parse(request.body ?? {});
        const actorContext = getRequestLogActorContext(request);
        const plan = await cuttingService.approveCuttingPlan(getRequiredPlanId(request.params.id), actorContext.userId ?? null, payload);
        sendSuccess(response, plan);
    },
    async createRemnantsFromCuttingPlan(request, response) {
        const actorContext = getRequestLogActorContext(request);
        const plan = await cuttingService.createRemnantsFromCuttingPlan(getRequiredPlanId(request.params.id), actorContext.userId ?? null);
        sendSuccess(response, plan);
    },
    async getPrintableCuttingPlan(request, response) {
        const actorContext = getRequestLogActorContext(request);
        const plan = await cuttingService.getPrintableCuttingPlan(getRequiredPlanId(request.params.id), actorContext.userId ?? null);
        sendSuccess(response, plan);
    },
};
