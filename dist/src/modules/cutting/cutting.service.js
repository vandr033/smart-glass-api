...
import { CUTTING_OPTIMIZATION_RUN_ENTITY_TYPE, CUTTING_PLAN_ENTITY_TYPE, CUTTING_PLAN_REMNANT_OUTPUT_ENTITY_TYPE, } from "./cutting.constants.js";
...
const LEGACY_INVENTORY_MINIMUM_REMNANT_AREA_M2 = "inventory.minimum_remnant_area_m2";
...
const areaFromDimensions = (widthMm, heightMm) => {
...
const mapMaterialSummary = (record) => {
    if (!record) {
        return null;
    }
    return {
...
const mapWarehouseSummary = (record) => {
    if (!record) {
        return null;
    }
    return {
...
const mapQuotationSummary = (record) => {
    if (!record) {
        return null;
    }
    return {
...
const mapProjectSummary = (record) => {
    if (!record) {
        return null;
    }
    return {
...
const parseRunInputJson = (value) => {
    ...
    return {
        ...
        mode: record.mode === "COMMERCIAL_ESTIMATION" ||
...
const parseOptimizationResultJson = (value) => {
    if (!value || !isObjectRecord(value)) {
        return null;
    }
    return value;
};
const parseSheetLayoutJson = (value) => {
...
const mapOptimizationRunListItem = (record) => {
    return {
...
const mapOptimizationRunRecord = (record) => {
    return {
...
const mapCuttingPlanPiece = (record) => {
    return {
...
const mapCuttingPlanRemnantOutput = (record) => {
    return {
        areaM2: roundTo(Number(record.areaM2), 4),
        createdAt: record.createdAt.toISOString(),
        cuttingPlanSheetId: record.cuttingPlanSheetId,
...
const mapCuttingPlanSheet = (record) => {
    return {
        createdAt: record.createdAt.toISOString(),
...
const mapCuttingPlanListItem = (record) => {
    return {
        code: record.code,
        createdAt: record.createdAt.toISOString(),
        id: record.id,
...
const mapCuttingPlanRecord = (record) => {
    return {
        ...mapCuttingPlanListItem(record),
        sheets: record.sheets.map(mapCuttingPlanSheet),
    };
};
const getSheetCountFromQuantity = (quantity, unit, areaM2) => {
    if (quantity <= 0) {
        return 0;
    }
    if (unit === "M2" && areaM2 > 0) {
...
const sourcePriority = (sheetSource, preferRemnants) => {
...
const getPieceOrientations = (piece, allowRotation) => {
    const orientations = [
        {
...
const canFitOnRectangle = (piece, widthMm, heightMm, allowRotation) => {
    return getPieceOrientations(piece, allowRotation).some((orientation) => orientation.widthMm <= widthMm && orientation.heightMm <= heightMm);
};
const getSheetUsedHeight = (sheet) => {
    return sheet.shelves.reduce((total, shelf) => total + shelf.heightMm, 0);
};
const findBestPlacement = (sheet, piece, allowRotation) => {
    const candidates = [];
    for (const orientation of getPieceOrientations(piece, allowRotation)) {
        sheet.shelves.forEach((shelf, shelfIndex) => {
            if (orientation.heightMm <= shelf.heightMm &&
                shelf.usedWidthMm + orientation.widthMm <= sheet.widthMm) {
                const remainingWidth = sheet.widthMm - (shelf.usedWidthMm + orientation.widthMm);
...
const placePieceOnSheet = (sheet, piece, allowRotation) => {
    ...
    if (placement.addsShelf) {
        ...
        sheet.shelves[placement.shelfIndex] = {
            ...sheet.shelves[placement.shelfIndex],
            usedWidthMm: sheet.shelves[placement.shelfIndex].usedWidthMm +
...
const calculateSheetWaste = (sheet) => {
    const wasteAreaM2 = roundTo(Math.max(sheet.sheetAreaM2 - sheet.usedAreaM2, 0), 4);
    const wastePercent = sheet.sheetAreaM2 > 0
...
const buildRemnantZones = (sheet, minimumRemnantAreaM2) => {
    const zones = [];
...
const assertRuntimeLimit = (startedAt, maxRuntimeSeconds) => {
    const elapsedMs = Date.now() - startedAt;
    if (elapsedMs > maxRuntimeSeconds * 1000) {
        throw new AppError(`Optimization exceeded the configured runtime limit of ${maxRuntimeSeconds} seconds.`, 408);
    }
};
const buildSheetState = (candidate) => {
    return {
        ...candidate,
        pieces: [],
        shelves: [],
...
const getDefaultSheetDimensions = (material, settings) => {
    const warnings = [];
    const defaultPreset = material.dimensionPresets[0];
    const widthMm = decimalToNumber(material.standardWidthMm) ??
        decimalToNumber(defaultPreset?.widthMm) ??
        settings.defaultSheetWidthMm;
    const heightMm = decimalToNumber(material.standardHeightMm) ??
    ...
    const thicknessMm = decimalToNumber(material.thicknessMm) ??
        decimalToNumber(defaultPreset?.thicknessMm);
    if (decimalToNumber(material.standardWidthMm) === null &&
        decimalToNumber(defaultPreset?.widthMm) === null) {
        warnings.push(`Material "${material.name}" is missing a configured standard sheet width. The cutting default setting was used instead.`);
    }
    if (decimalToNumber(material.standardHeightMm) === null &&
        ...
        warnings.push(`Material "${material.name}" is missing a configured standard sheet height. The cutting default setting was used instead.`);
    }
    return {
        heightMm,
...
const getCuttingSettings = async (db) => {
    const settingKeys = [
    ...
    return {
        defaultAllowRotation: readBoolean(CUTTING_DEFAULT_ALLOW_ROTATION, true),
        defaultSheetHeightMm: readNumber(CUTTING_DEFAULT_SHEET_HEIGHT_MM, 2250),
        defaultSheetWidthMm: readNumber(CUTTING_DEFAULT_SHEET_WIDTH_MM, 3210),
        maxRuntimeSeconds: readNumber(CUTTING_MAX_RUNTIME_SECONDS, 10),
        minimumRemnantAreaM2: readNumber(CUTTING_MINIMUM_REMNANT_AREA_M2, 0.1, LEGACY_INVENTORY_MINIMUM_REMNANT_AREA_M2),
        preferRemnants: readBoolean(CUTTING_PREFER_REMNANTS, true, LEGACY_INVENTORY_PREFER_REMNANTS),
    };
};
const getMaterialMapOrThrow = async (db, materialIds) => {
    ...
    materialMap.forEach((material) => {
        if (material.materialType !== "SHEET") {
            throw new AppError(`Material "${material.name}" is not a sheet material and cannot be optimized here.`, 400);
        }
        if (!material.isCuttable) {
            throw new AppError(`Material "${material.name}" is not marked as cuttable.`, 400);
        }
    });
    return materialMap;
};
const buildExpandedPieces = (pieces, materialMap, allowRotation) => {
    ...
    pieces.forEach((piece) => {
        ...
        const resolvedThickness = piece.thicknessMm ?? decimalToNumber(material.thicknessMm);
        ...
        for (let index = 0; index < piece.quantity; index += 1) {
            group.pieces.push({
                ...
                heightMm: piece.heightMm,
                label: piece.quantity > 1 ? `${piece.label} (${index + 1}/${piece.quantity})` : piece.label,
                materialId: piece.materialId,
                metadata: piece.metadata,
                pieceId: randomUUID(),
                quotationItemId: piece.quotationItemId,
...
const collectPiecesFromSimulationResult = (simulationResult, options) => {
    const pieces = [];
    const warnings = [];
    const repeatCount = options?.repeatCount ?? 1;
    simulationResult.cuts.sheets.forEach((sheetCut, sheetCutIndex) => {
        const pieceTemplates = sheetCut.sheetPieces.length > 0
        ...
        if (pieceTemplates.length === 0) {
            warnings.push(`Sheet cut "${sheetCut.label}" did not provide any piece geometry and was skipped.`);
            return;
        }
...
const optimizeGroup = (input) => {
    const mutableSheets = input.candidateSheets
        .map(buildSheetState)
        .sort((left, right) => {
        const priorityDelta = sourcePriority(left.sheetSource, input.preferRemnants) -
    ...
    for (const piece of sortedPieces) {
        ...
        if (!placed) {
            if (!canFitOnRectangle(piece, input.defaultSheetWidthMm, input.defaultSheetHeightMm, input.allowRotation)) {
                unplacedPieces.push({
                    heightMm: piece.heightMm,
                    label: piece.label,
                    quantity: 1,
                    reason: "The piece exceeds the configured default or material sheet dimensions even after considering rotation.",
                    widthMm: piece.widthMm,
                });
                continue;
            }
            ...
            const virtualSheet = buildSheetState({
                areaM2: areaFromDimensions(input.defaultSheetWidthMm, input.defaultSheetHeightMm),
                code: input.mode === "COMMERCIAL_ESTIMATION"
                    ? `VIRTUAL-${generatedSheetCounter}`
                    : `PURCHASE-${generatedSheetCounter}`,
                heightMm: input.defaultSheetHeightMm,
                inventoryStockId: null,
                remnantPieceId: null,
                sheetSource: input.mode === "COMMERCIAL_ESTIMATION" ? "VIRTUAL" : "PURCHASE_REQUIRED",
                sourceId: null,
            ...
            if (!placePieceOnSheet(virtualSheet, piece, input.allowRotation)) {
                unplacedPieces.push({
                    ...
                    label: piece.label,
                    quantity: 1,
                    reason: "The piece could not be placed even after provisioning a new virtual sheet.",
                    widthMm: piece.widthMm,
                });
                continue;
            }
    ...
    const requiredAreaM2 = roundTo(input.pieces.reduce((total, piece) => total + piece.areaM2, 0), 4);
    const sheetAreaM2 = roundTo(resultSheets.reduce((total, sheet) => total + sheet.sheetAreaM2, 0), 4);
    const wasteAreaM2 = roundTo(resultSheets.reduce((total, sheet) => total + sheet.wasteAreaM2, 0), 4);
    const wastePercent = sheetAreaM2 > 0 ? roundTo((wasteAreaM2 / sheetAreaM2) * 100, 4) : 0;
    if (unplacedPieces.length > 0) {
        groupWarnings.push(`${unplacedPieces.length} piece(s) could not be placed for material "${input.material.name}".`);
    }
    return {
...
const findCandidateSheetsWithWarnings = async (db, materialId, warehouseId, requiredPieces) => {
    if (!warehouseId) {
        return {
            candidates: [],
            warnings: [
    ...
    records.forEach((record) => {
        ...
        if (widthMm === null || heightMm === null) {
            warnings.push(`Inventory stock ${record.id} is missing width or height and was ignored for cutting optimization.`);
            return;
        }
        const baseCandidate = {
        ...
        if (!requiredPieces.some((piece) => canFitOnRectangle({
...
const findCandidateRemnantsWithWarnings = async (db, materialId, warehouseId, requiredPieces) => {
    if (!warehouseId) {
        return {
            candidates: [],
            warnings: [
    ...
    records.forEach((record) => {
        ...
        if (widthMm === null || heightMm === null) {
            warnings.push(`Remnant ${record.code} is missing width or length and was ignored for cutting optimization.`);
            return;
        }
        const baseCandidate = {
...
const getOptimizationRunOrThrow = async (db, runId) => {
...
const getCuttingPlanOrThrow = async (db, cuttingPlanId) => {
...
export const cuttingService = {
    async collectGlassRequirementsFromQuotation(quotationId) {
        ...
        const warnings = [];
        const pieces = [];
        const manualMaterialIds = Array.from(new Set(quotation.items.flatMap((item) => item.itemType === "MANUAL_MATERIAL"
            ? item.materials
                .map((material) => material.materialId)
                .filter((materialId) => Boolean(materialId))
            : [])));
        const manualMaterialTypes = new Map((await prisma.material.findMany({
            select: {
                id: true,
                materialType: true,
                name: true,
        ...
        quotation.items.forEach((item) => {
            const calculationContainer = isObjectRecord(item.calculationResultJson)
                ...
                : null;
            const requestedQuantity = asNumber(calculationContainer?.requestedQuantity) ??
                Number(item.quantity);
            const repeatCount = Math.max(1, Math.round(requestedQuantity));
            if (calculationContainer?.kind === "template_simulation" &&
                isObjectRecord(calculationContainer.result)) {
                const result = calculationContainer.result;
            ...
            if (item.itemType === "MANUAL_MATERIAL") {
                const sheetMaterial = item.materials
                    .map((material) => material.materialId ? manualMaterialTypes.get(material.materialId) : null)
                    .find((material) => material?.materialType === "SHEET");
                if (sheetMaterial) {
                    warnings.push(`Manual quotation item "${item.name}" references sheet material "${sheetMaterial.name}" but does not include cut-piece dimensions, so it was skipped.`);
                }
            }
        });
        return {
    ...
    collectGlassRequirementsFromTemplateSimulation(simulationResult) {
        return collectPiecesFromSimulationResult(simulationResult);
    },
    async findCandidateSheets(materialId, warehouseId, requiredPieces) {
        const result = await findCandidateSheetsWithWarnings(prisma, materialId, warehouseId, requiredPieces);
        return result.candidates;
    },
    async findCandidateRemnants(materialId, warehouseId, requiredPieces) {
        const result = await findCandidateRemnantsWithWarnings(prisma, materialId, warehouseId, requiredPieces);
        return result.candidates;
    },
    calculateSheetWaste,
    async runGlassOptimization(input, userId) {
        ...
        try {
            const materialIds = Array.from(new Set(input.pieces.map((piece) => piece.materialId)));
            if (input.materialId && materialIds.some((materialId) => materialId !== input.materialId)) {
                throw new AppError("All optimized pieces must match the selected material when materialId is provided.", 400);
            }
            const materialMap = await getMaterialMapOrThrow(prisma, materialIds);
            const groupedPieces = buildExpandedPieces(input.pieces, materialMap, allowRotation);
            const warnings = [];
            const groups = [];
            const runtimeStartedAt = Date.now();
            if (input.mode === "OPERATIONAL_PURCHASE" && !input.warehouseId) {
                warnings.push("Operational purchase mode was executed without a warehouse, so inventory and remnant candidates were skipped.");
            }
            for (const [, group] of groupedPieces) {
                ...
                const piecesForCandidateSearch = input.pieces.filter((piece) => piece.materialId === group.material.id);
                const remnantSearch = preferRemnants
                    ? await findCandidateRemnantsWithWarnings(prisma, group.material.id, input.mode === "OPERATIONAL_PURCHASE" ? input.warehouseId : null, piecesForCandidateSearch)
                    : { candidates: [], warnings: [] };
                const inventorySearch = input.mode === "OPERATIONAL_PURCHASE"
                    ? await findCandidateSheetsWithWarnings(prisma, group.material.id, input.warehouseId, piecesForCandidateSearch)
                    : { candidates: [], warnings: [] };
                const groupWarnings = [
            ...
            const totals = groups.reduce((aggregate, group) => {
                aggregate.requiredAreaM2 = roundTo(aggregate.requiredAreaM2 + group.totals.requiredAreaM2, 4);
                aggregate.sheetAreaM2 = roundTo(aggregate.sheetAreaM2 + group.totals.sheetAreaM2, 4);
                aggregate.sheetCount += group.totals.sheetCount;
                aggregate.wasteAreaM2 = roundTo(aggregate.wasteAreaM2 + group.totals.wasteAreaM2, 4);
                return aggregate;
            }, {
                requiredAreaM2: 0,
                sheetAreaM2: 0,
            ...
            const totalWastePercent = totals.sheetAreaM2 > 0
                ? roundTo((totals.wasteAreaM2 / totals.sheetAreaM2) * 100, 4)
                : 0;
            if (totals.sheetCount === 0) {
                throw new AppError("Optimization could not place any piece into available or virtual sheets.", 400);
            }
            const resultJson = {
                allowRotation,
    ...
    async listCuttingOptimizations(query) {
        const where = {
            ...(query.materialId
                ? {
    ...
    async cancelOptimizationRun(runId, userId) {
        return prisma.$transaction(async (db) => {
            ...
            const blockingPlan = run.cuttingPlans.find((plan) => ["APPROVED", "COMPLETED", "SENT_TO_PRODUCTION"].includes(plan.status));
            if (blockingPlan) {
                throw new AppError("Cannot cancel an optimization run that already has an approved or completed cutting plan.", 400);
            }
            const cancelledPlans = await Promise.all(run.cuttingPlans
                .filter((plan) => plan.status !== "CANCELLED")
                .map((plan) => db.cuttingPlan.update({
                data: {
                    status: "CANCELLED",
                },
            ...
            for (const cancelledPlan of cancelledPlans) {
                await auditCuttingAction({
                    ...
                    before: mapPlanSummary(run.cuttingPlans.find((plan) => plan.id === cancelledPlan.id)),
    ...
    async generateCuttingPlanFromOptimizationRun(runId, userId) {
        return prisma.$transaction(async (db) => {
            const run = await getOptimizationRunOrThrow(db, runId);
            ...
            for (const [groupIndex, group] of groupsWithSheets.entries()) {
                const notes = group.unplacedPieces.length > 0
                    ...
                    for (const piece of sheet.pieces) {
                        await db.cuttingPlanPiece.create({
                            data: {
                                ...
                                label: piece.label,
                                materialId: piece.materialId,
                                metadataJson: piece.metadata === undefined || piece.metadata === null
    ...
    async approveCuttingPlan(cuttingPlanId, userId, input) {
        return prisma.$transaction(async (db) => {
            ...
            const allPlansApproved = siblingPlans.every((siblingPlan) => ["APPROVED", "COMPLETED", "SENT_TO_PRODUCTION"].includes(siblingPlan.status));
            if (allPlansApproved) {
    ...
    async createRemnantsFromCuttingPlan(cuttingPlanId, userId) {
        return prisma.$transaction(async (db) => {
            const plan = await getCuttingPlanOrThrow(db, cuttingPlanId);
            if (plan.status !== "APPROVED") {
                throw new AppError("Cutting plan remnants can only be created after approval.", 400);
            }
            if (plan.sheets.some((sheet) => sheet.remnantOutputs.some((output) => output.status === "CREATED"))) {
            ...
            if (!targetWarehouseId) {
                throw new AppError("A warehouse is required before remnants can be created from this cutting plan.", 400);
            }
            const remnantUnit = material.stockUnit === "M2" ? "M2" : material.stockUnit;
            for (const sheet of plan.sheets) {
                for (const [index, output] of sheet.remnantOutputs.entries()) {
                    if (!output.shouldCreateRemnant ||
                    ...
                    const createdRemnant = await inventoryService.createRemnantPiece({
                        ...
                        quantity: remnantUnit === "M2" ? Number(output.areaM2) : 1,
                        sourceId: output.id,
    ...
    async getPrintableCuttingPlan(cuttingPlanId, userId) {
