...
import { TABLERO_VISIBLE_PARA_ROLES, } from "./tableros.constants.js";
...
const OPEN_POSTVENTA_STATUSES = new Set(OPEN_POSTVENTA_STATUS_VALUES);
const ACTIVE_PRODUCTION_STATUS_VALUES = ["READY", "IN_PROGRESS", "PAUSED"];
const ACTIVE_PRODUCTION_STATUSES = new Set(ACTIVE_PRODUCTION_STATUS_VALUES);
...
const ACTIVE_INSTALLATION_STATUSES = new Set(ACTIVE_INSTALLATION_STATUS_VALUES);
...
const buildClientDisplayName = (client) => {
    if (client.clientType === "COMPANY") {
        return (normalizeText(client.commercialName) ||
...
const formatMonthLabel = (date) => `${MONTH_LABELS[date.getUTCMonth()]} ${String(date.getUTCFullYear()).slice(-2)}`;
const buildMonthlyBuckets = (dateFrom, dateTo) => {
...
const resolveEffectiveQuery = (query) => {
...
const buildQuotationWhere = (query, options) => {
    const andFilters = [
        {
            deletedAt: null,
        },
    ...
    const quotationStatusFilter = query.status && QUOTATION_STATUS_SET.has(query.status) ? query.status : null;
    if (query.clientId) {
        andFilters.push({
...
const buildProjectWhere = (query, options = {}) => {
    const andFilters = [
        {
            deletedAt: null,
        },
    ];
    const projectStatusFilter = query.status && PROJECT_STATUS_SET.has(query.status) ? query.status : null;
    if (query.clientId) {
        andFilters.push({
            clientId: query.clientId,
        });
...
const buildProductionWhere = (query, options = {}) => {
    const andFilters = [
        {
            deletedAt: null,
        },
    ];
    const productionStatusFilter = query.status && PRODUCTION_STATUS_SET.has(query.status) ? query.status : null;
    if (query.projectId) {
        andFilters.push({
            projectId: query.projectId,
        });
...
const buildInstallationWhere = (query, options = {}) => {
    const andFilters = [
        {
            deletedAt: null,
        },
    ];
    const installationStatusFilter = query.status && INSTALLATION_STATUS_SET.has(query.status) ? query.status : null;
    if (query.clientId) {
        andFilters.push({
            clientId: query.clientId,
        });
...
const buildPostventaWhere = (query) => {
    const andFilters = [];
    const postventaStatusFilter = query.status && POSTVENTA_STATUS_SET.has(query.status) ? query.status : null;
    const dateRange = buildDateRangeFilter(query.dateFrom, query.dateTo);
    if (query.clientId) {
        andFilters.push({
            clientId: query.clientId,
...
const buildReservationWhere = (query) => {
    const andFilters = [
        {
            status: "ACTIVE",
        },
...
const buildMetaState = (codigo, valorActual, meta) => {
    if (meta === null) {
        return {
            estado: "VIGENTE",
            tendencia: "ESTABLE",
...
const mapSerieValues = (entries) => Array.from(entries.values()).map((item) => ({
...
const sortRanking = (items, take = 6) => [...items]
    .sort((left, right) => right.valor - left.valor)
    .slice(0, take);
const buildLegacyNormalizedReportes = (query) => {
    const filtros = {
...
const buildTableroRecord = () => {
    const timestamp = new Date().toISOString();
    return {
        actualizadoEn: timestamp,
        creadoEn: "2026-07-07T00:00:00.000Z",
        creadoPor: "Sistema",
        descripcion: "Vista consolidada comercial, operativa, productiva y financiera para gerencia de Vidriera Sebitas ERP.",
...
export const tablerosService = {
    async getPanelEjecutivo(query) {
        const effectiveQuery = resolveEffectiveQuery(query);
        const periodo = buildPeriodoLabel(effectiveQuery);
        const now = new Date();
        const movementWhere = {};
        const movementDateRange = buildDateRangeFilter(effectiveQuery.dateFrom, effectiveQuery.dateTo);
        ...
        const [emittedQuotations, acceptedPipelineQuotations, salesQuotations, activeProjectsCount, productionStatusRows, delayedProductionJobs, completedProductionJobs, installationStatusRows, delayedInstallations, completedInstallations, inventoryStocks, reservationsByMaterial, currentPrices, wasteReports, movementCounts, remnantCount, postventaCases, warrantyCount, profitabilityResult,] = await Promise.all([
            ...
            projectProfitabilityService.listProjectProfitability({
                ...
                sortDirection: "desc",
                status: effectiveQuery.status && PROJECT_STATUS_SET.has(effectiveQuery.status)
        ...
        const profitabilityRows = profitabilityResult.data.filter((row) => {
            ...
            if (effectiveQuery.responsibleId &&
        ...
        salesQuotations.forEach((quotation) => {
            ...
            const vendedorId = quotation.project?.salesUser?.id ??
                quotation.createdByUser?.id ??
                "sin-vendedor";
            const vendedorNombre = quotation.project?.salesUser?.name ??
            ...
            ventasPorCliente.set(quotation.client.id, {
                ...
                valor: (ventasPorCliente.get(quotation.client.id)?.valor ?? 0) +
                    monto,
            });
        ...
        const cotizacionesPorEstadoBase = new Map();
        emittedQuotations.forEach((quotation) => {
            cotizacionesPorEstadoBase.set(quotation.status, (cotizacionesPorEstadoBase.get(quotation.status) ?? 0) + 1);
        });
        const cotizacionesPorEstado = [
            {
                color: CHART_COLORS.accent,
                etiqueta: "Aceptadas",
                porcentaje: roundTo(safeDivide(acceptedPipelineQuotations.length, Math.max(emittedQuotations.length, 1)), 4),
                valor: acceptedPipelineQuotations.length,
            },
            {
                color: CHART_COLORS.rose,
                etiqueta: "Perdidas",
                porcentaje: roundTo(safeDivide(emittedQuotations.filter((quotation) => LOST_QUOTATION_STATUSES.has(quotation.status)).length, Math.max(emittedQuotations.length, 1)), 4),
            ...
            },
            {
                ...
                porcentaje: roundTo(safeDivide(emittedQuotations.filter((quotation) => quotation.status === "SENT" ||
                    quotation.status === "APPROVED" ||
                    quotation.status === "PENDING_APPROVAL").length, Math.max(emittedQuotations.length, 1)), 4),
                valor: emittedQuotations.filter((quotation) => quotation.status === "SENT" ||
                    quotation.status === "APPROVED" ||
                    quotation.status === "PENDING_APPROVAL").length,
            },
        ];
        const proyectosGanados = acceptedPipelineQuotations.length;
        const proyectosPerdidos = emittedQuotations.filter((quotation) => LOST_QUOTATION_STATUSES.has(quotation.status)).length;
        const cotizacionesEnSeguimiento = Math.max(emittedQuotations.length - proyectosGanados - proyectosPerdidos, 0);
        ...
        const tiempoPromedioProduccion = roundTo(productionDurations.reduce((sum, value) => sum + value, 0) /
            Math.max(productionDurations.length, 1), 2);
        const instalacionesConCierre = completedInstallations
            .map((order) => {
            const lastTaskAt = order.tasks.reduce((latest, task) => {
                if (!task.completedAt) {
                    return latest;
                }
        ...
        const tiempoPromedioInstalacion = roundTo(instalacionesConCierre.reduce((sum, entry) => {
            return (sum +
                (entry.closedAt.getTime() - entry.scheduledDate.getTime()) /
                    (1000 * 60 * 60 * 24));
        }, 0) / Math.max(instalacionesConCierre.length, 1), 2);
        const cumplimientoProduccion = roundTo(safeDivide(completedProductionJobs.filter((job) => Boolean(job.actualEndDate && job.plannedEndDate) &&
            job.actualEndDate.getTime() <= job.plannedEndDate.getTime()).length, completedProductionJobs.filter((job) => Boolean(job.actualEndDate && job.plannedEndDate))
            .length), 4);
        const cumplimientoInstalacion = roundTo(safeDivide(instalacionesConCierre.filter((entry) => entry.closedAt.getTime() <= entry.scheduledDate.getTime() + 24 * 60 * 60 * 1000).length, instalacionesConCierre.length), 4);
        const cumplimientoFechas = roundTo(safeDivide(cumplimientoProduccion + cumplimientoInstalacion, [cumplimientoProduccion, cumplimientoInstalacion].filter((value) => value > 0).length || 1), 4);
        const reservationMap = new Map(reservationsByMaterial.map((row) => [row.materialId, decimalToNumber(row._sum.quantity)]));
        ...
        const stockBajoRanking = sortRanking(Array.from(stockByMaterial.values())
            .map((entry) => {
        ...
        const rotacionInventario = roundTo(safeDivide(movimientosSalida, Math.max(stockByMaterial.size, 1)), 2);
        const desperdicioGenerado = roundTo(wasteReports.reduce((sum, report) => sum + decimalToNumber(report.actualWasteAreaM2), 0), 2);
        const rentabilidadFiltrada = profitabilityRows;
        const ingresoPresupuestado = roundTo(rentabilidadFiltrada.reduce((sum, row) => sum + row.rentabilidad.ingresoPresupuestado, 0), 2);
        const ingresoReal = roundTo(rentabilidadFiltrada.reduce((sum, row) => sum + row.rentabilidad.ingresoReal, 0), 2);
        const costosReales = roundTo(rentabilidadFiltrada.reduce((sum, row) => sum + row.rentabilidad.totalCostoReal, 0), 2);
        const utilidadBruta = roundTo(rentabilidadFiltrada.reduce((sum, row) => sum + row.rentabilidad.utilidadBruta, 0), 2);
        const margenBruto = roundTo(safeDivide(utilidadBruta, Math.max(ingresoReal, 1)), 4);
        const margenPromedio = roundTo(safeDivide(rentabilidadFiltrada.reduce((sum, row) => sum + row.rentabilidad.margenBruto, 0), Math.max(rentabilidadFiltrada.length, 1)), 4);
        const desperdicioPromedio = roundTo(safeDivide(rentabilidadFiltrada.reduce((sum, row) => sum + row.rentabilidad.desperdicioReal, 0), Math.max(rentabilidadFiltrada.length, 1)), 4);
        const proyectosEnPerdida = rentabilidadFiltrada.filter((row) => row.indicadores.rentabilidadPorProyecto < 0).length;
        const desviacionPresupuesto = roundTo(rentabilidadFiltrada.reduce((sum, row) => sum + row.rentabilidad.diferenciaContraPresupuesto, 0), 2);
        const proyectosEnPerdidaRanking = sortRanking(rentabilidadFiltrada
        ...
        const desviacionProyectos = sortRanking(rentabilidadFiltrada.map((row) => ({
        ...
        const postventaAbiertos = postventaCases.filter((item) => OPEN_POSTVENTA_STATUSES.has(item.status)).length;
        const postventaCerrados = postventaCases.filter((item) => item.status === "CERRADO" || item.closedAt !== null).length;
        ...
        const tiempoResolucionPromedio = roundTo(tiemposResolucion.reduce((sum, value) => sum + value, 0) /
            Math.max(tiemposResolucion.length, 1), 2);
        const costosGarantia = roundTo(postventaCases.reduce((sum, item) => {
            return (sum +
        ...
        const reclamosPorTipoMap = new Map();
        ...
        const indicadoresConMeta = [
            ...
            createIndicador({
                ...
                periodo,
                valorActual: roundTo(safeDivide(acceptedPipelineQuotations.length, Math.max(emittedQuotations.length, 1)), 4),
            }),
            createIndicador({
                categoria: "Comercial",
        ...
        const tarjetas = [
            ...
            createTarjeta({
                ...
                unidad: "porcentaje",
                valor: roundTo(safeDivide(acceptedPipelineQuotations.length, Math.max(emittedQuotations.length, 1)), 4),
            }),
            createTarjeta({
                categoria: "Operaciones",
            ...
            secciones: {
                comercial: {
                    conversion: {
                        ...
                        tasa: roundTo(safeDivide(acceptedPipelineQuotations.length, Math.max(emittedQuotations.length, 1)), 4),
                    },
                    ...
                    proyectosResultado: [
                        {
                            ...
                            etiqueta: "Ganados",
                            porcentaje: roundTo(safeDivide(proyectosGanados, Math.max(emittedQuotations.length, 1)), 4),
                            valor: proyectosGanados,
                        },
                        {
                            color: CHART_COLORS.rose,
                            etiqueta: "Perdidos",
                            porcentaje: roundTo(safeDivide(proyectosPerdidos, Math.max(emittedQuotations.length, 1)), 4),
                            valor: proyectosPerdidos,
                        },
                        {
                            color: CHART_COLORS.slate,
                            etiqueta: "En seguimiento",
                            porcentaje: roundTo(safeDivide(cotizacionesEnSeguimiento, Math.max(emittedQuotations.length, 1)), 4),
                    ...
                    instalacionesPorEstado: installationStatusRows.map((row) => ({
                        color: row.status === "COMPLETED"
                            ...
                            : row.status === "CANCELLED"
                                ...
                                : CHART_COLORS.accent,
                        etiqueta: row.status,
                        porcentaje: roundTo(safeDivide(row._count._all, Math.max(installationStatusRows.reduce((sum, item) => sum + item._count._all, 0), 1)), 4),
                    ...
                    ordenesTrabajoPorEstado: productionStatusRows.map((row) => ({
                        color: row.status === "COMPLETED"
                            ...
                            : row.status === "CANCELLED"
                                ...
                                : CHART_COLORS.accent,
                        etiqueta: row.status,
                        porcentaje: roundTo(safeDivide(row._count._all, Math.max(productionStatusRows.reduce((sum, item) => sum + item._count._all, 0), 1)), 4),
                        valor: row._count._all,
                    })),
                },
                postventa: {
                    estados: [
                        {
                            color: CHART_COLORS.rose,
                            etiqueta: "Abiertos",
                            porcentaje: roundTo(safeDivide(postventaAbiertos, Math.max(postventaCases.length, 1)), 4),
                            valor: postventaAbiertos,
                        },
                        {
                            color: CHART_COLORS.emerald,
                            etiqueta: "Cerrados",
                            porcentaje: roundTo(safeDivide(postventaCerrados, Math.max(postventaCases.length, 1)), 4),
                    ...
                    reclamosPorProyecto: sortRanking(Array.from(reclamosPorProyectoMap.values()).map((entry) => ({
                    ...
                    reclamosPorTipo: Array.from(reclamosPorTipoMap.entries()).map(([tipo, cantidad]) => ({
                        color: tipo === "GARANTIA"
                            ...
                            : tipo === "RECLAMO"
                                ...
                                : CHART_COLORS.slate,
                        etiqueta: tipo,
                        porcentaje: roundTo(safeDivide(cantidad, Math.max(postventaCases.length, 1)), 4),
                        valor: cantidad,
                    })),
                    resumen: [
                        {
...
//# sourceMappingURL=tableros.service.js.map