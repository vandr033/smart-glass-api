...
const decimal = (value) => value === null || value === undefined ? 0 : Number(value);
const date = (value) => value
    ? new Intl.DateTimeFormat("es-BO", { dateStyle: "short", timeZone: "America/La_Paz" }).format(value)
    : "No definida";
const money = (value, currency) => new Intl.NumberFormat("es-BO", { currency, currencyDisplay: "code", minimumFractionDigits: 2, style: "currency" }).format(Number(value ?? 0));
const percent = (value) => `${new Intl.NumberFormat("es-BO", { maximumFractionDigits: 2, minimumFractionDigits: 2 }).format(Number(value ?? 0))}%`;
...
const displayClientName = (client) => client.commercialName ?? client.legalName ?? [client.firstName, client.lastName].filter(Boolean).join(" ") ?? "Cliente sin nombre";
const getString = (value, fallback) => typeof value === "string" && value.trim() ? value.trim() : fallback;
const readCompanyProfile = (value) => {
...
const buildPdfLines = (source, settings, company, variant, versionNumber, generatedBy) => {
    ...
    for (const item of source.items) {
        ...
        lines.push(`${item.name} | ${decimal(item.quantity)} | ${unit} | ${money(decimal(item.subtotalSale) / Math.max(decimal(item.quantity), 1), source.currency)} | ${money(item.subtotalSale, source.currency)}`);
    ...
    lines.push("", "## TOTALES", `Subtotal: ${money(source.subtotalSale, source.currency)}`, `Descuento: ${money(source.discountAmount, source.currency)}`, `Impuestos: ${money(source.taxAmount, source.currency)}`, `Total: ${money(source.totalSale, source.currency)}`, `Moneda: ${source.currency}`, "", "## CONDICIONES COMERCIALES", ...wrapPdfText(source.notes ?? "Sin condiciones comerciales registradas.", 96), "Condiciones de pago: No registradas", "Tiempo estimado de entrega: No registrado", `Validez: ${date(source.validUntil)}`);
    if (internal) {
        lines.push("", "## INFORMACIÓN INTERNA", `Subtotal de costos: ${money(source.subtotalCost, source.currency)}`, `Costo total: ${money(source.subtotalCost, source.currency)}`, `Precio de venta: ${money(source.totalSale, source.currency)}`, `Utilidad estimada: ${money(source.marginAmount, source.currency)}`, `Margen estimado: ${percent(source.marginPercent)}`, `Impacto del descuento: ${money(source.discountAmount, source.currency)}`, "", "## DETALLE DE MATERIALES");
        for (const item of source.items) {
            for (const material of item.materials) {
                lines.push(`${item.name} · ${material.materialCode ?? "Sin código"} · ${material.materialName} · ${decimal(material.requiredQuantity)} ${material.unit} · Costo: ${money(material.totalCost, source.currency)} · Desperdicio: ${percent(material.wastePercent)}`);
                if (material.supplier) {
                    lines.push(`  Proveedor: ${material.supplier.commercialName ?? material.supplier.legalName}`);
                }
            }
        }
        lines.push("", "## APROBACIÓN Y TRAZABILIDAD", `Aprobada por: ${source.approvedByUser?.name ?? "Pendiente"} · Fecha: ${date(source.approvedAt)}`, `Generada por: ${generatedBy}`, ...wrapPdfText(source.internalNotes ?? "Sin observaciones internas registradas.", 96));
    }
    else {
        lines.push("", "## NOTAS PARA EL CLIENTE", ...wrapPdfText(source.notes ?? "Sin notas adicionales para el cliente.", 96));
    }
    lines.push("", "Smart Glass Bolivia · Documento generado desde Vidriera Sebitas ERP");
    return lines;
};
const toPdfSource = (quotation, items) => ({
...
export const quotationPdfService = {
    async generate(quotationId, input) {
