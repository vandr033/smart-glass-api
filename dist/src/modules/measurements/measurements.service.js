...
const mapOpening = (record, evidence) => ({
    code: record.code,
    createdAt: record.createdAt.toISOString(),
...
const mapQuotationSummary = (records) => {
...
const mapProductionJobSummary = (records) => {
    return records.map((record) => ({
        code: record.code,
        id: record.id,
        status: record.status,
...
const toScheduleDateString = (value) => value ? value.toISOString() : null;
const collectVisitMetrics = (visits) => {
    return visits.reduce((result, visit) => {
        result.evidenceCount += visit.evidence.length;
        result.openingCount += visit.openings.length;
        result.openObservationCount += visit.observations.filter((entry) => entry.status === "OPEN" || entry.status === "IN_PROGRESS").length;
...
const buildMeasurementAlerts = (input) => {
    const quotation = [];
    const production = [];
    const statusMessage = input.status === "REJECTED"
...
const mapMeasurementRequestListItem = (record, hasScheduleConflict) => {
...
const mapMeasurementRequestDetail = (record, hasScheduleConflict) => {
...
const assertAddressMatchesClient = async (addressId, clientId, db) => {
    if (!addressId) {
        return;
    }
...
const assertMeasurementRequestHeaderExists = async (requestId, db) => {
...
const assertMeasurementRequestExists = async (requestId, db) => {
    const request = await db.measurementRequest.findFirst({
...
const assertRequestIsMutable = (status) => {
    if (status === "APPROVED") {
        throw new AppError("La solicitud ya fue aprobada. Rechazala o reprogramala para modificarla.", 400);
    }
    if (status === "CANCELLED") {
        throw new AppError("La solicitud esta cancelada y no puede modificarse.", 400);
    }
};
const buildMeasurementRequestWhere = (query) => {
...
const generateMeasurementRequestCode = async (db = prisma, value = new Date()) => {
...
const getLatestVisit = async (requestId, db) => {
...
const hasScheduleOverlap = (leftStart, leftEnd, rightStart, rightEnd) => {
...
const hasMeasurementScheduleConflict = async (request, db) => {
    ...
    return siblingRequests.some((entry) => hasScheduleOverlap(request.scheduledStartTime, request.scheduledEndTime, entry.scheduledStartTime, entry.scheduledEndTime));
};
const createMeasurementStatusHistory = async (input, db) => {
...
const createScheduledVisit = async (input, db) => {
...
const syncEditableRequestStatus = async (requestId, userId, db) => {
    const request = await db.measurementRequest.findFirst({
        include: {
            technicalVisits: {
                include: {
    ...
    const openObservationCount = request.technicalVisits.reduce((sum, visit) => sum +
...
const getMeasurementRequestByIdInternal = async (requestId, db) => {
...
export const measurementsService = {
    async listMeasurementRequests(query) {
        ...
        const conflictStates = await Promise.all(records.map((record) => hasMeasurementScheduleConflict(record, prisma)));
        return {
            data: records.map((record, index) => mapMeasurementRequestListItem(record, conflictStates[index] ?? false)),
            pagination: {
                page: query.page,
    ...
    async listMeasurementCalendar(query) {
        ...
        const conflictStates = await Promise.all(records.map((record) => hasMeasurementScheduleConflict(record, prisma)));
        return records.map((record, index) => mapMeasurementRequestListItem(record, conflictStates[index] ?? false));
    },
    async getMeasurementRequestById(requestId) {
        return getMeasurementRequestByIdInternal(requestId, prisma);
    },
    async createMeasurementRequest(input, userId) {
        ...
        for (let attempt = 0; attempt < 8; attempt += 1) {
            try {
                const requestId = await prisma.$transaction(async (db) => {
                    ...
                    const initialStatus = input.scheduledDate
                        ? "SCHEDULED"
                        : "REQUESTED";
        ...
        throw new AppError("No se pudo generar un codigo unico para la solicitud de medicion.", 500);
    },
    async updateMeasurementRequest(requestId, input, userId) {
        const request = await assertMeasurementRequestHeaderExists(requestId, prisma);
        assertRequestIsMutable(request.status);
        await assertClientExists(input.clientId, prisma);
        await assertProjectMatchesClient(input.projectId, input.clientId, prisma);
        await assertAddressMatchesClient(input.addressId, input.clientId, prisma);
        const nextStatus = request.status === "REQUESTED" && input.scheduledDate ? "SCHEDULED" : request.status;
    ...
    async scheduleMeasurementRequest(requestId, input, userId) {
        const request = await assertMeasurementRequestHeaderExists(requestId, prisma);
        assertRequestIsMutable(request.status);
        const nextStatus = request.scheduledDate || request.status === "RESCHEDULED"
            ? "RESCHEDULED"
            : "SCHEDULED";
        await prisma.$transaction(async (db) => {
            await db.measurementRequest.update({
    ...
    async reprogramMeasurementRequest(requestId, input, userId) {
        const request = await assertMeasurementRequestHeaderExists(requestId, prisma);
        assertRequestIsMutable(request.status);
        await prisma.$transaction(async (db) => {
            await db.measurementRequest.update({
    ...
    async cancelMeasurementRequest(requestId, input, userId) {
        const request = await assertMeasurementRequestHeaderExists(requestId, prisma);
    ...
    async startMeasurementVisit(requestId, input, userId) {
        const request = await assertMeasurementRequestHeaderExists(requestId, prisma);
        assertRequestIsMutable(request.status);
    ...
    async createMeasurementOpening(requestId, input, userId) {
        const request = await assertMeasurementRequestHeaderExists(requestId, prisma);
        assertRequestIsMutable(request.status);
        await prisma.$transaction(async (db) => {
            const latestVisit = await getLatestVisit(requestId, db);
            if (!latestVisit) {
                throw new AppError("Debes iniciar o programar una visita tecnica antes de registrar medidas.", 400);
            }
            ...
            const nextStatus = input.requiresCorrection || request.status === "WITH_OBSERVATIONS"
                ? "WITH_OBSERVATIONS"
                : "REGISTERED";
            await db.measurementRequest.update({
                data: {
    ...
    async updateMeasurementOpening(openingId, input, userId) {
        ...
        const request = await assertMeasurementRequestHeaderExists(opening.measurementVisit.measurementRequestId, prisma);
        assertRequestIsMutable(request.status);
        await prisma.$transaction(async (db) => {
            ...
            await syncEditableRequestStatus(opening.measurementVisit.measurementRequestId, userId, db);
        });
        return this.getMeasurementRequestById(opening.measurementVisit.measurementRequestId);
    },
    async duplicateMeasurementOpening(openingId, userId) {
        const opening = await prisma.measurementOpening.findFirst({
            include: {
                measurementVisit: {
                    select: {
    ...
    async uploadMeasurementEvidence(requestId, metadata, file, userId) {
        const request = await assertMeasurementRequestHeaderExists(requestId, prisma);
        assertRequestIsMutable(request.status);
        ...
        if (metadata.measurementOpeningId) {
            const opening = latestVisit.openings.find((entry) => entry.id === metadata.measurementOpeningId);
            if (!opening) {
        ...
        try {
            ...
            throw new AppError(error instanceof Error
                ? error.message
                : "No se pudo guardar la evidencia de medicion.", 500);
        }
        try {
    ...
    async createTechnicalObservation(requestId, input, userId) {
        const request = await assertMeasurementRequestHeaderExists(requestId, prisma);
        assertRequestIsMutable(request.status);
        const latestVisit = await getLatestVisit(requestId, prisma);
        if (!latestVisit) {
    ...
    async resolveTechnicalObservation(observationId, input, userId) {
        ...
        await prisma.$transaction(async (db) => {
            await db.technicalObservation.update({
                data: {
                    ...
                    resolvedByUserId: input.status === "RESOLVED" || input.status === "REJECTED" ? userId : null,
                    status: input.status,
                },
                where: {
                    id: observationId,
            ...
            await syncEditableRequestStatus(observation.measurementVisit.measurementRequestId, userId, db);
        });
        return this.getMeasurementRequestById(observation.measurementVisit.measurementRequestId);
    },
    async submitMeasurementForApproval(requestId, input, userId) {
        const request = await assertMeasurementRequestHeaderExists(requestId, prisma);
        assertRequestIsMutable(request.status);
        const detail = await assertMeasurementRequestExists(requestId, prisma);
        const openingCount = detail.technicalVisits.reduce((sum, visit) => sum + visit.openings.length, 0);
        if (openingCount === 0) {
            throw new AppError("No puedes enviar a aprobacion una solicitud sin medidas registradas.", 400);
        }
        const openObservationCount = detail.technicalVisits.reduce((sum, visit) => sum +
            visit.observations.filter((entry) => entry.status === "OPEN" || entry.status === "IN_PROGRESS").length, 0);
        const nextStatus = input.result === "REQUIRES_REVISIT" || openObservationCount > 0
            ? "WITH_OBSERVATIONS"
            : "PENDING_APPROVAL";
        await prisma.$transaction(async (db) => {
            const latestVisit = await db.measurementVisit.findFirst({
    ...
    async approveMeasurementRequest(requestId, input, userId) {
        const request = await assertMeasurementRequestHeaderExists(requestId, prisma);
        if (request.status === "APPROVED") {
            return this.getMeasurementRequestById(requestId);
        }
    ...
    async rejectMeasurementRequest(requestId, input, userId) {
        const request = await assertMeasurementRequestHeaderExists(requestId, prisma);
        await prisma.$transaction(async (db) => {
            await db.measurementRequest.update({
                data: {
    ...
    async createQuotationFromMeasurement(requestId, userId) {
        const request = await assertMeasurementRequestExists(requestId, prisma);
        if (request.status !== "APPROVED") {
            throw new AppError("Solo puedes crear cotizaciones desde mediciones aprobadas.", 400);
        }
