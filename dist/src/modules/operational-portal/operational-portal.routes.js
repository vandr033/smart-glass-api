...
import { OPERATIONAL_AREA_PERMISSIONS, OPERATIONAL_PORTAL_ACCESS_PERMISSIONS, } from "./operational-portal.constants.js";
const hasAny = (permissions, required) => required.some((permission) => permissions.includes(permission));
...
operationalPortalRouter.get("/operaciones/permisos", portalAccessGuard, asyncHandler(async (request, response) => {
...
operationalPortalRouter.get("/operaciones/resumen", portalAccessGuard, asyncHandler(async (request, response) => {
    const userId = request.authSession?.user.id ?? "";
    const permissions = request.authorizationSummary?.permissions ?? [];
    const canSeeAll = permissions.includes("operaciones.supervision.ver") ||
        permissions.includes("production.read");
    const [production, productionPending, measurements, installations, notifications, stock, tasks] = await Promise.all([
...
operationalPortalRouter.get("/operaciones/mis-tareas", portalAccessGuard, asyncHandler(async (request, response) => {
    const userId = request.authSession?.user.id ?? "";
...
operationalPortalRouter.post("/operaciones/escanear/resolver", portalAccessGuard, asyncHandler(async (request, response) => {
...
//# sourceMappingURL=operational-portal.routes.js.map