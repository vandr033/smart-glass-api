...
import { PROFILE_CUTTING_PLAN_ENTITY_TYPE, PROFILE_OPTIMIZATION_RUN_ENTITY_TYPE, PROFILE_REMNANT_OUTPUT_ENTITY_TYPE, } from "./profile-optimization.constants.js";
const INVENTORY_PREFER_PROFILE_REMNANTS = "inventory.prefer_profile_remnants";
const INVENTORY_MINIMUM_PROFILE_REMNANT_MM = "inventory.minimum_profile_remnant_mm";
...
const mapProfileCutPiece = (record) => {
    return {
        createdAt: record.createdAt.toISOString(),
...
const mapProfileRemnantOutput = (record) => {
    return {
        createdAt: record.createdAt.toISOString(),
        cuttingBarId: record.cuttingBarId,
        id: record.id,
...
const mapProfileCuttingBar = (record) => {
    return {
        createdAt: record.createdAt.toISOString(),
...
const mapProfileCuttingPlanListItem = (record) => {
    return {
        code: record.code,
        createdAt: record.createdAt.toISOString(),
        id: record.id,
...
const mapProfileCuttingPlanRecord = (record) => {
    return {
        ...mapProfileCuttingPlanListItem(record),
        bars: record.bars.map(mapProfileCuttingBar),
    };
};
const resolveStandardBarLengths = (material) => {
    const barLengths = new Set();
...
const collectProfileCutsFromSimulationResult = (result, options) => {
    ...
    result.cuts.linear.forEach((linearCut) => {
        ...
        if (existing) {
            ...
            existing.totalRequiredLengthMm = roundTo(existing.totalRequiredLengthMm + lengthMm * quantity, 2);
            return;
        }
...
const getProfileSettings = async (db) => {
    ...
    const minimumProfileRemnantMm = asNumber(settingsMap.get(INVENTORY_MINIMUM_PROFILE_REMNANT_MM)) ?? 300;
    const preferProfileRemnants = asBoolean(settingsMap.get(INVENTORY_PREFER_PROFILE_REMNANTS)) ?? true;
    return {
...
const getMaterialOrThrow = async (db, materialId) => {
    ...
    if (barLengths.length === 0) {
        throw new AppError("Linear materials require a standard bar length or dimension preset before optimization.", 400);
    }
    return material;
};
const sourcePriority = (sourceType, preferRemnants) => {
...
const calculateBarCountFromStock = (material, stock) => {
...
const computeOptimization = (input) => {
    const expandedCuts = input.cuts.flatMap((cut) => Array.from({ length: cut.quantity }, () => ({
        cutId: randomUUID(),
        label: cut.label,
    ...
    const remainingRemnants = [...input.availableRemnants].sort((left, right) => left.lengthMm - right.lengthMm);
    const remainingInventoryBars = [...input.availableInventoryBars].sort((left, right) => left.lengthMm - right.lengthMm);
    const bars = [];
    const warnings = [];
    const allocateFromStandardLength = (cutLengthMm) => {
        const selectedLength = input.standardBarLengths.find((lengthMm) => lengthMm >= cutLengthMm);
        if (!selectedLength) {
            throw new AppError(`No standard bar length can satisfy a required cut of ${Math.round(cutLengthMm)} mm.`, 400);
        }
        return {
            cuts: [],
            ...
            sourceType: input.mode === "COMMERCIAL_ESTIMATION"
                ? "VIRTUAL"
                : "PURCHASE_REQUIRED",
    ...
    const openNewBar = (cutLengthMm) => {
        ...
        for (const queue of sourceQueues) {
            const matchingIndex = queue.values.findIndex((candidate) => candidate.lengthMm >= cutLengthMm);
            if (matchingIndex === -1) {
                continue;
            }
            const candidate = queue.values.splice(matchingIndex, 1)[0];
            if (queue.sourceType === "REMNANT") {
                const remnantCandidate = candidate;
                return {
                    cuts: [],
                    inventoryStockId: null,
    ...
    for (const cut of expandedCuts) {
        const bestFit = bars
            .map((bar) => ({
            bar,
            remainingAfterPlacement: roundTo(bar.originalLengthMm - bar.usedLengthMm - cut.lengthMm, 2),
        }))
            ...
            const sourceDiff = sourcePriority(left.bar.sourceType, input.preferRemnants) -
    ...
    const resultBars = bars.map((bar) => {
        ...
        return {
            ...
            wasteLengthMm,
            wastePercent: roundTo(bar.originalLengthMm > 0 ? (wasteLengthMm / bar.originalLengthMm) * 100 : 0, 4),
        };
    });
    const totalRequiredLengthMm = roundTo(expandedCuts.reduce((sum, cut) => sum + cut.lengthMm, 0), 2);
    const totalBarLengthMm = roundTo(resultBars.reduce((sum, bar) => sum + bar.originalLengthMm, 0), 2);
    const totalWasteLengthMm = roundTo(resultBars.reduce((sum, bar) => sum + bar.wasteLengthMm, 0), 2);
    const remnantLengthMm = roundTo(resultBars
        .filter((bar) => bar.sourceType === "REMNANT")
        .reduce((sum, bar) => sum + bar.originalLengthMm, 0), 2);
    const inventoryLengthMm = roundTo(resultBars
        .filter((bar) => bar.sourceType === "INVENTORY_BAR")
        .reduce((sum, bar) => sum + bar.originalLengthMm, 0), 2);
    const purchasedBars = resultBars.filter((bar) => ["PURCHASE_REQUIRED", "VIRTUAL"].includes(bar.sourceType));
    const purchasedLengthMm = roundTo(purchasedBars.reduce((sum, bar) => sum + bar.originalLengthMm, 0), 2);
    const wastePercent = roundTo(totalBarLengthMm > 0 ? (totalWasteLengthMm / totalBarLengthMm) * 100 : 0, 4);
    const efficiencyPercent = roundTo(totalBarLengthMm > 0 ? (totalRequiredLengthMm / totalBarLengthMm) * 100 : 0, 4);
    const purchaseBarsByLength = Array.from(purchasedBars.reduce((map, bar) => {
        const key = String(bar.originalLengthMm);
        map.set(key, (map.get(key) ?? 0) + 1);
        return map;
    }, new Map()).entries()).map(([lengthKey, quantity]) => ({
    ...
    return {
        ...
        mode: input.mode,
        purchaseRequirement: purchasedBars.length > 0
            ? {
...
const getProfileOptimizationRunOrThrow = async (db, runId) => {
...
const getProfileCuttingPlanOrThrow = async (db, planId) => {
...
export const profileOptimizationService = {
    async collectProfileRequirementsFromQuotation(quotationId) {
        ...
        quotation.items.forEach((item) => {
            ...
            if (calculationContainer?.kind === "template_simulation" &&
                ...
                collection.groups.forEach((group) => {
                    ...
                    if (existing) {
                        ...
                        existing.totalRequiredLengthMm = roundTo(existing.totalRequiredLengthMm + group.totalRequiredLengthMm, 2);
                        return;
                    }
            ...
            if (item.itemType === "MANUAL_MATERIAL") {
                ...
                if (linearMaterial) {
                    warnings.push(`Manual quotation item "${item.name}" references linear material "${linearMaterial.name}" but does not include cut lengths, so it was skipped.`);
                }
            }
        });
        const materialIds = Array.from(groupedCuts.keys());
        const materialsById = new Map((await prisma.material.findMany({
            select: materialSummarySelect,
            where: {
                id: {
                    in: materialIds,
        ...
        const groups = materialIds.flatMap((materialId) => {
            ...
            if (!group || !material) {
                warnings.push(`Profile requirements were found for material ${materialId}, but the material record could not be loaded.`);
                return [];
            }
            return [
                {
    ...
    async findUsableProfileRemnants(materialId) {
    ...
    async findAvailableProfileBars(materialId) {
        const material = await getMaterialOrThrow(prisma, materialId);
    ...
    async runProfileOptimization(input, userId) {
        const settings = await getProfileSettings(prisma);
        const preferRemnants = input.preferRemnants ?? settings.preferProfileRemnants;
        const inferredMaterialIds = Array.from(new Set(input.cuts.map((cut) => cut.materialId)));
        ...
        try {
            ...
            const availableInventoryBars = input.mode === "OPERATIONAL_EXECUTION"
                ...
                : [];
            const inventoryBarCandidates = availableInventoryBars.flatMap((bar) => Array.from({ length: bar.quantityBars }, () => ({
                batchNumber: bar.batchNumber,
                id: bar.id,
                lengthMm: bar.lengthMm,
                locationCode: bar.locationCode,
    ...
    async runQuotationProfileOptimizations(quotationId, input, userId) {
        const requirements = await this.collectProfileRequirementsFromQuotation(quotationId);
        if (requirements.groups.length === 0) {
            throw new AppError("No linear profile requirements could be extracted from this quotation.", 400);
        }
        const runs = [];
    ...
    async generateProfileCuttingPlan(runId, userId) {
    ...
    async createProfileRemnants(planId, userId) {
        return prisma.$transaction(async (db) => {
            const plan = await getProfileCuttingPlanOrThrow(db, planId);
            if (plan.bars.some((bar) => bar.remnantOutputs.some((output) => output.status === "CREATED"))) {
    ...
    async listProfileOptimizations(query) {
        const where = {
            ...(query.materialId
                ? {
