...
import { CLIENT_ADDRESS_ENTITY_TYPE, CLIENT_CONTACT_ENTITY_TYPE, CLIENT_ENTITY_TYPE, CLIENTS_PERMISSIONS, } from "./clients.constants.js";
import { clientsService } from "./clients.service.js";
import { clientAddressInputSchema, clientAddressParamsSchema, clientContactInputSchema, clientContactParamsSchema, clientIdParamSchema, clientMutationSchema, listClientsQuerySchema, } from "./clients.validators.js";
...
const getRequiredClientId = (value) => {
...
const getRequiredContactParams = (clientId, contactId) => {
...
const getRequiredAddressParams = (clientId, addressId) => {
    const normalizedClientId = Array.isArray(clientId) ? clientId[0] : clientId;
...
export const clientsController = {
    ...
    async getClientById(request, response) {
        const client = await clientsService.getClientById(getRequiredClientId(request.params.id), {
    ...
    async listClientContacts(request, response) {
        const contacts = await clientsService.listClientContacts(getRequiredClientId(request.params.id));
        sendSuccess(response, contacts);
    },
    async createClientContact(request, response) {
        const clientId = getRequiredClientId(request.params.id);
    ...
    async updateClientContact(request, response) {
        const params = getRequiredContactParams(request.params.id, request.params.contactId);
        const payload = clientContactInputSchema.parse(request.body);
        const result = await clientsService.updateClientContact(params.id, params.contactId, payload);
        await Promise.all([
            logActivityEvent(request, {
                action: CLIENTS_PERMISSIONS.update,
                entityId: result.current.id,
    ...
    async deleteClientContact(request, response) {
        const params = getRequiredContactParams(request.params.id, request.params.contactId);
    ...
    async listClientAddresses(request, response) {
        const addresses = await clientsService.listClientAddresses(getRequiredClientId(request.params.id));
        sendSuccess(response, addresses);
    },
    async createClientAddress(request, response) {
        const clientId = getRequiredClientId(request.params.id);
    ...
    async updateClientAddress(request, response) {
        const params = getRequiredAddressParams(request.params.id, request.params.addressId);
        const payload = clientAddressInputSchema.parse(request.body);
        const result = await clientsService.updateClientAddress(params.id, params.addressId, payload);
        await Promise.all([
            logActivityEvent(request, {
                action: CLIENTS_PERMISSIONS.update,
                entityId: result.current.id,
    ...
    async deleteClientAddress(request, response) {
        const params = getRequiredAddressParams(request.params.id, request.params.addressId);
        const address = await clientsService.deleteClientAddress(params.id, params.addressId);
        await Promise.all([
            logActivityEvent(request, {
                action: CLIENTS_PERMISSIONS.update,
                entityId: address.id,
