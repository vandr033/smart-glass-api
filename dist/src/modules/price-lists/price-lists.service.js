...
import { buildPriceListFileUrl, priceListUploadsDir, } from "../../utils/uploads.js";
import { PRICE_LIST_ACCEPTED_EXTENSIONS, PRICE_LIST_ACCEPTED_MIME_TYPES, } from "./price-lists.constants.js";
...
const toDecimalString = (value, fractionDigits) => {
...
const headerMatchesAlias = (header, alias) => {
    ...
    return (header.startsWith(`${alias} `) ||
...
const findColumnIndex = (normalizedHeaders, aliases, excludedIndexes = new Set()) => {
    let bestIndex = null;
    let bestScore = -1;
...
const resolveColumnMapping = (normalizedHeaders) => {
    const usedIndexes = new Set();
    const supplierSku = findColumnIndex(normalizedHeaders, headerAliasGroups.supplierSku, usedIndexes);
    if (supplierSku !== null) {
        usedIndexes.add(supplierSku);
    }
    const supplierName = findColumnIndex(normalizedHeaders, headerAliasGroups.supplierName, usedIndexes);
    if (supplierName !== null) {
        usedIndexes.add(supplierName);
    }
    const supplierDescription = findColumnIndex(normalizedHeaders, headerAliasGroups.supplierDescription, usedIndexes);
    if (supplierDescription !== null) {
        usedIndexes.add(supplierDescription);
    }
    const supplierUnit = findColumnIndex(normalizedHeaders, headerAliasGroups.supplierUnit, usedIndexes);
    ...
    const currency = findColumnIndex(normalizedHeaders, headerAliasGroups.currency, usedIndexes);
    return {
        currency,
        price,
        supplierDescription,
...
const buildRawJsonObject = (headers, row) => {
    const result = {};
    headers.forEach((header, index) => {
        const cellValue = row[index];
        result[header] = cellValue === undefined || cellValue === "" ? null : cellValue;
    });
    return (toLogJsonValue(result) ?? {});
};
export const normalizePriceListRow = (rawRow, options) => {
    if (!isMeaningfulRow(rawRow)) {
        return null;
    }
    const getRawValue = (index) => {
        if (index === null) {
            return null;
        }
    ...
    const supplierName = getRawValue(options.mapping.supplierName) ??
        getRawValue(options.mapping.supplierDescription) ??
        "";
    const supplierDescription = options.mapping.supplierDescription !== null &&
...
const inferSourceType = (fileName, mimeType) => {
    const extension = path.extname(fileName).toLowerCase();
    if (extension &&
        !PRICE_LIST_ACCEPTED_EXTENSIONS.includes(extension)) {
        throw new AppError("Only .xlsx, .xls, and .csv files are supported.", 400);
    }
    if (mimeType &&
        !PRICE_LIST_ACCEPTED_MIME_TYPES.includes(mimeType)) {
        const allowedByExtension = extension === ".csv" || extension === ".xls" || extension === ".xlsx";
...
export const parsePriceListFile = (file, options) => {
    ...
    const parsedRows = rows
        ...
        .map((row, index) => normalizePriceListRow(row, {
...
const levenshteinDistance = (left, right) => {
    ...
    for (let leftIndex = 1; leftIndex <= left.length; leftIndex += 1) {
        ...
        for (let rightIndex = 1; rightIndex <= right.length; rightIndex += 1) {
            ...
            currentRow[rightIndex] = Math.min((currentRow[rightIndex - 1] ?? 0) + 1, (previousRow[rightIndex] ?? 0) + 1, (previousRow[rightIndex - 1] ?? 0) + cost);
        }
        for (let index = 0; index <= right.length; index += 1) {
...
const textSimilarity = (left, right) => {
    ...
    return Math.max(diceCoefficient(normalizedLeft, normalizedRight), jaccardSimilarity(normalizedLeft, normalizedRight), editSimilarity);
};
const buildAutoMapDecision = (row, options) => {
    ...
    if (normalizedName.length > 0) {
        ...
        const exactMaterial = options.materialByName.get(normalizedName) ??
    ...
    const searchText = normalizeText([row.supplierName, row.supplierDescription].filter(Boolean).join(" "));
    ...
    for (const material of options.materials) {
        const score = Math.max(textSimilarity(searchText, material.normalizedSearchText), textSimilarity(searchText, material.normalizedName), textSimilarity(searchText, material.normalizedCode));
        if (score > bestScore) {
...
const mapPriceListImportRecord = (record) => {
    return {
...
const mapPriceListImportRowRecord = (record) => {
    return {
        createdAt: record.createdAt.toISOString(),
        currency: record.currency,
        ...
        rawJson: record.rawJson && typeof record.rawJson === "object" && !Array.isArray(record.rawJson)
            ? record.rawJson
            : {},
...
const mapSupplierMaterialPriceRecord = (record) => {
    return {
        base: {
...
const mapPriceChangeLogRecord = (record) => {
    return {
...
const getPriceListImportOrThrow = async (importId, options) => {
...
const getMutableImportOrThrow = async (importId, options) => {
...
const getImportCounts = async (importId, options) => {
    const db = options?.tx ?? prisma;
    const [rowCount, mappedCount, unmappedCount, invalidCount, ignoredCount] = await Promise.all([
    ...
    const status = options?.validationMode === true
...
const refreshImportProgress = async (importId, options) => {
    const db = options?.tx ?? prisma;
...
const mapPriceListImportDetail = async (importId) => {
...
const buildImportOrderBy = (sortBy, sortDirection) => {
    switch (sortBy) {
        case "approvedAt":
            return {
...
const buildImportWhereClause = (query) => {
...
const buildImportRowOrderBy = (sortBy, sortDirection) => {
    switch (sortBy) {
        case "mappingStatus":
            return {
...
const buildImportRowWhereClause = (importId, query) => {
    const andClauses = [];
...
const findExactEquivalenceForRow = async (supplierId, row, options) => {
    const db = options?.tx ?? prisma;
    const normalizedName = normalizeText(row.supplierName);
    const normalizedSku = row.supplierSku ? normalizeCompactText(row.supplierSku) : "";
    ...
    if (normalizedSku.length > 0) {
        const match = equivalences.find((equivalence) => equivalence.supplierSku &&
            normalizeCompactText(equivalence.supplierSku) === normalizedSku);
        if (match) {
            return match;
        }
    }
    return (equivalences.find((equivalence) => normalizeText(equivalence.supplierName) === normalizedName) ?? null);
};
export const autoMapPriceListRows = async (importId) => {
    await prisma.$transaction(async (tx) => {
        ...
        const equivalenceBySku = new Map();
        const equivalenceByName = new Map();
        equivalences.forEach((equivalence) => {
            if (equivalence.supplierSku) {
                equivalenceBySku.set(normalizeCompactText(equivalence.supplierSku), equivalence);
            }
            equivalenceByName.set(normalizeText(equivalence.supplierName), equivalence);
        });
        const autoMapMaterials = materials.map((material) => ({
            code: material.code,
            id: material.id,
            name: material.name,
        ...
        for (const row of rows) {
            ...
            await tx.priceListImportRow.update({
                data: {
                    ...
                    validationMessage: row.validationStatus === "INVALID" ? row.validationMessage : null,
                    validationStatus: row.validationStatus === "INVALID"
                        ? "INVALID"
...
export const validatePriceListImport = async (importId) => {
    await prisma.$transaction(async (tx) => {
...
export const approvePriceListImport = async (importId, userId) => {
    const approvalSummary = await prisma.$transaction(async (tx) => {
        ...
        const approvedRows = importRecord.rows.filter((row) => row.mappingStatus !== "IGNORED" &&
        ...
        const lastRowByMaterialId = new Map();
        ...
        const currentPriceByMaterialId = new Map(existingCurrentPrices.map((record) => [record.materialId, record]));
        const approvalTimestamp = new Date();
        const appliedPriceAudits = [];
        ...
        for (const [materialId, row] of lastRowByMaterialId.entries()) {
            ...
            const createdPrice = await tx.supplierMaterialPrice.create({
                data: {
                    conversionFactor: row.supplierMaterialEquivalence?.conversionFactor === null
                        ? null
                        : row.supplierMaterialEquivalence?.conversionFactor
                            ? toDecimalString(decimalToNumber(row.supplierMaterialEquivalence.conversionFactor) ?? 0, 6)
                            : null,
            ...
            const changePercent = previousPriceValue && previousPriceValue > 0
                ? ((nextPriceValue - previousPriceValue) / previousPriceValue) * 100
                : null;
            await tx.priceChangeLog.create({
                data: {
                    changePercent: changePercent === null ? null : toDecimalString(changePercent, 4),
                    importId,
                    materialId,
                    ...
                    oldPrice: previousPriceValue === null ? null : toDecimalString(previousPriceValue, 4),
                    supplierId: importRecord.supplierId,
                },
            });
...
export const getCurrentSupplierPrices = async (materialId) => {
...
export const priceListsService = {
    async createImport(payload, file, importedByUserId) {
            ...
            await prisma.$transaction(async (tx) => {
                ...
                await tx.priceListImportRow.createMany({
                    data: parsedFile.rows.map((row) => ({
                        ...
                        normalizedPrice: row.normalizedPrice === null
                            ? null
    ...
    async listImportRows(importId, query) {
    ...
    async autoMapImportRows(importId) {
        return autoMapPriceListRows(importId);
    },
    async mapImportRow(importId, rowId, input) {
        let supplierMaterialEquivalenceAudit = null;
        await prisma.$transaction(async (tx) => {
            const importRecord = await getMutableImportOrThrow(importId, {
                tx,
            });
            ...
            let equivalenceId = null;
            if (input.createOrUpdateEquivalence) {
                const existingEquivalence = await findExactEquivalenceForRow(importRecord.supplierId, row, {
                    tx,
                });
                if (existingEquivalence) {
                    const updatedEquivalence = await tx.supplierMaterialEquivalence.update({
                        data: {
                            ...
                            status: "ACTIVE",
                            supplierDescription: row.supplierDescription ?? existingEquivalence.supplierDescription,
    ...
    async ignoreImportRow(importId, rowId) {
        await prisma.$transaction(async (tx) => {
            await getMutableImportOrThrow(importId, {
                tx,
            });
    ...
    async approveImport(importId, userId) {
    ...
    async getCurrentSupplierPrices(materialId) {
        return getCurrentSupplierPrices(materialId);
    },
    async listSupplierMaterialPrices(supplierId) {
        const prices = await prisma.supplierMaterialPrice.findMany({
            include: supplierMaterialPriceInclude,
            orderBy: [
                {
