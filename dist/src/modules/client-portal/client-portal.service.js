...
import { buildClientPortalFileUrl, clientPortalUploadsDir, } from "../../utils/uploads.js";
...
import { CLIENT_PORTAL_COOKIE_NAME, CLIENT_PORTAL_DEFAULT_PROJECT_PERMISSIONS, CLIENT_PORTAL_INVITATION_DURATION_MS, CLIENT_PORTAL_PASSWORD_RESET_DURATION_MS, CLIENT_PORTAL_SESSION_DURATION_MS, } from "./client-portal.constants.js";
const portalBaseUrl = `${env.FRONTEND_URL.replace(/\/$/, "")}/portal-cliente`;
const projectStatusProgressMap = {
    LEAD: 10,
...
const dateToIso = (value) => value ? value.toISOString() : null;
const hashPortalToken = (token) => {
    return createHash("sha256").update(token).digest("hex");
};
const createPortalToken = () => {
    return randomBytes(32).toString("hex");
};
const buildPortalSessionToken = (session) => {
...
export const verifyClientPortalSessionToken = (token) => {
    try {
...
const storePortalFile = async (file) => {
...
const buildPortalUserSummary = (context) => {
    return {
        client: {
...
const mapQuotationListItemToPortal = (quotation) => {
    return {
...
const mapQuotationDetailToPortal = (quotation) => {
    return {
        client: quotation.client,
        code: quotation.code,
        createdAt: quotation.createdAt,
...
const resolvePortalPostventaEvidenceType = (file) => {
    if (!file?.mimetype) {
        return "OTRO";
    }
...
const getPortalUserContext = async (userId) => {
...
const ensurePortalUserCanAccess = async (userId) => {
...
const getAccessibleProjectIds = (context) => {
    return context.projectAccesses
        .filter((access) => access.status === "ACTIVO")
        .map((access) => access.projectId);
};
const assertProjectBelongsToPortalUser = (context, projectId) => {
    if (!projectId) {
        return;
    }
...
const buildPortalQuotations = async (context) => {
    ...
    return result.data
        .filter((quotation) => !quotation.project?.id || allowedProjectIds.has(quotation.project.id))
        .map(mapQuotationListItemToPortal);
};
const buildPortalDocuments = async (context) => {
    ...
    const [manualDocuments, projectAttachments, quotations, completedInstallations, warranties] = await Promise.all([
    ...
    const attachments = projectAttachments.map((attachment) => ({
        ...
        documentType: attachment.attachmentType === "CONTRACT"
            ? "CONTRATO"
    ...
    const installationDocuments = completedInstallations.map((installation) => ({
        ...
        downloadKind: "REPORTE_INSTALACION_PDF",
        fileUrl: null,
...
const registerDocumentDownload = async (userId, input) => {
...
export const clientPortalService = {
    ...
    async getAuthenticatedPortalUser(session) {
    ...
    async resetPassword(input) {
        ...
        await prisma.$transaction(async (db) => {
            await db.clientPortalUser.update({
                data: {
                    passwordHash,
                    status: resetToken.user.status === "ACCESO_BLOQUEADO"
                        ? "ACCESO_BLOQUEADO"
                        : "ACTIVO",
                },
                where: {
    ...
    async inviteUser(input, actorUserId) {
        ...
        if (client.projects.length !== input.projectIds.length) {
            throw new AppError("Uno o mas proyectos no pertenecen al cliente seleccionado.", 400);
        }
        ...
        if (existingUser && existingUser.status === "ACTIVO") {
            throw new AppError("Ya existe un usuario cliente activo con ese correo. Actualiza sus accesos desde el modulo.", 400);
        }
    ...
    async updateUser(userId, input) {
        ...
        if (input.projectIds) {
            ...
            if (projectCount !== input.projectIds.length) {
                throw new AppError("Uno o mas proyectos no pertenecen al cliente del usuario.", 400);
            }
        }
        await prisma.$transaction(async (db) => {
            await db.clientPortalUser.update({
    ...
    async createDocument(input, file, actorUserId) {
        const client = await prisma.client.findFirst({
            select: {
                id: true,
            },
    ...
    async createInternalMessage(input, actorUserId, file) {
        const project = await prisma.project.findFirst({
            select: {
                clientId: true,
                code: true,
    ...
    async getDashboard(userId) {
        ...
        const projectIds = getAccessibleProjectIds(context);
        const [quotations, activeProjectCount, installations, documents, openCases, activeWarranties,] = await Promise.all([
            buildPortalQuotations(context),
        ...
        return {
            ...
            contadores: {
                ...
                cotizacionesPendientes: quotations.filter((quotation) => ["APPROVED", "SENT"].includes(quotation.status)).length,
    ...
    async listQuotations(userId) {
        const context = await ensurePortalUserCanAccess(userId);
        return buildPortalQuotations(context);
    },
    async getQuotation(userId, quotationId) {
        const context = await ensurePortalUserCanAccess(userId);
    ...
    async decideQuotation(userId, quotationId, input) {
        const context = await ensurePortalUserCanAccess(userId);
        const quotation = await quotationsService.getQuotationById(quotationId, {
            canViewCost: false,
        });
        ...
        assertProjectBelongsToPortalUser(context, quotation.project?.id);
        const result = await quotationsService.changeQuotationStatus(quotationId, {
            notes: input.motivo,
    ...
    async prepareQuotationPdf(userId, quotationId) {
    ...
    async getInstallation(userId, orderId) {
        const context = await ensurePortalUserCanAccess(userId);
    ...
    async prepareInstallationReport(userId, orderId) {
    ...
    async prepareWarrantyDocument(userId, warrantyId) {
        const context = await ensurePortalUserCanAccess(userId);
    ...
    async listPostventaCases(userId) {
        const context = await ensurePortalUserCanAccess(userId);
        const projectIds = getAccessibleProjectIds(context);
    ...
    async getPostventaCase(userId, caseId) {
        const context = await ensurePortalUserCanAccess(userId);
    ...
    async createPostventaCase(userId, input, file) {
        const context = await ensurePortalUserCanAccess(userId);
        ...
        if (file) {
            await postventaService.createEvidence(created.id, {
                activityId: null,
    ...
    async prepareDocumentDownload(userId, documentId) {
        ...
        await registerDocumentDownload(userId, {
            documentId: document.referenceKey === "MANUAL" ? document.referenceId : null,
    ...
    async createMessage(userId, input, file) {
        const context = await ensurePortalUserCanAccess(userId);
        assertProjectBelongsToPortalUser(context, input.projectId);
        const project = await prisma.project.findFirst({
            select: {
        ...
        const internalUserIds = Array.from(new Set([project.responsibleUserId, project.salesUserId].filter(Boolean)));
...
//# sourceMappingURL=client-portal.service.js.map