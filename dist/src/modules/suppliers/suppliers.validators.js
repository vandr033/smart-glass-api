...
const nullableUrlSchema = (label) => nullableStringSchema(255, label).refine((value) => {
    if (value === null) {
        return true;
    }
    try {
...
const uniqueCriterionIds = (values) => {
    const seenIds = new Set();
    for (const value of values) {
...
const hasOnlyOnePrimaryContact = (values) => values.filter((value) => value.isPrimary).length <= 1;
...
//# sourceMappingURL=suppliers.validators.js.map