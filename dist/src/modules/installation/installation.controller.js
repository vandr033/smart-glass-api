...
import { INSTALLATION_ENTITY_TYPES, INSTALLATION_PERMISSIONS, } from "./installation.constants.js";
import { installationService } from "./installation.service.js";
import { assignInstallationOrderSchema, changeInstallationStatusSchema, createInstallationIssueSchema, createInstallationOrderSchema, installationCalendarQuerySchema, installationEvidenceMetadataSchema, installationIssueIdParamSchema, installationOrderIdParamSchema, installationTaskIdParamSchema, installationTeamIdParamSchema, installationTeamMutationSchema, listInstallationOrdersQuerySchema, projectIdParamSchema, quotationIdParamSchema, resolveInstallationIssueSchema, rescheduleInstallationOrderSchema, updateInstallationOrderSchema, updateInstallationTaskSchema, } from "./installation.validators.js";
...
const getRequiredOrderId = (value) => {
...
const getRequiredIssueId = (value) => {
...
const getRequiredPermissionForStatus = (status) => {
    if (status === "COMPLETED") {
...
export const installationController = {
    async listInstallationOrders(request, response) {
    ...
    async listInstallationCalendar(request, response) {
    ...
    async getInstallationOrderById(request, response) {
        const result = await installationService.getInstallationOrderById(getRequiredOrderId(request.params.id));
        sendSuccess(response, result);
    },
    async createInstallationOrder(request, response) {
        const payload = createInstallationOrderSchema.parse(request.body);
        const actorContext = getRequestLogActorContext(request);
        const result = await installationService.createInstallationOrder(payload, actorContext.userId ?? null);
    ...
    async createInstallationOrderFromProject(request, response) {
        const payload = createInstallationOrderSchema.parse(request.body);
        const actorContext = getRequestLogActorContext(request);
        const { projectId } = projectIdParamSchema.parse(request.params);
        const result = await installationService.createInstallationOrderFromProject(projectId, payload, actorContext.userId ?? null);
        await logActivityEvent(request, {
            action: INSTALLATION_PERMISSIONS.create,
            entityId: result.id,
            entityType: INSTALLATION_ENTITY_TYPES.order,
    ...
    async createInstallationOrderFromQuotation(request, response) {
        const payload = createInstallationOrderSchema.parse(request.body);
        const actorContext = getRequestLogActorContext(request);
        const { quotationId } = quotationIdParamSchema.parse(request.params);
        const result = await installationService.createInstallationOrderFromQuotation(quotationId, payload, actorContext.userId ?? null);
        await logActivityEvent(request, {
            action: INSTALLATION_PERMISSIONS.create,
            entityId: result.id,
            entityType: INSTALLATION_ENTITY_TYPES.order,
    ...
    async updateInstallationOrder(request, response) {
        ...
        const result = await installationService.updateInstallationOrder(installationOrderId, payload);
        await logActivityEvent(request, {
    ...
    async assignInstallationOrder(request, response) {
        const installationOrderId = getRequiredOrderId(request.params.id);
        const payload = assignInstallationOrderSchema.parse(request.body);
        const result = await installationService.assignInstallationOrder(installationOrderId, payload);
        await logActivityEvent(request, {
    ...
    async rescheduleInstallationOrder(request, response) {
        const installationOrderId = getRequiredOrderId(request.params.id);
        const payload = rescheduleInstallationOrderSchema.parse(request.body);
        const actorContext = getRequestLogActorContext(request);
        const result = await installationService.rescheduleInstallationOrder(installationOrderId, payload, actorContext.userId ?? null);
        await logActivityEvent(request, {
    ...
    async changeInstallationOrderStatus(request, response) {
        const installationOrderId = getRequiredOrderId(request.params.id);
        ...
        const actorPermissions = new Set(request.authorizationSummary?.permissions ?? []);
        if (!actorPermissions.has(requiredPermission)) {
            throw new AppError("No tienes permiso para registrar el estado seleccionado.", 403);
        }
        const previous = await installationService.getInstallationOrderById(installationOrderId);
        const result = await installationService.changeInstallationOrderStatus(installationOrderId, payload, actorContext.userId ?? null);
        await logActivityEvent(request, {
    ...
    async updateInstallationTask(request, response) {
        const taskId = getRequiredTaskId(request.params.taskId);
        const payload = updateInstallationTaskSchema.parse(request.body);
        const actorContext = getRequestLogActorContext(request);
        const result = await installationService.updateInstallationTask(taskId, payload, actorContext.userId ?? null);
        await logActivityEvent(request, {
            action: INSTALLATION_PERMISSIONS.update,
            entityId: result.current.id,
    ...
    async completeInstallationTask(request, response) {
        const taskId = getRequiredTaskId(request.params.taskId);
        const actorContext = getRequestLogActorContext(request);
        const actorPermissions = new Set(request.authorizationSummary?.permissions ?? []);
        const result = await installationService.completeInstallationTask(taskId, actorContext.userId ?? null);
    ...
    async createInstallationEvidence(request, response) {
        const installationOrderId = getRequiredOrderId(request.params.id);
        ...
        const actorContext = getRequestLogActorContext(request);
        const result = await installationService.createInstallationEvidence(installationOrderId, metadata, {
    ...
    async createInstallationIssue(request, response) {
        const installationOrderId = getRequiredOrderId(request.params.id);
        const payload = createInstallationIssueSchema.parse(request.body);
        const actorContext = getRequestLogActorContext(request);
        const result = await installationService.createInstallationIssue(installationOrderId, payload, actorContext.userId ?? null);
        await logActivityEvent(request, {
            action: INSTALLATION_PERMISSIONS.execute,
            entityId: result.id,
    ...
    async resolveInstallationIssue(request, response) {
        const issueId = getRequiredIssueId(request.params.issueId);
        const payload = resolveInstallationIssueSchema.parse(request.body);
        const actorContext = getRequestLogActorContext(request);
        const result = await installationService.resolveInstallationIssue(issueId, payload, actorContext.userId ?? null);
        await logActivityEvent(request, {
            action: INSTALLATION_PERMISSIONS.update,
            entityId: result.current.id,
            entityType: INSTALLATION_ENTITY_TYPES.issue,
    ...
    async listInstallationTeams(_request, response) {
        const result = await installationService.listInstallationTeams();
        sendSuccess(response, result);
    },
    async createInstallationTeam(request, response) {
    ...
    async updateInstallationTeam(request, response) {
