...
const installationActiveStatusSet = new Set(INSTALLATION_ORDER_ACTIVE_STATUSES);
...
const buildAddressSummary = (order) => {
    if (order.address) {
        return {
...
const buildReadinessSummary = async (input, db = prisma) => {
    ...
    const productionCompletedCount = productionJobs.filter((job) => job.status === "COMPLETED").length;
    ...
    return {
        ...
        productionReady: productionJobs.length > 0 && productionPendingCount === 0,
        readyReservationCount,
        reservationsReady: activeReservationCount > 0 && readyReservationCount === activeReservationCount,
        warnings,
    };
};
const mapInstallationOrderListItem = async (order, db = prisma) => {
...
const mapInstallationOrderDetail = async (order, db = prisma) => {
...
const getRangeStartMinutes = (value) => {
    return parseTimeToMinutes(value) ?? 0;
};
const getRangeEndMinutes = (value) => {
    return parseTimeToMinutes(value) ?? 24 * 60;
};
const ensureValidScheduleWindow = (scheduledStartTime, scheduledEndTime) => {
...
const findProjectSummaryById = async (projectId, db) => {
...
const findQuotationSummaryById = async (quotationId, db) => {
...
const assertTeamExists = async (teamId, db) => {
...
const assertAddressBelongsToClient = async (addressId, clientId, db) => {
...
const resolveOrderRelations = async (input, db) => {
    const project = input.projectId ? await findProjectSummaryById(input.projectId, db) : null;
    if (input.projectId && !project) {
        throw new AppError("El proyecto seleccionado no existe.", 404);
    }
    const quotation = input.quotationId ? await findQuotationSummaryById(input.quotationId, db) : null;
...
const assertNoSchedulingConflict = async (input, db) => {
    ...
    const conflictingOrders = candidates.filter((candidate) => {
        ...
        return rangesOverlap(input.scheduledStartTime, input.scheduledEndTime, candidate.scheduledStartTime, candidate.scheduledEndTime);
    });
    if (conflictingOrders.length > 0) {
        throw new AppError(`Se detecto un conflicto de agenda con ${conflictingOrders
            .map((order) => order.code)
            .join(", ")}.`, 409);
    }
};
const buildOrderWhereClause = (query) => {
...
const getInstallationEvidenceRequirement = async (db) => {
...
const createStatusHistoryEntry = async (db, input) => {
    await db.installationStatusHistory.create({
        data: {
            changedByUserId: input.changedByUserId,
            fromStatus: input.fromStatus,
            installationOrderId: input.installationOrderId,
            metadataJson: input.metadataJson === undefined || input.metadataJson === null
...
const buildTaskCreateData = (orderId, tasks) => {
    const sourceTasks = tasks.length > 0
        ? tasks
...
const generateInstallationOrderCode = async (db = prisma, value = new Date()) => {
...
const createOrderWithUniqueCode = async (run) => {
...
const findInstallationOrderHeaderOrThrow = async (installationOrderId, db) => {
...
const findInstallationTaskOrThrow = async (taskId, db) => {
...
const findInstallationIssueOrThrow = async (issueId, db) => {
...
const assertCompletionRules = async (installationOrderId, db) => {
    ...
    if (pendingTaskCount > 0) {
        throw new AppError("No puedes completar la instalacion mientras existan tareas pendientes.", 400);
    }
    if (requireEvidence && evidenceCount === 0) {
        throw new AppError("La configuracion exige al menos una evidencia antes de completar la instalacion.", 400);
    }
};
const assertTransitionAllowed = (currentStatus, nextStatus) => {
    ...
    if (!allowedTransitions.includes(nextStatus)) {
        throw new AppError(`No se puede cambiar el estado de ${currentStatus} a ${nextStatus}.`, 400);
    }
};
const buildEvidenceFileName = (originalName) => {
    const extension = path.extname(originalName).toLowerCase() || ".bin";
    return `installation-${Date.now()}-${randomUUID()}${extension}`;
};
const assertUsersExist = async (userIds, db) => {
    if (userIds.length === 0) {
        return;
    }
...
export const installationService = {
    async listInstallationOrders(query) {
        ...
        const data = await Promise.all(orders.map((order) => mapInstallationOrderListItem(order)));
        return {
            data,
            pagination: {
                page: query.page,
    ...
    async listInstallationCalendar(query) {
    ...
    async getInstallationOrderById(installationOrderId) {
    ...
    async createInstallationOrder(input, userId) {
    ...
    async createInstallationOrderFromProject(projectId, input, userId) {
        return this.createInstallationOrder({
            ...input,
            projectId,
        }, userId);
    },
    async createInstallationOrderFromQuotation(quotationId, input, userId) {
        return this.createInstallationOrder({
            ...input,
            quotationId,
        }, userId);
    },
    async updateInstallationOrder(installationOrderId, input) {
    ...
    async assignInstallationOrder(installationOrderId, input) {
        const previous = await this.getInstallationOrderById(installationOrderId);
        const header = await findInstallationOrderHeaderOrThrow(installationOrderId, prisma);
        if (input.assignedTeamId) {
            await assertTeamExists(input.assignedTeamId, prisma);
    ...
    async rescheduleInstallationOrder(installationOrderId, input, userId) {
        const previous = await this.getInstallationOrderById(installationOrderId);
        const header = await findInstallationOrderHeaderOrThrow(installationOrderId, prisma);
        await assertNoSchedulingConflict({
    ...
    async changeInstallationOrderStatus(installationOrderId, input, userId) {
        const header = await findInstallationOrderHeaderOrThrow(installationOrderId, prisma);
    ...
    async updateInstallationTask(taskId, input, userId) {
    ...
    async completeInstallationTask(taskId, userId) {
        const task = await findInstallationTaskOrThrow(taskId, prisma);
        const updated = await prisma.installationTask.update({
            include: taskInclude,
            data: {
    ...
    async createInstallationEvidence(installationOrderId, metadata, file, userId) {
    ...
    async createInstallationIssue(installationOrderId, input, userId) {
        const header = await findInstallationOrderHeaderOrThrow(installationOrderId, prisma);
        const issueId = await prisma.$transaction(async (db) => {
            ...
            if (header.status !== "WITH_OBSERVATIONS" &&
    ...
    async resolveInstallationIssue(issueId, input, userId) {
        const issue = await findInstallationIssueOrThrow(issueId, prisma);
        const resolvedAt = input.status === "RESOLVED" || input.status === "CLOSED" ? new Date() : null;
        const resolvedByUserId = input.status === "RESOLVED" || input.status === "CLOSED" ? userId : null;
        const updated = await prisma.installationIssue.update({
            include: issueInclude,
            data: {
                description: input.notes && input.notes.length > 0
    ...
    async createInstallationTeam(input) {
    ...
    async updateInstallationTeam(teamId, input) {
    ...
    async softDeleteInstallationOrder(installationOrderId) {
        const previous = await this.getInstallationOrderById(installationOrderId);
        await prisma.installationOrder.update({
            data: {
                deletedAt: new Date(),
