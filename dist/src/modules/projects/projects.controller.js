...
import { projectsService, PROJECT_ENTITY_TYPES, transitionProjectStatus, } from "./projects.service.js";
import { listProjectsQuerySchema, projectAttachmentInputSchema, projectAttachmentParamsSchema, projectFileUploadSchema, projectIdParamSchema, projectMeasurementInputSchema, projectMeasurementParamsSchema, projectMutationSchema, projectNoteInputSchema, projectNoteParamsSchema, projectTransitionSchema, } from "./projects.validators.js";
...
const getRequiredNoteParams = (projectId, noteId) => {
...
const getRequiredAttachmentParams = (projectId, attachmentId) => {
    const normalizedProjectId = Array.isArray(projectId) ? projectId[0] : projectId;
...
const getRequiredMeasurementParams = (projectId, measurementId) => {
    const normalizedProjectId = Array.isArray(projectId) ? projectId[0] : projectId;
...
export const projectsController = {
    ...
    async getProjectById(request, response) {
        const project = await projectsService.getProjectById(getRequiredProjectId(request.params.id), {
    ...
    async deleteProject(request, response) {
        const project = await projectsService.deleteProject(getRequiredProjectId(request.params.id));
        await logActivityEvent(request, {
    ...
    async transitionProject(request, response) {
        const projectId = getRequiredProjectId(request.params.id);
    ...
    async getProjectStatusHistory(request, response) {
        const history = await projectsService.listProjectStatusHistory(getRequiredProjectId(request.params.id));
        sendSuccess(response, history);
    },
    async listProjectNotes(request, response) {
        const notes = await projectsService.listProjectNotes(getRequiredProjectId(request.params.id));
        sendSuccess(response, notes);
    },
    async createProjectNote(request, response) {
        const projectId = getRequiredProjectId(request.params.id);
        const payload = projectNoteInputSchema.parse(request.body);
        const note = await projectsService.addProjectNote(projectId, payload, request.authSession?.user.id ?? null);
        await logActivityEvent(request, {
            action: PROJECTS_PERMISSIONS.update,
            entityId: note.id,
    ...
    async updateProjectNote(request, response) {
        const params = getRequiredNoteParams(request.params.id, request.params.noteId);
        const payload = projectNoteInputSchema.parse(request.body);
        const result = await projectsService.updateProjectNote(params.id, params.noteId, payload);
        await logActivityEvent(request, {
            action: PROJECTS_PERMISSIONS.update,
            entityId: result.current.id,
            entityType: PROJECT_ENTITY_TYPES.note,
    ...
    async deleteProjectNote(request, response) {
        const params = getRequiredNoteParams(request.params.id, request.params.noteId);
    ...
    async listProjectAttachments(request, response) {
        const attachments = await projectsService.listProjectAttachments(getRequiredProjectId(request.params.id));
        sendSuccess(response, attachments);
    },
    async createProjectAttachment(request, response) {
        const projectId = getRequiredProjectId(request.params.id);
        ...
        const attachment = await projectsService.addProjectAttachment(projectId, payload, {
            buffer: file.buffer,
    ...
    async deleteProjectAttachment(request, response) {
        const params = getRequiredAttachmentParams(request.params.id, request.params.attachmentId);
        const attachment = await projectsService.deleteProjectAttachment(params.id, params.attachmentId);
        await logActivityEvent(request, {
            action: PROJECTS_PERMISSIONS.update,
            entityId: attachment.id,
            entityType: PROJECT_ENTITY_TYPES.attachment,
    ...
    async listProjectMeasurements(request, response) {
        const measurements = await projectsService.listProjectMeasurements(getRequiredProjectId(request.params.id));
        sendSuccess(response, measurements);
    },
    async createProjectMeasurement(request, response) {
        const projectId = getRequiredProjectId(request.params.id);
        const payload = projectMeasurementInputSchema.parse(request.body);
        const measurement = await projectsService.addProjectMeasurement(projectId, payload, request.authSession?.user.id ?? null);
        await logActivityEvent(request, {
            action: PROJECTS_PERMISSIONS.update,
    ...
    async updateProjectMeasurement(request, response) {
        const params = getRequiredMeasurementParams(request.params.id, request.params.measurementId);
        const payload = projectMeasurementInputSchema.parse(request.body);
        const result = await projectsService.updateProjectMeasurement(params.id, params.measurementId, payload);
        await logActivityEvent(request, {
            action: PROJECTS_PERMISSIONS.update,
            entityId: result.current.id,
            entityType: PROJECT_ENTITY_TYPES.measurement,
    ...
    async deleteProjectMeasurement(request, response) {
        const params = getRequiredMeasurementParams(request.params.id, request.params.measurementId);
        const measurement = await projectsService.deleteProjectMeasurement(params.id, params.measurementId);
        await logActivityEvent(request, {
            action: PROJECTS_PERMISSIONS.update,
            entityId: measurement.id,
            entityType: PROJECT_ENTITY_TYPES.measurement,
    ...
    async getDashboardSummary(_request, response) {
        const summary = await projectsService.getDashboardSummary();
        sendSuccess(response, summary);
    },
    async listProjectUserOptions(_request, response) {
