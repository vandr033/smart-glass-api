...
import {
  DAMAGED_MATERIAL_ENTITY_TYPE,
  INVENTORY_RESERVATION_ENTITY_TYPE,
  INVENTORY_STOCK_ENTITY_TYPE,
  REMNANT_PIECE_ENTITY_TYPE,
  WAREHOUSE_ENTITY_TYPE,
} from "./inventory.constants.js";
import type {
  AdjustStockInput,
  AvailabilityMaterialQuery,
  CreateRemnantPieceInput,
  ...
  ListDamagedMaterialsQuery,
  ListInventoryMovementsQuery,
  ListInventoryStockQuery,
  ListRemnantsQuery,
  ListReservationsQuery,
  ListWarehousesQuery,
  MarkDamagedMaterialInput,
  MaterialAvailabilityRecord,
  RemnantPieceRecord,
  ReservationInput,
  ReviewDamagedMaterialInput,
...
} satisfies Prisma.RemnantPieceInclude;
...
} satisfies Prisma.DamagedMaterialInclude;

type InventoryStockEntity = Prisma.InventoryStockGetPayload<{
  include: typeof inventoryStockInclude;
}>;

type InventoryMovementEntity = Prisma.InventoryMovementGetPayload<{
  include: typeof inventoryMovementInclude;
}>;

type InventoryReservationEntity = Prisma.InventoryReservationGetPayload<{
  include: typeof inventoryReservationInclude;
}>;

type RemnantEntity = Prisma.RemnantPieceGetPayload<{
  include: typeof remnantInclude;
}>;

type DamagedMaterialEntity = Prisma.DamagedMaterialGetPayload<{
  include: typeof damagedMaterialInclude;
}>;
...
const materialBehaviorFromRecord = (
  material: Pick<
    MaterialForInventory,
    "code" | "materialType" | "name" | "unitConversionJson"
  >,
): Pick<MaterialBehaviorInput, "materialType" | "unitConversionJson"> & {
  code?: string;
  name?: string;
} => {
  return {
...
const mapWarehouseSummary = (
  warehouse: Prisma.InventoryStockGetPayload<{
    include: typeof inventoryStockInclude;
  }>["warehouse"],
): WarehouseSummary => {
  return {
...
const mapMaterialSummary = (
  material: InventoryStockEntity["material"],
): InventoryMaterialSummary => {
  return {
    category: {
...
const mapUserSummary = (
  user:
    | InventoryMovementEntity["createdByUser"]
    | InventoryReservationEntity["reservedByUser"]
    | DamagedMaterialEntity["reportedByUser"],
) => {
  if (!user) {
    return null;
  }
...
const isSoftReservableStockType = (stockType: InventoryStockType): boolean => {
  return stockType === "STANDARD" || stockType === "REMNANT";
};

const getAvailableQuantityForStock = (
  stock: Pick<
    InventoryStockEntity,
    "condition" | "quantity" | "stockType"
  >,
): number => {
  if (
    stock.stockType === "RESERVED" ||
...
const calculateStockMetrics = (
  stock: Pick<
    InventoryStockEntity,
    "condition" | "id" | "quantity" | "stockType"
  >,
  softReservationMap: Map<string, number>,
) => {
  const quantity = Number(stock.quantity);
  const reservedSoftQuantity = roundQuantity(softReservationMap.get(stock.id) ?? 0);
  const reservedFirmQuantity =
    stock.stockType === "RESERVED" ? roundQuantity(quantity) : 0;
...
const mapRemnantPiece = (remnant: RemnantEntity): RemnantPieceRecord => {
  return {
    code: remnant.code,
...
const mapDamagedMaterial = (
  damagedMaterial: DamagedMaterialEntity,
): DamagedMaterialRecord => {
  return {
...
const buildWarehouseWhereClause = (
...
const buildInventoryStockWhereClause = (
  query: ListInventoryStockQuery,
...
const buildInventoryStockOrderBy = (
  query: ListInventoryStockQuery,
): Prisma.InventoryStockOrderByWithRelationInput[] => {
  switch (query.sortBy) {
    case "material":
      return [
        {
...
const buildMovementWhereClause = (
...
const buildReservationWhereClause = (
...
const buildRemnantWhereClause = (
  query: ListRemnantsQuery,
): Prisma.RemnantPieceWhereInput => {
  return {
    ...(query.warehouseId
      ? {
          warehouseId: query.warehouseId,
...
const buildDamagedWhereClause = (
  query: ListDamagedMaterialsQuery,
): Prisma.DamagedMaterialWhereInput => {
  return {
    ...(query.warehouseId
      ? {
          warehouseId: query.warehouseId,
...
const getInventoryStockOrThrow = async (
  db: DbClient,
...
const getDamagedMaterialOrThrow = async (
  db: DbClient,
...
const getRemnantOrThrow = async (
  db: DbClient,
  remnantPieceId: string,
) => {
...
const validateDimensionsForMaterial = (
  material: MaterialForInventory,
  input: {
  ...
  if (material.materialType === "SHEET") {
    ...
    const hasLength =
      input.lengthMm !== null && input.lengthMm !== undefined;

    if (input.widthMm === null || input.widthMm === undefined) {
      throw new AppError(
        "Sheet materials require a width in millimeters.",
        400,
      );
    }
    ...
    if (input.thicknessMm === null || input.thicknessMm === undefined) {
      throw new AppError(
        "Sheet materials require a thickness in millimeters.",
        400,
      );
    }
  }
  ...
  if (material.materialType === "LINEAR") {
    if (input.lengthMm === null || input.lengthMm === undefined) {
      throw new AppError(
        "Linear materials require a length in millimeters.",
        400,
      );
    }
  }
...
const getSoftReservationCapacity = (
  stock: Pick<InventoryStockEntity, "condition" | "id" | "quantity" | "stockType">,
  softReservationMap: Map<string, number>,
) => {
...
const auditInventoryAction = async (
  input: {
    action: string;
...
const getActiveSoftReservationsByStock = async (
  db: DbClient,
  options: {
  ...
  const byStockId = new Map<string, number>();
  ...
  reservations.forEach((reservation) => {
    ...
    if (reservation.inventoryStockId) {
      byStockId.set(
        ...
        roundQuantity((byStockId.get(reservation.inventoryStockId) ?? 0) + quantity),
      );
      return;
    }
...
const updateSoftReservationCondition = async (
  db: DbClient,
  stockId: string,
) => {
...
const createOrRestoreStock = async (
  db: DbClient,
  input: {
  ...
  if (existingStock) {
    return db.inventoryStock.update({
      data: {
        quantity: new PrismaNamespace.Decimal(
          roundQuantity(Number(existingStock.quantity) + input.quantity),
        ),
      },
      where: {
        id: existingStock.id,
...
const syncRemnantStatusBySourceId = async (
  db: DbClient,
  remnantId: string,
...
const getDefaultSoftReservationHours = async (db: DbClient) => {
  ...
  const value =
    typeof setting.valueJson === "number" ? setting.valueJson : Number(setting.valueJson);

  return Number.isFinite(value) && value > 0 ? value : 48;
};

const expireSoftReservations = async (
  db: DbClient,
  scope?: {
    materialId?: string | undefined;
    warehouseId?: string | undefined;
...
const getReservationTargetStock = async (
  db: DbClient,
  input: ReservationInput,
  material: MaterialForInventory,
  options: {
    allowMaterialLevelSoftReservation?: boolean;
    reservationType: "FIRM" | "SOFT";
  },
) => {
  ...
  const requestedMaterialQuantity = convertQuantity(
    material,
    input.quantity,
    input.unit,
    candidateStocks[0]?.unit ?? input.unit,
  );

  const singleStock = candidateStocks.find((stock) => {
    const requestedQuantity = convertQuantity(
      material,
      input.quantity,
      input.unit,
      stock.unit,
  ...
  if (singleStock) {
    return {
      canReserve: true,
      requestedQuantityInStockUnit: convertQuantity(
        material,
        input.quantity,
        input.unit,
        singleStock.unit,
...
const decrementStockQuantity = async (
  db: DbClient,
  stockId: string,
...
const buildAvailabilitySummary = (
  ...
  softReservationState: {
    byStockId: Map<string, number>;
    unassignedQuantity: number;
  },
): InventoryAvailabilitySummary => {
  return stocks.reduce<InventoryAvailabilitySummary>(
    (summary, stock) => {
      const quantity = Number(stock.quantity);
      ...
      summary.availableQuantity = roundQuantity(
        summary.availableQuantity + metrics.availableQuantity,
      );
      summary.reservedFirmQuantity = roundQuantity(
        summary.reservedFirmQuantity + metrics.reservedFirmQuantity,
      );
      summary.reservedSoftQuantity = roundQuantity(
        summary.reservedSoftQuantity + metrics.reservedSoftQuantity,
      );
...
const buildRequestedQuantitySufficiency = (
  material: MaterialForInventory,
  summary: InventoryAvailabilitySummary,
  input: AvailabilityMaterialQuery,
) => {
  ...
  const requestedQuantity = convertQuantity(
    ...
    input.quantity,
    input.unit,
    material.materialType === "SERVICE" ? input.unit : input.unit,
  );
...
export const inventoryService = {
  async listWarehouses(query: ListWarehousesQuery): Promise<WarehouseRecord[]> {
  ...
  async deleteWarehouse(
    ...
    if (activeStockCount > 0) {
      throw new AppError(
        "Cannot delete a warehouse that still has active stock.",
        400,
      );
    }
  ...
  async getDashboard(): Promise<InventoryDashboardRecord> {
    ...
    const [materialRows, lowStockRows, activeReservationCount, damagedCount, remnantCount, recentMovements] =
  ...
  async listInventoryStock(query: ListInventoryStockQuery) {
    await expireSoftReservations(prisma, {
      materialId: query.materialId,
      warehouseId: query.warehouseId,
    });
  ...
  async listInventoryMovements(query: ListInventoryMovementsQuery) {
  ...
  async getAvailableStock(
    materialId: string,
    filters: AvailabilityMaterialQuery,
    ...
    return {
      material: mapMaterialSummary(material as InventoryStockEntity["material"]),
      ...
      sufficientForRequestedQuantity: buildRequestedQuantitySufficiency(
        material,
        summary,
        filters,
      ),
  ...
  async getAvailableGlassSheets(
    ...
    return {
      matchingRemnants,
      matchingStocks: stockMatches.map((stock) =>
        mapInventoryStock(stock, softReservationState.byStockId),
      ),
      material: mapMaterialSummary(material as InventoryStockEntity["material"]),
  ...
  async getAvailableLinearStock(
    ...
    return {
      material: mapMaterialSummary(material as InventoryStockEntity["material"]),
      matchingStocks: matchingStocks.map((stock) =>
        mapInventoryStock(stock, softReservationState.byStockId),
      ),
      requiredLengthMm,
  ...
  async createSoftReservation(
    ...
    return prisma.$transaction(async (db) => {
      ...
      if (!target.canReserve) {
        throw new AppError(
          "Cannot reserve more quantity than the available stock.",
          400,
        );
      }

      const expiresAt =
        input.expiresAt ?? addHours(new Date(), await getDefaultSoftReservationHours(db));
  ...
  async createFirmReservation(
    ...
    return prisma.$transaction(async (db) => {
      ...
      if (!target.canReserve || !target.stock || target.requestedQuantityInStockUnit === null) {
        throw new AppError(
          "Firm reservations require a specific stock row with enough available quantity.",
          400,
        );
      }
    ...
    return prisma.$transaction(async (db) => {
      ...
      if (reservation.reservationType === "FIRM") {
        ...
        await createOrRestoreStock(db, {
          ...
          stockType:
            reservedStock.sourceType === "REMNANT_GENERATED" ? "REMNANT" : "STANDARD",
      ...
      if (reservation.reservationType === "FIRM") {
        ...
        const stock =
          reservation.inventoryStockId
            ? await getInventoryStockOrThrow(db, reservation.inventoryStockId)
            : (
                await getReservationTargetStock(
                  db,
                  {
                    expiresAt: null,
        ...
        if (!stock) {
          throw new AppError(
            "No matching stock row could be found to consume this reservation.",
            400,
          );
        }

        const quantityToConsume = convertQuantity(
          material,
  ...
  async createRemnantPiece(
    input: CreateRemnantPieceInput,
    userId: string | null,
  ...
  async findUsableRemnants(
    ...
    return remnants
      .filter((remnant) => {
        ...
        return (
          (remnantWidth >= requiredWidthMm && remnantLength >= requiredHeightMm) ||
  ...
  async listRemnants(query: ListRemnantsQuery) {
  ...
  async markMaterialDamaged(
    input: MarkDamagedMaterialInput,
    userId: string | null,
  ): Promise<DamagedMaterialRecord> {
    return prisma.$transaction(async (db) => {
      const material = await getMaterialOrThrow(db, input.materialId);
      await getWarehouseOrThrow(db, input.warehouseId);
      const target = await getReservationTargetStock(
        db,
        {
          expiresAt: null,
      ...
      if (!target.canReserve || !target.stock || target.requestedQuantityInStockUnit === null) {
        throw new AppError(
          "Cannot mark more material as damaged than the current available stock.",
          400,
        );
      }
  ...
  async reviewDamagedMaterial(
    damagedMaterialId: string,
    input: ReviewDamagedMaterialInput,
    userId: string | null,
  ): Promise<DamagedMaterialRecord> {
    return prisma.$transaction(async (db) => {
      ...
      if (input.status === "REUSABLE") {
        ...
        await createOrRestoreStock(db, {
          ...
          sourceType: "RETURN",
          stockType:
            damagedStock.sourceType === "REMNANT_GENERATED" ? "REMNANT" : "STANDARD",
  ...
  async scrapMaterial(
    ...
    damagedMaterial?: DamagedMaterialRecord;
    remnant?: RemnantPieceRecord;
  }> {
    return prisma.$transaction(async (db) => {
  ...
  async listDamagedMaterials(query: ListDamagedMaterialsQuery) {
