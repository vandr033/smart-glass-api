...
import type {
  ...
  CostoProyectoRecord,
  CostoProyectoOrigen,
  EventoRentabilidadRecord,
  IndicadoresRentabilidadRecord,
  ListProjectProfitabilityDashboardQuery,
  ListProjectProfitabilityQuery,
  RentabilidadAgrupadaRecord,
  RentabilidadClientSummary,
  RentabilidadProjectSummary,
  ...
  RentabilidadProyectoRecord,
  RentabilidadUserSummary,
  VariacionRentabilidadRecord,
} from "./project-profitability.types.js";
import type { ALERTA_RENTABILIDAD_TIPOS } from "./project-profitability.constants.js";

type MaterialCostCandidate = {
  createdAt: Date;
  material: MaterialCostRecord;
  unit: string;
  unitCost: number;
};

type MaterialCostRecord = {
  code: string;
  defaultWastePercent: PrismaNamespace.Decimal | null;
  id: string;
...
type QuotationMaterialLine = {
  createdAt: Date;
  id: string;
  material: MaterialCostRecord | null;
  materialId: string | null;
  materialName: string;
  metadataJson: Prisma.JsonValue | null;
  requiredQuantity: PrismaNamespace.Decimal;
  totalCost: PrismaNamespace.Decimal;
  unit: string;
  unitCost: PrismaNamespace.Decimal | null;
  wastePercent: PrismaNamespace.Decimal | null;
};

const VALID_MATERIAL_UNITS = new Set<string>([
  "MM",
  "CM",
  "M",
  "M2",
...
type ProjectProfitabilityEntity = Prisma.ProjectGetPayload<{
  include: typeof projectProfitabilityInclude;
}>;

type PurchaseOrderProfitabilityEntity = Prisma.PurchaseOrderGetPayload<{
...
const safeDivide = (dividend: number, divisor: number): number | null => {
...
const getJsonRecord = (value: Prisma.JsonValue | null): Record<string, unknown> | null => {
...
const getMaterialBehaviorInput = (material: MaterialCostRecord) => ({
  code: material.code,
...
const convertUnitCost = (
  candidate: MaterialCostCandidate,
  targetUnit: string,
): number | null => {
  ...
  if (
    !VALID_MATERIAL_UNITS.has(candidate.unit) ||
  ...
  try {
    const sourceQuantityForTargetUnit = convertMaterialUnit({
      fromUnit: targetUnit as MaterialUnit,
      material: getMaterialBehaviorInput(candidate.material),
      quantity: 1,
      toUnit: candidate.unit as MaterialUnit,
    });
...
const buildClientSummary = (
  client: ProjectProfitabilityEntity["client"],
): RentabilidadClientSummary => ({
...
const buildProjectSummary = (
  project: ProjectProfitabilityEntity,
): RentabilidadProjectSummary => ({
...
const chooseBudgetQuotation = (project: ProjectProfitabilityEntity) => {
  const preferred = project.quotations.find((quotation) =>
    ["ACCEPTED", "APPROVED", "SENT"].includes(quotation.status),
  );

  return preferred ?? project.quotations[0] ?? null;
};

const chooseActualQuotation = (project: ProjectProfitabilityEntity) => {
  const preferred = project.quotations.find((quotation) =>
    ["ACCEPTED", "APPROVED", "SENT", "PENDING_APPROVAL"].includes(quotation.status),
  );

  return preferred ?? project.quotations[0] ?? null;
};

const getLaborType = (metadataJson: Prisma.JsonValue | null): string | null => {
...
const classifyServiceCategory = (value: {
  ...
  if (
    haystack.includes("mano de obra") ||
...
const classifyQuotationMaterialCategory = (
  line: QuotationMaterialLine,
...
const classifyPurchaseItemCategory = (
  item: PurchaseOrderProfitabilityEntity["items"][number],
): CostoProyectoCategoria => {
...
const buildMaterialCostCandidateMap = (
  project: ProjectProfitabilityEntity,
  purchaseOrders: PurchaseOrderProfitabilityEntity[],
) => {
  const candidates = new Map<string, MaterialCostCandidate[]>();

  const appendCandidate = (materialId: string, candidate: MaterialCostCandidate) => {
...
const resolveMaterialUnitCost = (
  material: MaterialCostRecord,
  unit: string,
  candidates: Map<string, MaterialCostCandidate[]>,
): number => {
...
const calculateProjectAreaM2 = (project: ProjectProfitabilityEntity): number | null => {
...
const buildAlertRecord = (input: {
  ...
  tipo: (typeof ALERTA_RENTABILIDAD_TIPOS)[number];
}): AlertaRentabilidadRecord => ({
...
const mapProjectState = (input: {
  project: ProjectProfitabilityEntity;
  hasOpenInstallation: boolean;
  hasOpenPurchases: boolean;
  hasOpenProduction: boolean;
...
const sortProfitabilityItems = (
  rows: RentabilidadProyectoListItem[],
...
const buildGroupingReport = (
  rows: RentabilidadProyectoListItem[],
  selector: (row: RentabilidadProyectoListItem) => {
    clave: string;
    nombre: string;
  },
): RentabilidadAgrupadaRecord[] => {
  const grouped = new Map<string, RentabilidadAgrupadaRecord>();
  ...
  rows.forEach((row) => {
    ...
    if (existing) {
      existing.margenPromedio = roundTo(
        (existing.margenPromedio * existing.proyectos +
          row.rentabilidad.margenBruto) /
          (existing.proyectos + 1),
        4,
      );
      existing.proyectos += 1;
      existing.utilidadTotal = roundTo(
        existing.utilidadTotal + row.rentabilidad.utilidadBruta,
        4,
      );
      existing.ventaTotal = roundTo(
        existing.ventaTotal + row.rentabilidad.ingresoReal,
        4,
      );
      return;
    }
  ...
  return Array.from(grouped.values()).sort(
    (left, right) => right.utilidadTotal - left.utilidadTotal,
  );
};

const buildReceiptImpact = (
  receipt: PurchaseOrderProfitabilityEntity["receipts"][number],
  order: PurchaseOrderProfitabilityEntity,
  candidateMap: Map<string, MaterialCostCandidate[]>,
): number => {
  return roundTo(
...
const buildProfitabilityForProject = (
  project: ProjectProfitabilityEntity,
  purchaseOrders: PurchaseOrderProfitabilityEntity[],
  minimumMarginPercent: number,
  allRowsForReports?: RentabilidadProyectoListItem[],
): RentabilidadProyectoDetailRecord => {
  ...
  const costos: CostoProyectoRecord[] = [];
  const eventos: EventoRentabilidadRecord[] = [];
  const metodologia = [
  ...
  const budgetLines =
    budgetQuotation?.items.flatMap((item) =>
      item.materials.map((line) => ({
        ...
        origen: "COTIZACION" as CostoProyectoOrigen,
  ...
  const poMaterialIds = new Set<string>();
  let poLaborCost = 0;
  ...
  purchaseOrders.forEach((order) => {
    eventos.push({
      ...
      descripcion: `Orden de compra ${order.code} registrada con proveedor ${
        order.supplier.commercialName || order.supplier.legalName
      }.`,
  ...
  const actualMaterialBase = roundTo(
    costos
      .filter(
        (line) =>
          line.categoria === "MATERIALES" &&
  ...
  const productionProgress = weightedAverage(
    project.productionJobs.map((job) => ({
      value: calculateTaskProgress(job.tasks),
      weight: Math.max(job.tasks.length, 1),
    })),
  );
  const installationProgress = weightedAverage(
    project.installationOrders.map((order) => ({
  ...
  const derivedInstallationTarget = roundTo(
    budgetInstallation * installationProgress,
    4,
  );
  ...
  if (derivedLaborTarget > poLaborCost) {
    costos.push({
      ...
      descripcion:
        "Costo de mano de obra derivado del avance de produccion registrado.",
  ...
  if (derivedInstallationTarget > poInstallationCost) {
    costos.push({
      ...
      descripcion:
        "Costo de instalacion derivado del avance operativo registrado.",
      fecha: new Date().toISOString(),
  ...
  const actualMaterial = roundTo(
    costos
      .filter((line) => line.categoria === "MATERIALES")
      .reduce((sum, line) => sum + line.monto, 0),
    4,
  );
  const actualLabor = roundTo(
    costos
      .filter((line) => line.categoria === "MANO_DE_OBRA")
      .reduce((sum, line) => sum + line.monto, 0),
    4,
  );
  const actualInstallation = roundTo(
    costos
      .filter((line) => line.categoria === "INSTALACION")
      .reduce((sum, line) => sum + line.monto, 0),
    4,
  ...
  const totalBudgetCostFromLines = roundTo(
    budgetLines.reduce((sum, line) => sum + line.monto, 0),
    4,
  );
  const totalBudgetCost = roundTo(
    Math.max(
      totalBudgetCostFromLines,
      budgetQuotation ? Number(budgetQuotation.subtotalCost) : 0,
    ),
    4,
  );
  const totalActualCost = roundTo(
    costos.reduce((sum, line) => sum + line.monto, 0),
    4,
  );

  const ingresoPresupuestado = roundTo(
    budgetQuotation ? Number(budgetQuotation.totalSale) : 0,
    4,
  );
  const ingresoReal = roundTo(
    actualQuotation ? Number(actualQuotation.totalSale) : ingresoPresupuestado,
    4,
  );
  ...
  const margenBruto = roundTo(
    safeDivide(utilidadBruta, ingresoReal) === null
      ? 0
      : (safeDivide(utilidadBruta, ingresoReal) ?? 0) * 100,
    4,
  );

  const budgetWaste = weightedAverage(
    (budgetQuotation?.items ?? []).flatMap((item) =>
      item.materials.map((line) => ({
  ...
  const actualWasteFromReports = weightedAverage(
    wasteReportRows.map((report) => ({
  ...
  const actualWasteFromConsumptions = (() => {
    const actualConsumptions = project.productionJobs.flatMap((job) =>
      job.materialConsumptions.filter((consumption) =>
        ["ACTUAL", "WASTE", "SCRAP"].includes(consumption.consumptionType),
      ),
    );
    const wasteQuantity = actualConsumptions
      .filter((consumption) =>
        ["WASTE", "SCRAP"].includes(consumption.consumptionType),
      )
      .reduce((sum, row) => sum + Number(row.quantity), 0);
    const totalQuantity = actualConsumptions.reduce(
  ...
  const desperdicioReal =
    actualWasteFromReports > 0 ? actualWasteFromReports : actualWasteFromConsumptions;

  const recuperacionPorRemanentes = roundTo(
    [
      ...project.cuttingOptimizationRuns.flatMap((run) =>
        run.cuttingPlans.flatMap((plan) =>
          plan.sheets.flatMap((sheet) =>
            sheet.remnantOutputs
      ...
      ...project.profileOptimizationRuns.flatMap((run) =>
        run.cuttingPlans.flatMap((plan) =>
          plan.bars.flatMap((bar) =>
            bar.remnantOutputs
              .filter((output) => output.remnantPieceId)
              .map((output) => {
                const unitCost = resolveMaterialUnitCost(
  ...
  const netMargin = roundTo(
    safeDivide(ingresoReal - totalActualCost, ingresoReal) === null
      ? 0
      : (safeDivide(ingresoReal - totalActualCost, ingresoReal) ?? 0) * 100,
    4,
  );

  const indicators: IndicadoresRentabilidadRecord = {
    margenNeto: netMargin,
    rentabilidadPorMetroCuadrado:
      calculateProjectAreaM2(project) && calculateProjectAreaM2(project)! > 0
        ? roundTo(
            utilidadBruta / (calculateProjectAreaM2(project) ?? 1),
            4,
          )
        : null,
  ...
  const alerts: AlertaRentabilidadRecord[] = [];

  if (margenBruto < minimumMarginPercent) {
    alerts.push(
      buildAlertRecord({
        descripcion: `El margen bruto (${margenBruto.toFixed(
          2,
        )}%) esta por debajo del minimo permitido (${minimumMarginPercent.toFixed(2)}%).`,
  ...
  if (desperdicioReal > budgetWaste && budgetWaste > 0) {
    alerts.push(
      buildAlertRecord({
        descripcion:
          "El desperdicio real excede el objetivo presupuestado del proyecto.",
  ...
  const rentabilidad: RentabilidadProyectoRecord = {
    ...
    estado: mapProjectState({
      hasOpenInstallation: project.installationOrders.some((order) =>
        !["COMPLETED", "CANCELLED"].includes(order.status),
      ),
      hasOpenProduction: project.productionJobs.some((job) =>
        !["COMPLETED", "CANCELLED"].includes(job.status),
      ),
      hasOpenPurchases: purchaseOrders.some((order) =>
  ...
  const listItem: RentabilidadProyectoListItem = {
    alertas: alerts,
...
const fetchProjectsWithContext = async (
  ...
  const quotationIdToProjectId = new Map<string, string>();
  const planIdToProjectId = new Map<string, string>();
  ...
  const projectPurchaseOrderMap = new Map<string, PurchaseOrderProfitabilityEntity[]>();
...
const getMinimumMarginPercent = async (): Promise<number> => {
...
export const projectProfitabilityService = {
  async listProjectProfitability(query: ListProjectProfitabilityQuery) {
    ...
    const detailRows = projects.map(({ project, purchaseOrders }) =>
      buildProfitabilityForProject(project, purchaseOrders, minimumMarginPercent),
    );
  ...
  async getDashboard(
    query: ListProjectProfitabilityDashboardQuery,
    ...
    const byUtility = [...rows].sort(
      (left, right) =>
        right.rentabilidad.utilidadBruta - left.rentabilidad.utilidadBruta,
    );

    return {
      desperdicioPromedio: roundTo(
        safeDivide(
          rows.reduce(
      ...
      margenPromedio: roundTo(
        safeDivide(
          rows.reduce((sum, row) => sum + row.rentabilidad.margenBruto, 0),
          Math.max(rows.length, 1),
        ) ?? 0,
        4,
      ),
      ...
      proyectosRentables: rows.filter(
      ...
      utilidadTotal: roundTo(
        rows.reduce((sum, row) => sum + row.rentabilidad.utilidadBruta, 0),
        4,
      ),
    };
  },
  ...
  async getProjectProfitabilityByProjectId(
    ...
    const allRows = [
      buildProfitabilityForProject(
        entry.project,
        entry.purchaseOrders,
        minimumMarginPercent,
      ),
    ];

    return buildProfitabilityForProject(
      entry.project,
      entry.purchaseOrders,
      minimumMarginPercent,
      allRows.map((row) => ({
