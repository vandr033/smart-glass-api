...
import {
  ...
  SupplierScoringScope,
} from "../../../generated/prisma/client.js";

import { AppError } from "../../utils/app-error.js";
import { prisma } from "../../utils/prisma.js";
import type {
  CreateSupplierScoringConfigInput,
  ListSupplierScoringConfigsQuery,
  RankedSupplierScoreRecord,
  SimulateSupplierScoringInput,
  SupplierScoringConfigRecord,
  SupplierScoringConfigWeightRecord,
  SupplierScoringCriterionRecord,
  SupplierScoringManualScores,
  UpdateSupplierScoringConfigInput,
...
} satisfies Prisma.SupplierScoringConfigInclude;

type SupplierScoringCriterionEntity = Prisma.SupplierScoringCriterionGetPayload<{
  select: {
    createdAt: true;
    description: true;
    id: true;
...
type SupplierScoringConfigEntity = Prisma.SupplierScoringConfigGetPayload<{
  include: typeof supplierScoringConfigInclude;
}>;

type SupplierForScoring = {
  creditAvailable: boolean;
  creditLimit: PrismaNamespace.Decimal | null;
  defaultLeadTimeDays: number | null;
  id: string;
  legalName: string;
  preferenceScore: PrismaNamespace.Decimal | null;
  reliabilityScore: PrismaNamespace.Decimal | null;
};
...
const sortWeights = (
  left: NonNullable<SupplierScoringConfigEntity["weights"]>[number],
  right: NonNullable<SupplierScoringConfigEntity["weights"]>[number],
) => {
...
const mapCriterion = (
  record: SupplierScoringCriterionEntity,
): SupplierScoringCriterionRecord => {
  return {
    ...
    key: record.key as SupplierScoringCriterionKey,
    label: record.label,
...
const mapConfigWeight = (
  record: NonNullable<SupplierScoringConfigEntity["weights"]>[number],
): SupplierScoringConfigWeightRecord => {
  return {
    criterionId: record.criterionId,
    criterionKey: record.criterion.key as SupplierScoringCriterionKey,
...
const mapConfig = (
  record: SupplierScoringConfigEntity,
): SupplierScoringConfigRecord => {
...
const buildConfigWhereClause = (
  query: ListSupplierScoringConfigsQuery,
): Prisma.SupplierScoringConfigWhereInput => {
  return {
...
const findConfigByIdOrThrow = async (
  id: string,
  db: Prisma.TransactionClient | typeof prisma = prisma,
): Promise<SupplierScoringConfigEntity> => {
...
const ensureCriteriaExist = async (
  criterionIds: string[],
  db: Prisma.TransactionClient | typeof prisma,
): Promise<void> => {
...
const ensureCategoryConfigConstraints = async (
  configId: string | null,
  input: CreateSupplierScoringConfigInput | UpdateSupplierScoringConfigInput,
  ...
  if (existingConfig) {
    throw new AppError(
      "Only one active CATEGORY scoring config may exist per category.",
      409,
    );
  }
};

const normalizeManualScore = (value: number | null | undefined): number | null => {
...
const getCriterionScore = (
  ...
  supplier: SupplierForScoring,
  manualScores: SupplierScoringManualScores[string] | undefined,
  stats: {
    creditLimits: number[];
    leadTimes: number[];
  },
  ...
  switch (criterionKey) {
    ...
    case "delivery_time":
      return {
        normalizedScore: roundToTwoDecimals(
          normalizeLowerIsBetter(supplier.defaultLeadTimeDays, stats.leadTimes),
        ),
        source: "derived",
      };
    case "reliability":
      return {
        normalizedScore: roundToTwoDecimals(
          clampScore(decimalToNumber(supplier.reliabilityScore) ?? 0),
        ),
        source: "direct",
      };
    case "credit": {
      const creditLimit = decimalToNumber(supplier.creditLimit);
      const creditLimitComponent = normalizeHigherIsBetter(
        creditLimit,
        stats.creditLimits,
      );
    ...
    case "preference":
      return {
        normalizedScore: roundToTwoDecimals(
          clampScore(decimalToNumber(supplier.preferenceScore) ?? 0),
        ),
        source: "direct",
      };
  }
...
const buildRankedSupplierScores = (
  config: SupplierScoringConfigRecord,
  suppliers: SupplierForScoring[],
  manualScores: SupplierScoringManualScores,
): RankedSupplierScoreRecord[] => {
    ...
    .map((supplier) => {
      const breakdown = config.weights.map((weight) => {
        const score = getCriterionScore(
          weight.criterionKey,
          supplier,
          manualScores[supplier.id],
          {
            creditLimits,
            leadTimes,
          },
        );
        const contribution = roundToTwoDecimals(
          (score.normalizedScore * weight.weight) / 100,
        );
...
export const supplierScoringService = {
  async listCriteria(): Promise<SupplierScoringCriterionRecord[]> {
  ...
  async getConfigById(id: string): Promise<SupplierScoringConfigRecord> {
    const config = await findConfigByIdOrThrow(id);
    return mapConfig(config);
  },

  async createConfig(
    input: CreateSupplierScoringConfigInput,
  ): Promise<SupplierScoringConfigRecord> {
    try {
      return await prisma.$transaction(async (db) => {
        ...
        await ensureCriteriaExist(
          input.weights.map((weight) => weight.criterionId),
          db,
        );
  ...
  async updateConfig(
    id: string,
    input: UpdateSupplierScoringConfigInput,
  ): Promise<{
    current: SupplierScoringConfigRecord;
    previous: SupplierScoringConfigRecord;
  }> {
  ...
  async deleteConfig(id: string): Promise<SupplierScoringConfigRecord> {
  ...
  async scoreSuppliers(input: {
    categoryId?: string | null;
    configOverride?: SupplierScoringConfigRecord | null;
    manualScores?: SupplierScoringManualScores;
    suppliers: SupplierForScoring[];
  }) {
    ...
    return {
      rankedSuppliers: buildRankedSupplierScores(
        selectedConfig,
        input.suppliers,
        input.manualScores ?? {},
      ),
  ...
  async simulateScores(input: SimulateSupplierScoringInput) {
    if (input.categoryId) {
