...
import {
  INVENTORY_MOVEMENT_ENTITY_TYPE,
...
import type { SupplierScoringConfigRecord } from "../suppliers/suppliers.types.js";
import type {
...
import {
  ...
  SUPPLIER_COMPARISON_ENTITY_TYPE,
} from "./purchasing.constants.js";

type DbClient = typeof prisma | Prisma.TransactionClient;

type PurchasingSettings = {
  defaultSupplierScoringConfigId: string | null;
  preferSingleSupplier: boolean;
  requireApprovalBeforePo: boolean;
};

type MaterialForPurchase = {
  code: string;
  id: string;
  materialType: "LINEAR" | "PACKAGE" | "SERVICE" | "SHEET" | "UNIT";
  name: string;
  purchaseUnit: MaterialUnit;
  standardHeightMm: PrismaNamespace.Decimal | null;
  standardLengthMm: PrismaNamespace.Decimal | null;
  standardWidthMm: PrismaNamespace.Decimal | null;
  stockUnit: MaterialUnit;
  thicknessMm: PrismaNamespace.Decimal | null;
  unitConversionJson: Prisma.JsonValue | null;
};
...
type ComparisonCandidate = {
  ...
  supplierCommercialName: string | null;
  supplierId: string;
  supplierLegalName: string;
  supplierUnit: string | null;
};

type RequestItemPricing = {
...
const PURCHASING_DEFAULT_SCORING_CONFIG =
  "purchasing.default_supplier_scoring_config_id";
const PURCHASING_REQUIRE_APPROVAL_BEFORE_PO =
  "purchasing.require_approval_before_po";
...
const hideCostValue = (value: number, canViewCost: boolean): number | null => {
...
} satisfies Prisma.SupplierComparisonInclude;
...
type PurchaseRequestListEntity = Prisma.PurchaseRequestGetPayload<{
  include: typeof purchaseRequestListInclude;
}>;

type PurchaseRequestDetailEntity = Prisma.PurchaseRequestGetPayload<{
  include: typeof purchaseRequestDetailInclude;
}>;

type SupplierComparisonListEntity = Prisma.SupplierComparisonGetPayload<{
  include: typeof supplierComparisonListInclude;
}>;

type SupplierComparisonDetailEntity = Prisma.SupplierComparisonGetPayload<{
  include: typeof supplierComparisonDetailInclude;
}>;

type PurchaseOrderListEntity = Prisma.PurchaseOrderGetPayload<{
  include: typeof purchaseOrderListInclude;
}>;

type PurchaseOrderDetailEntity = Prisma.PurchaseOrderGetPayload<{
  include: typeof purchaseOrderDetailInclude;
}>;

type PurchaseReceiptListEntity = Prisma.PurchaseReceiptGetPayload<{
  include: typeof purchaseReceiptListInclude;
}>;

type PurchaseReceiptDetailEntity = Prisma.PurchaseReceiptGetPayload<{
  include: typeof purchaseReceiptDetailInclude;
}>;

const mapUserSummary = (
  user: PurchaseRequestDetailEntity["requestedByUser"] | null,
) => {
  if (!user) {
    return null;
  }
...
const mapSupplierSummary = (
  supplier:
    | PurchaseRequestDetailEntity["items"][number]["preferredSupplier"]
    | PurchaseOrderDetailEntity["supplier"]
    | PurchaseReceiptDetailEntity["purchaseOrder"]["supplier"]
    | null,
) => {
  if (!supplier) {
    return null;
...
const mapMaterialSummary = (
  material:
    | PurchaseRequestDetailEntity["items"][number]["material"]
    | PurchaseOrderDetailEntity["items"][number]["material"]
    | PurchaseReceiptDetailEntity["items"][number]["material"],
) => {
  return {
    code: material.code,
    id: material.id,
...
const mapWarehouseSummary = (
  warehouse:
    | PurchaseOrderDetailEntity["receipts"][number]["warehouse"]
    | PurchaseReceiptDetailEntity["warehouse"]
    | null,
) => {
  if (!warehouse) {
    return null;
...
const computePurchaseRequestTotals = (
  items: Array<{
    estimatedTotalCost: PrismaNamespace.Decimal | null;
    selectedSupplierId?: string | null;
  }>,
  canViewCost: boolean,
) => {
  const estimatedSubtotal = roundTo(
    items.reduce((sum, item) => sum + Number(item.estimatedTotalCost ?? 0), 0),
  );
...
const resolveSourceReferenceLabel = async (
  db: DbClient,
  sourceType: PurchaseRequestStatus | string,
...
const mapPurchaseRequestItem = (
  item: PurchaseRequestDetailEntity["items"][number],
  canViewCost: boolean,
) => {
  return {
    createdAt: item.createdAt.toISOString(),
    description: item.description,
    estimatedTotalCost: hideCostValue(
      roundTo(Number(item.estimatedTotalCost ?? 0)),
      canViewCost,
    ),
...
const mapPurchaseRequestListItem = async (
  record: Pick<
    PurchaseRequestListEntity,
    | "approvedAt"
    | "approvedByUser"
    | "code"
    | "createdAt"
  ...
  return {
    ...
    sourceReferenceLabel: await resolveSourceReferenceLabel(
      db,
      record.sourceType,
      record.sourceId,
    ),
...
const mapPurchaseRequestRecord = async (
  record: PurchaseRequestDetailEntity,
  canViewCost: boolean,
  db: DbClient,
...
const mapSupplierComparisonOption = (
  option: SupplierComparisonDetailEntity["options"][number],
  canViewCost: boolean,
) => {
  return {
...
const mapSupplierComparisonListItem = (
  record: SupplierComparisonListEntity,
) => {
  return {
    approvedAt: toIsoString(record.approvedAt),
    approvedByUser: mapUserSummary(record.approvedByUser),
...
const hideComparisonSummaryCosts = (
  value: unknown,
  canViewCost: boolean,
): unknown => {
...
const mapSupplierComparisonRecord = async (
  record: SupplierComparisonDetailEntity,
  canViewCost: boolean,
  db: DbClient,
  ...
  return {
    ...
    options: record.options.map((option) =>
      mapSupplierComparisonOption(option, canViewCost),
    ),
    purchaseRequestDetail: await mapPurchaseRequestRecord(
      record.purchaseRequest,
      canViewCost,
      db,
    ),
    resultJson: hideComparisonSummaryCosts(record.resultJson, canViewCost),
    selectedCombinationJson: hideComparisonSummaryCosts(
      record.selectedCombinationJson,
      canViewCost,
    ),
  };
};

const mapPurchaseOrderItem = (
  item: PurchaseOrderDetailEntity["items"][number],
  canViewCost: boolean,
) => {
  return {
    createdAt: item.createdAt.toISOString(),
...
const mapPurchaseOrderListItem = (
  record: PurchaseOrderListEntity,
  canViewCost: boolean,
) => {
  return {
    code: record.code,
...
const mapPurchaseOrderRecord = (
  record: PurchaseOrderDetailEntity,
  canViewCost: boolean,
): PurchaseOrderRecord => {
  return {
...
const mapPurchaseReceiptItem = (
  item: PurchaseReceiptDetailEntity["items"][number],
) => {
  return {
...
const mapPurchaseReceiptListItem = (
  record: PurchaseReceiptListEntity,
) => {
  return {
    code: record.code,
    createdAt: record.createdAt.toISOString(),
...
const mapPurchaseReceiptRecord = (
  record: PurchaseReceiptDetailEntity,
): PurchaseReceiptRecord => {
  return {
...
const generatePurchaseReceiptCode = async (
  db: DbClient,
  value = new Date(),
): Promise<string> => {
  const year = value.getUTCFullYear();
...
const getPurchasingSettings = async (db: DbClient): Promise<PurchasingSettings> => {
  ...
  return {
    defaultSupplierScoringConfigId: readNullableString(
      PURCHASING_DEFAULT_SCORING_CONFIG,
    ),
...
const findPurchaseRequestOrThrow = async (
  db: DbClient,
  purchaseRequestId: string,
): Promise<PurchaseRequestDetailEntity> => {
...
const findSupplierComparisonOrThrow = async (
  db: DbClient,
  comparisonId: string,
): Promise<SupplierComparisonDetailEntity> => {
...
const findPurchaseOrderOrThrow = async (
  db: DbClient,
...
const findPurchaseReceiptOrThrow = async (
  db: DbClient,
  purchaseReceiptId: string,
): Promise<PurchaseReceiptDetailEntity> => {
...
const ensurePurchaseRequestIsEditable = (status: string) => {
  if (["CONVERTED_TO_PO", "CANCELLED"].includes(status)) {
    throw new AppError("This purchase request can no longer be edited.", 400);
  }
};

const ensurePurchaseRequestCanChangeLines = (status: string) => {
  if (!["DRAFT", "PENDING_APPROVAL", "REJECTED"].includes(status)) {
    throw new AppError(
      "Only draft, pending approval, or rejected purchase requests can change items.",
      400,
    );
  }
};

const ensureAuthenticatedUser = (userId: string | null): string => {
  if (!userId) {
...
const getMaterialForPurchaseOrThrow = async (
  db: DbClient,
  materialId: string,
): Promise<MaterialForPurchase> => {
...
const ensureSupplierExists = async (db: DbClient, supplierId: string) => {
...
const ensureWarehouseExists = async (db: DbClient, warehouseId: string) => {
...
const buildPurchaseRequestWhereClause = (
...
const buildSupplierComparisonWhereClause = (
  query: ListSupplierComparisonsQuery,
): Prisma.SupplierComparisonWhereInput => {
  return {
    ...(query.status
      ? {
          status: query.status,
...
const buildPurchaseOrderWhereClause = (
...
const buildPurchaseReceiptWhereClause = (
...
const sanitizePurchaseRequestItems = (
  ...
  return items.map((item) => {
    ...
    const estimatedTotalCost =
      estimatedUnitCost === null ? null : roundTo(item.quantity * estimatedUnitCost);
...
const upsertPurchaseRequestItems = async (
  db: DbClient,
  purchaseRequestId: string,
  items: CreatePurchaseRequestInput["items"],
) => {
...
const calculateOrderTotalsFromInputs = (
  items: Array<{
    quantity: number;
    unitPrice: number;
  }>,
...
const resolveComparisonConfig = async (
  db: DbClient,
  scoringConfigId: string | null | undefined,
  ...
  if (settings.defaultSupplierScoringConfigId) {
    return supplierScoringService.getConfigById(
      settings.defaultSupplierScoringConfigId,
    );
  }
...
const normalizeHigherIsBetter = (value: number | null, values: number[]): number => {
...
const normalizeLowerIsBetter = (value: number | null, values: number[]): number => {
  if (value === null || values.length === 0) {
    return 0;
  }
...
const calculateItemPricing = (
  ...
  const targetUnit =
    candidate.normalizedUnit ?? candidate.supplierUnit ?? material.purchaseUnit;
  const targetMaterialUnit = targetUnit as MaterialUnit;
        ...
        : roundTo(
            convertMaterialUnit({
              ...
              material: {
                ...
                materialType: material.materialType,
                name: material.name,
                unitConversionJson: material.unitConversionJson as JsonLike | null,
              },
...
const createInventoryStockFromReceipt = async (
  db: DbClient,
  input: {
  ...
  if (input.material.materialType === "SHEET") {
    ...
    if (input.heightMm === null && input.lengthMm === null) {
      throw new AppError(
        "Sheet materials require a height or length in millimeters.",
        400,
      );
    }
...
const convertReceiptQuantityToStockUnit = (
  material: MaterialForPurchase,
  quantity: number,
  unit: MaterialUnit,
) => {
...
const resolveReceiptDimensions = (
  material: MaterialForPurchase,
  line: ReceivePurchaseOrderInput["items"][number],
) => {
  return {
    heightMm:
      line.heightMm ??
...
export const purchasingService = {
  async getDashboard(
  ...
  ): Promise<PurchasingDashboardRecord> {
    const now = new Date();
    const [
    ...
    return {
      delayedExpectedDeliveries: delayedOrders.map((order) =>
        mapPurchaseOrderListItem(order, options.canViewCost),
      ),
      openPurchaseOrders,
      partialPurchaseOrders: partialOrders.map((order) =>
        mapPurchaseOrderListItem(order, options.canViewCost),
      ),
      pendingApprovals,
      pendingPurchaseRequests,
  ...
  async listPurchaseRequests(
    ...
    return {
      data: await Promise.all(
        records.map((record) =>
          mapPurchaseRequestListItem(record, options.canViewCost, prisma),
        ),
      ),
      pagination: {
        page: query.page,
  ...
  async createPurchaseRequestFromQuotation(
    ...
    quotation.items.forEach((item) => {
      item.materials.forEach((material) => {
        ...
        if (existing) {
          existing.quantity = roundTo(
            existing.quantity + Number(material.requiredQuantity),
          );
    ...
    if (grouped.size === 0) {
      throw new AppError(
        "The quotation does not contain material requirements to purchase.",
        400,
      );
    }
    ...
    return this.createPurchaseRequest(
      {
        items: Array.from(grouped.values()).map((entry) => ({
          ...
          preferredSupplierId:
            entry.preferredSupplierIds.size === 1
  ...
  async createPurchaseRequestFromCuttingPlan(
    ...
    if (cuttingPlan.sheets.length === 0) {
      throw new AppError(
        "This cutting plan does not require additional purchased sheets.",
        400,
      );
    }

    const totalAreaM2 = roundTo(
      cuttingPlan.sheets.reduce((sum, sheet) => sum + Number(sheet.sheetAreaM2), 0),
    );
  ...
  async createPurchaseRequestFromProfileCuttingPlan(
    ...
    if (profileCuttingPlan.bars.length === 0) {
      throw new AppError(
        "This profile cutting plan does not require additional purchased bars.",
        400,
      );
    }

    const totalLengthMm = roundTo(
      profileCuttingPlan.bars.reduce(
        (sum, bar) => sum + Number(bar.originalLengthMm),
        0,
      ),
      2,
    );
    const purchaseQuantity = roundTo(
      convertMaterialUnit({
        fromUnit: "MM",
        material: {
          ...
          unitConversionJson: profileCuttingPlan.material.unitConversionJson as JsonLike,
        },
  ...
  async createPurchaseRequestFromInventoryShortage(
    ...
    const stockByMaterialId = new Map(
      stockRows.map((row) => [row.materialId, Number(row._sum.quantity ?? 0)]),
    );
    ...
    return this.createPurchaseRequest(
      {
        items: materials.map((material) => ({
          ...
          metadataJson: {
            ...
            assumption:
              "Shortage requests default to a quantity of 1 when no reorder target is configured.",
          },
          preferredSupplierId: undefined,
          quantity: 1,
          requiredDate: null,
  ...
  async deletePurchaseRequest(purchaseRequestId: string, userId: string | null) {
    return prisma.$transaction(async (db) => {
      const existing = await findPurchaseRequestOrThrow(db, purchaseRequestId);

      if (!["DRAFT", "REJECTED", "CANCELLED"].includes(existing.status)) {
        throw new AppError(
          "Only draft, rejected, or cancelled purchase requests can be deleted.",
          400,
        );
      }
  ...
  async approvePurchaseRequest(
    ...
    return prisma.$transaction(async (db) => {
      ...
      if (existing.items.length === 0) {
        throw new AppError(
          "Purchase request must contain at least one item before approval.",
          400,
        );
      }
  ...
  async calculatePurchaseRequestTotals(
    purchaseRequestId: string,
    options: {
      canViewCost: boolean;
    },
  ...
  async getSupplierComparisonById(
    comparisonId: string,
    options: {
      canViewCost: boolean;
    },
  ...
  async compareSuppliersForPurchaseRequest(
    ...
    return prisma.$transaction(async (db) => {
      ...
      if (purchaseRequest.items.length === 0) {
        throw new AppError(
          "Purchase request must contain at least one item before supplier comparison.",
          400,
        );
      }

      if (
        settings.requireApprovalBeforePo &&
        purchaseRequest.status !== "APPROVED"
      ) {
        throw new AppError(
          "This purchase request must be approved before suppliers can be compared.",
          400,
        );
      }
      ...
      const materialIds = Array.from(
        new Set(purchaseRequest.items.map((item) => item.materialId)),
      );
      ...
      const supplierById = new Map(
        directSuppliers.map((supplier) => [supplier.id, supplier]),
      );
      equivalenceRows.forEach((row) => {
        supplierById.set(row.supplier.id, row.supplier);
      });

      const latestPriceByKey = new Map<string, PriceCandidate>();
      ...
      const candidatesByMaterialId = new Map<string, ComparisonCandidate[]>();
      materialIds.forEach((materialId) => {
        ...
        const candidates: ComparisonCandidate[] = Array.from(supplierIds)
          .map((supplierId) => {
            ...
            const priceCandidate =
              latestPriceByKey.get(`${materialId}:${supplierId}`) ?? null;
            const hasEquivalence = equivalenceRows.some(
              (row) => row.materialId === materialId && row.supplierId === supplierId,
            );
      ...
      const bestPriceSelections: Record<string, string | null> = {};
      const bestScoreSelections: Record<string, string | null> = {};
      const optionIdsByItemAndSupplier = new Map<string, string>();
      const optionTotalsBySupplier = new Map<string, number>();
      const optionScoresBySupplier = new Map<string, number[]>();
      const selectionByItem = new Map<string, string | null>();
      ...
      for (const requestItem of purchaseRequest.items) {
        ...
        const baseCandidates =
          candidatesByMaterialId.get(requestItem.materialId) ?? [];

        if (baseCandidates.length === 0) {
          warnings.push(
            `No supplier candidates were found for material ${requestItem.material.name}.`,
          );
          continue;
        }
        ...
        const scoredOptions = pricingOptions.map((option) => {
          const breakdown = scoringConfig.weights.map((weight) => {
            ...
            switch (weight.criterionKey) {
              case "price":
                normalizedScore = roundTo(
                  normalizeLowerIsBetter(option.totalPrice, priceValues),
                  2,
                );
                break;
              case "delivery_time":
                normalizedScore = roundTo(
                  normalizeLowerIsBetter(option.deliveryDays, deliveryValues),
                  2,
                );
                break;
              case "reliability":
                normalizedScore = roundTo(
                  Math.max(0, Math.min(100, option.reliabilityScore ?? 0)),
                  2,
                );
                break;
              case "credit": {
                const creditLimitScore = normalizeHigherIsBetter(
                  option.creditLimit,
                  creditValues,
                );
                normalizedScore = roundTo(
                  Math.max(
                    ...
                    Math.min(
                      100,
                      (option.availableCredit ? 60 : 0) + creditLimitScore * 0.4,
                    ),
                  ),
                  2,
                );
              ...
              case "preference":
                normalizedScore = roundTo(
                  Math.max(
                    ...
                    Math.min(
                      100,
                      (option.preferenceScore ?? 0) + (option.preferred ? 10 : 0),
                    ),
                  ),
                  2,
                );
        ...
        for (const option of scoredOptions) {
          ...
          optionIdsByItemAndSupplier.set(
            `${requestItem.id}:${option.supplierId}`,
            optionRecord.id,
          );
          optionTotalsBySupplier.set(
            option.supplierId,
            roundTo(
              (optionTotalsBySupplier.get(option.supplierId) ?? 0) +
                (option.totalPrice ?? 0),
            ),
          );
      ...
      const singleSupplierCandidates = candidateSuppliers
        .map((supplierId) => {
          const coversAllItems = purchaseRequest.items.every((item) =>
            optionIdsByItemAndSupplier.has(`${item.id}:${supplierId}`),
          );
        ...
        .filter(
          (
            candidate,
          ): candidate is { averageScore: number; supplierId: string; totalPrice: number } =>
            Boolean(candidate),
        )
        .sort((left, right) => {
      ...
      const selectedStrategy =
        settings.preferSingleSupplier && singleSupplierCandidates.length > 0
          ? "single_supplier"
      ...
      const selectedCombinationJson = {
        ...
        supplierIds: Array.from(
          new Set(selectedOptions.map((option) => option.supplierId)),
        ),
      };
  ...
  async createPurchaseOrdersFromComparison(
    ...
    return prisma.$transaction(async (db) => {
      ...
      if (comparison.status !== "APPROVED") {
        throw new AppError(
          "Supplier comparison must be approved before purchase orders can be created.",
          400,
        );
      }
      ...
      if (
        ...
        throw new AppError(
          "The purchase request must be approved before purchase orders can be created.",
          400,
        );
      }
      ...
      if (comparison.purchaseRequest.status === "CONVERTED_TO_PO") {
        throw new AppError(
          "Purchase orders have already been created for this purchase request.",
          400,
        );
      }

      const selectedOptions = comparison.options.filter((option) => option.isSelected);
      const actionableItems = comparison.purchaseRequest.items.filter(
      ...
      if (
        actionableItems.some(
          (item) =>
            !selectedOptions.some((option) => option.purchaseRequestItemId === item.id),
        )
      ) {
        throw new AppError(
          "Cannot create purchase orders until every purchase request item has a selected supplier.",
          400,
        );
      }
      ...
      const groupedBySupplier = new Map<
        string,
        Array<SupplierComparisonDetailEntity["options"][number]>
      >();
      ...
      const createdOrders: PurchaseOrderRecord[] = [];
      ...
      for (const [supplierId, supplierOptions] of groupedBySupplier.entries()) {
        if (supplierOptions.some((option) => option.unitPrice === null || option.totalPrice === null)) {
          throw new AppError(
            "Selected supplier options must include pricing before purchase orders can be created.",
            400,
          );
        }

        const code = await generatePurchaseOrderCode(db);
        const expectedDeliveryDays = Math.max(
          ...supplierOptions.map((option) => option.deliveryDays ?? 0),
        );
        ...
        const expectedDeliveryDate =
          expectedDeliveryDays > 0
            ...
            : null;
        const totals = calculateOrderTotalsFromInputs(
          supplierOptions.map((option) => ({
            quantity: Number(
              comparison.purchaseRequest.items.find((item) => item.id === option.purchaseRequestItemId)
                ?.quantity ?? 0,
            ),
          ...
          data: {
            ...
            items: {
              create: supplierOptions.map((option) => {
                const requestItem = comparison.purchaseRequest.items.find(
                  (item) => item.id === option.purchaseRequestItemId,
                );
  ...
  async listPurchaseOrders(
    ...
    return {
      data: records.map((record) =>
        mapPurchaseOrderListItem(record, options.canViewCost),
      ),
      pagination: {
        page: query.page,
        perPage: query.perPage,
  ...
  async createPurchaseOrderManually(
    ...
    return prisma.$transaction(async (db) => {
      ...
      if (input.purchaseRequestId) {
        ...
        if (settings.requireApprovalBeforePo && request.status !== "APPROVED") {
          throw new AppError(
            "The linked purchase request must be approved before creating a purchase order.",
            400,
          );
        }
      }
  ...
  async updatePurchaseOrder(
    ...
    return prisma.$transaction(async (db) => {
      ...
      if (
        ...
        throw new AppError(
          "Received purchase orders can only update notes.",
          400,
        );
      }
      ...
      if (hasReceipts && input.items) {
        throw new AppError(
          "Purchase order items cannot be replaced after receipts have been recorded.",
          400,
        );
      }
      ...
      if (input.items) {
        const recalculated = calculateOrderTotalsFromInputs(
          input.items.map((item) => ({
            quantity: item.quantity,
            unitPrice: item.unitPrice,
          })),
  ...
  async deletePurchaseOrder(purchaseOrderId: string, userId: string | null) {
    return prisma.$transaction(async (db) => {
      const existing = await findPurchaseOrderOrThrow(db, purchaseOrderId);
    ...
    return prisma.$transaction(async (db) => {
      ...
      for (const line of input.items) {
        ...
        if (line.receivedQuantity > remainingQuantity) {
          throw new AppError(
            "Cannot receive more quantity than remains on the purchase order item.",
            400,
          );
        }
        ...
        const stockQuantity = convertReceiptQuantityToStockUnit(
          material,
        ...
        await db.purchaseOrderItem.update({
          data: {
            receivedQuantity: roundTo(
              Number(orderItem.receivedQuantity) + line.receivedQuantity,
            ),
          },
          where: {
            id: orderItem.id,
      ...
      const fullyReceived = refreshedOrder.items.every(
        (item) => Number(item.receivedQuantity) >= Number(item.quantity),
      );
      const partiallyReceived = refreshedOrder.items.some(
  ...
  async listPurchaseReceipts(query: ListPurchaseReceiptsQuery) {
  ...
  async getPurchaseReceiptById(purchaseReceiptId: string) {
