...
import type {
  AddManualMaterialItemInput,
  AddManualServiceItemInput,
...
type TemplateItemManualOverride = {
  marginPercent: number | null;
  reason: string | null;
  subtotalCost: number | null;
  subtotalSale: number | null;
  updatedAt: string;
};

type PreparedQuotationItem = {
  calculationResultJson: Prisma.InputJsonValue | typeof PrismaNamespace.JsonNull;
  description: string | null;
  inputValuesJson: Prisma.InputJsonValue | typeof PrismaNamespace.JsonNull;
  itemType:
    | "DISCOUNT"
    | "MANUAL_MATERIAL"
    | "MANUAL_SERVICE"
  ...
  materials: Array<{
    ...
    materialName: string;
    metadataJson: Prisma.InputJsonValue | typeof PrismaNamespace.JsonNull;
    ...
    ruleType:
      ...
      | "SHEET_CUT"
      | "UNIT_QUANTITY";
    source: "MANUAL" | "PRICE_LIST" | "TEMPLATE";
...
type QuotationHeaderRecord = {
  ...
  currency: string;
  deletedAt: Date | null;
  discountAmount: PrismaNamespace.Decimal;
  exchangeRate: PrismaNamespace.Decimal | null;
  id: string;
  internalNotes: string | null;
  marginAmount: PrismaNamespace.Decimal;
  marginPercent: PrismaNamespace.Decimal;
  notes: string | null;
  projectId: string | null;
  status: QuotationStatus;
  subtotalCost: PrismaNamespace.Decimal;
  subtotalSale: PrismaNamespace.Decimal;
  taxAmount: PrismaNamespace.Decimal;
  totalSale: PrismaNamespace.Decimal;
  updatedAt: Date;
...
const measurementRequestSummarySelect = {
  ...
  id: true,
  status: true,
} satisfies Prisma.MeasurementRequestSelect;
...
} satisfies Prisma.QuotationApprovalInclude;
...
} satisfies Prisma.QuotationVersionInclude;
...
type QuotationListEntity = Prisma.QuotationGetPayload<{
  include: typeof quotationListInclude;
}>;

type QuotationDetailEntity = Prisma.QuotationGetPayload<{
  include: typeof quotationDetailInclude;
}>;

type QuotationApprovalEntity = Prisma.QuotationApprovalGetPayload<{
  include: typeof quotationApprovalInclude;
}>;

type QuotationVersionEntity = Prisma.QuotationVersionGetPayload<{
  include: typeof quotationVersionListInclude;
}>;

type QuotationItemEntity = QuotationDetailEntity["items"][number];
type QuotationItemMaterialEntity = QuotationItemEntity["materials"][number];
type QuotationStatusHistoryEntity = QuotationDetailEntity["statusHistory"][number];

type MaterialSummaryRecord = Prisma.MaterialGetPayload<{
  select: {
    code: true;
...
type ProductTemplateVersionRecord = Prisma.ProductTemplateVersionGetPayload<{
  include: {
    template: {
      select: {
        code: true;
...
} as const satisfies Prisma.ProductTemplateVersionInclude;
...
const mapMeasurementRequestSummary = (
  measurementRequest:
    | {
        code: string;
        id: string;
...
const computeMarginPercent = (subtotalCost: number, subtotalSale: number): number => {
  if (subtotalSale <= 0) {
    return 0;
  }
...
const hasManualOverride = (calculationResultJson: Prisma.JsonValue | null): boolean => {
  return extractTemplateManualOverride(calculationResultJson) !== null;
};

const extractTemplateManualOverride = (
  calculationResultJson: Prisma.JsonValue | null,
): TemplateItemManualOverride | null => {
  ...
  return {
    ...
    subtotalSale:
      typeof overrideRecord.subtotalSale === "number"
        ...
        : null,
    updatedAt:
      typeof overrideRecord.updatedAt === "string"
        ? overrideRecord.updatedAt
        : new Date().toISOString(),
  };
};

const buildTemplateCalculationResultJson = (
  input: {
    manualOverride: TemplateItemManualOverride | null;
...
const applyTemplateManualOverride = (input: {
  baseSubtotalCost: number;
  baseSubtotalSale: number;
  manualOverride: TemplateItemManualOverride | null;
}) => {
  if (!input.manualOverride) {
    return {
      ...
      marginPercent: computeMarginPercent(
        input.baseSubtotalCost,
        input.baseSubtotalSale,
      ),
  ...
  const subtotalCost =
    input.manualOverride.subtotalCost ?? input.baseSubtotalCost;
  ...
  if (input.manualOverride.subtotalSale !== null) {
    ...
    subtotalSale = computeSaleFromMargin(
      subtotalCost,
      input.manualOverride.marginPercent,
    );
  }
...
const mapQuotationItemMaterial = (
  record: QuotationItemMaterialEntity,
  canViewCost: boolean,
): QuotationItemMaterialRecord => {
...
const mapQuotationItem = (
  ...
  return {
    ...
    materials: record.materials.map((material) =>
      mapQuotationItemMaterial(material, canViewCost),
    ),
    name: record.name,
...
const mapQuotationVersion = (
  record: QuotationVersionEntity,
  canViewCost: boolean,
): QuotationVersionRecord => {
  const subtotalCost = roundTo(Number(record.subtotalCost), 4);
...
const mapQuotationApproval = (
  record: QuotationApprovalEntity,
): QuotationApprovalRecord => {
  return {
...
const mapQuotationStatusHistory = (
  record: QuotationStatusHistoryEntity,
): QuotationStatusHistoryRecord => {
  return {
...
const mapQuotationListItem = (
  record: QuotationListEntity,
  canViewCost: boolean,
): QuotationListItem => {
  const subtotalCost = roundTo(Number(record.subtotalCost), 4);
...
const mapQuotationDetail = (
  record: QuotationDetailEntity,
  canViewCost: boolean,
): QuotationDetailRecord => {
  return {
    ...
    versions: record.versions.map((version) =>
      mapQuotationVersion(version, canViewCost),
    ),
  };
};
...
const assertQuotationHeaderExists = async (
  quotationId: string,
  db: DbClient,
): Promise<QuotationHeaderRecord> => {
...
const assertQuotationExists = async (
  quotationId: string,
  db: DbClient,
): Promise<QuotationDetailEntity> => {
  const quotation = await db.quotation.findFirst({
...
const assertQuotationIsEditable = (status: QuotationStatus) => {
  if (
    !(QUOTATION_EDITABLE_STATUSES as readonly string[]).includes(status)
  ) {
    throw new AppError("Only draft quotations can be edited.", 400);
  }
};

const assertQuotationIsNotLocked = (status: QuotationStatus) => {
  if ((QUOTATION_LOCKED_STATUSES as readonly string[]).includes(status)) {
    throw new AppError("This quotation is locked and can no longer be edited.", 400);
  }
};

const getQuotationSettings = async (db: DbClient) => {
  ...
  const minimumMarginPercent = Number(
    settingMap.get("quotation.minimum_margin_percent") ??
      QUOTATION_SETTINGS_DEFAULTS.minimumMarginPercent,
  );
  const maximumDiscountPercent = Number(
    settingMap.get("quotation.maximum_discount_percent") ??
      QUOTATION_SETTINGS_DEFAULTS.maximumDiscountPercent,
  );
  const requireApprovalForManualOverride = Boolean(
    settingMap.get("quotation.require_approval_for_manual_override") ??
...
const updateQuotationTotals = async (
  quotationId: string,
  db: DbClient,
) => {
  ...
  const subtotalCost = roundTo(
    items.reduce((sum, item) => sum + Number(item.subtotalCost), 0),
    4,
  );
  const subtotalSale = roundTo(
    items.reduce((sum, item) => sum + Number(item.subtotalSale), 0),
    4,
  );
...
const buildQuotationVersionSnapshot = (
  quotation: QuotationDetailEntity,
...
const createPreparedItemMaterials = async (
  quotationItemId: string,
  materials: PreparedQuotationItem["materials"],
  db: DbClient,
) => {
...
const buildManualAdjustmentMaterial = (
  costAdjustment: number,
): PreparedQuotationItem["materials"][number] | null => {
...
const getCurrentItemCount = async (quotationId: string, db: DbClient) => {
...
const assertMaterialExists = async (
  materialId: string,
  db: DbClient,
): Promise<MaterialSummaryRecord> => {
...
const assertTemplateVersionExists = async (
  versionId: string,
  db: DbClient,
): Promise<ProductTemplateVersionRecord> => {
...
const buildManualMaterialItem = async (
  input: AddManualMaterialItemInput,
  db: DbClient,
...
const buildManualServiceItem = async (
  input: AddManualServiceItemInput,
): Promise<PreparedQuotationItem> => {
  const quantity = roundTo(input.quantity, 4);
  const pricing = buildManualPricing({
    marginPercent: input.marginPercent,
...
const buildTemplateItem = async (
  input: {
    description: string | null;
    inputValues: Record<string, JsonLike>;
    manualOverride: TemplateItemManualOverride | null;
    name: string;
  ...
  const baseSubtotalCost = roundTo(
    simulation.resultJson.totalCost * input.quantity,
    4,
  );
  const baseSubtotalSale = roundTo(
    simulation.resultJson.suggestedSalePrice * input.quantity,
    4,
  );
  ...
  const materialRows: PreparedQuotationItem["materials"] = [];
  ...
  simulation.resultJson.materials.forEach((material) => {
    ...
    const wasteQuantity = roundTo(
      material.estimatedWasteQuantity * input.quantity,
      4,
    );
    ...
    const totalCost = roundTo(
      unitCost === null
        ? 0
        : (requiredQuantity + wasteQuantity) * unitCost,
      4,
    );
  ...
  const manualAdjustment = buildManualAdjustmentMaterial(
    overrideResult.costAdjustment,
  );
  ...
  return {
    calculationResultJson: toInputJsonValue(
      buildTemplateCalculationResultJson({
        manualOverride: input.manualOverride,
...
const evaluateApprovalRequirements = async (
  quotationId: string,
  options: {
    db: DbClient;
  ...
  const manualOverrideDetected = quotation.items.some((item) =>
    hasManualOverride(item.calculationResultJson),
  );
  ...
  if (
    settings.requireApprovalForManualOverride &&
...
const createCurrentItemVersionCopies = async (
  quotation: QuotationDetailEntity,
  ...
  for (const item of quotation.items) {
    const versionItem = await db.quotationItem.create({
      data: {
        calculationResultJson:
          item.calculationResultJson === null
            ? PrismaNamespace.JsonNull
            : (item.calculationResultJson as Prisma.InputJsonValue),
        description: item.description,
        inputValuesJson:
          item.inputValuesJson === null
            ? PrismaNamespace.JsonNull
            : (item.inputValuesJson as Prisma.InputJsonValue),
    ...
    await createPreparedItemMaterials(
      versionItem.id,
      item.materials.map((material) => ({
        ...
        materialName: material.materialName,
        metadataJson:
          material.metadataJson === null
            ? PrismaNamespace.JsonNull
            : (material.metadataJson as Prisma.InputJsonValue),
...
const buildQuotationOrderBy = (
  sortBy: ListQuotationsQuery["sortBy"],
  sortDirection: ListQuotationsQuery["sortDirection"],
...
const buildQuotationWhereClause = (
...
export const quotationsService = {
  async listQuotations(
    ...
    return {
      data: quotations.map((quotation) =>
        mapQuotationListItem(quotation, canViewCost),
      ),
      pagination: {
        page: query.page,
  ...
  async updateQuotation(
    ...
    current: QuotationDetailRecord;
    previous: QuotationDetailRecord;
  }> {
  ...
  async addTemplateQuotationItem(
    quotationId: string,
  ...
  async updateQuotationItem(
    ...
    current: QuotationItemRecord;
    previous: QuotationItemRecord;
    quotation: QuotationDetailRecord;
  }> {
    ...
    if (item.itemType === "TEMPLATE_PRODUCT") {
      ...
      if (hasOverrideChange && !options.canOverrideCost) {
        throw new AppError(
          "Missing required permission: quotations.override_cost.",
          403,
        );
      }
      ...
      if (input.clearManualOverride) {
        ...
        manualOverride = {
          ...
          subtotalCost:
            input.unitCost !== undefined ? input.unitCost : currentOverride?.subtotalCost ?? null,
          subtotalSale:
            input.unitSalePrice !== undefined
              ? input.unitSalePrice
      ...
      preparedItem = await buildTemplateItem(
        {
          description:
            input.description === undefined
              ...
              : input.description,
          inputValues:
            input.inputValues && isPlainObject(input.inputValues)
              ? asJsonLikeRecord(input.inputValues as Record<string, unknown>)
      ...
      preparedItem = await buildManualMaterialItem(
        {
          ...
          marginPercent:
            input.marginPercent === undefined
              ...
              : input.marginPercent,
          materialId:
            input.materialId ?? String(currentValues.materialId ?? ""),
          name:
  ...
  async deleteQuotationItem(
    ...
    const deleted = mapQuotationItem(item as QuotationItemEntity, true);
  ...
  async recalculateQuotation(
    ...
    for (const item of quotation.items) {
      await this.updateQuotationItem(
        item.id,
        {},
        {
      ...
      const version = await db.quotationVersion.create({
        data: {
          ...
          snapshotJson: toInputJsonValue(
            buildQuotationVersionSnapshot(quotation, versionNumber),
          ),
          status: "ACTIVE",
  ...
  async changeQuotationStatus(
    ...
    historyEntry: QuotationStatusHistoryRecord;
    quotation: QuotationDetailRecord;
  }> {
    const quotation = await assertQuotationExists(quotationId, prisma);
    ...
    if (input.toStatus === "PENDING_APPROVAL") {
      throw new AppError(
        "Use the submit approval endpoint to move a quotation into pending approval.",
        400,
      );
    }
    ...
    if (
      ...
      throw new AppError(
        "A quotation can only be accepted after it has been approved or sent.",
        400,
      );
    }
    ...
    if (
      ...
      throw new AppError(
        "Only approved or sent quotations can be marked as rejected.",
        400,
      );
    }
    ...
    if (
      ...
      throw new AppError(
        "Only approved or sent quotations can be marked as expired.",
        400,
      );
    }
...
const quotationUpdate: Prisma.QuotationUpdateInput = {
  ...
  async listQuotationVersions(
    ...
    return versions.map((version) =>
      mapQuotationVersion(version, options?.canViewCost ?? false),
    );
  },

  async listQuotationApprovals(
    quotationId: string,
  ): Promise<QuotationApprovalRecord[]> {
    await assertQuotationHeaderExists(quotationId, prisma);
