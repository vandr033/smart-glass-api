...
} from "./production-board.rules.js";
import type {
  ProductionBoardJob,
  ProductionBoardQuery,
  ProductionBoardResponse,
  ProductionBlockCreateInput,
  ProductionBlockRecord,
  ProductionBlockResolveInput,
  ProductionScheduleInput,
  ProductionTaskAssignmentInput,
  ProductionTransitionInput,
  ProductionWasteEntryInput,
...
const workflowToJobStatus: Record<
  string,
...
const workCenterSelect = {
  ...
  name: true,
  type: true,
} satisfies Prisma.ProductionWorkCenterSelect;
...
const jobInclude = {
  ...
  qualityChecks: { orderBy: { createdAt: "desc" as const }, take: 10 },
  blocks: {
...
type BoardJobEntity = Prisma.ProductionJobGetPayload<{
  include: typeof jobInclude;
}>;

const dateWindow = (from: string | undefined, to: string | undefined) => {
...
const effectiveWorkflowStatus = (
  job: Pick<BoardJobEntity, "workflowStatus" | "status">,
) => {
  if (job.workflowStatus !== "DRAFT") return job.workflowStatus;
  if (job.status === "READY") return "READY_TO_START";
...
const mapBoardJob = (
  ...
  const workflowStatus = effectiveWorkflowStatus(
    job,
  ) as ProductionBoardJob["workflowStatus"];
  ...
  const qualityStatus =
    job.tasks.some((task) => task.requiresQualityControl) ||
  ...
  return {
    ...
    project: job.project
      ? {
          ...
          clientName:
            job.project.client.commercialName ||
    ...
    estimatedMinutes,
    consumedMinutes: job.tasks.reduce(
    ...
    overdue: Boolean(
      due && due < now && !["COMPLETED", "CANCELLED"].includes(workflowStatus),
    ),
    version: job.version,
...
const assertExpectedVersion = (
  ...
  if (expected !== undefined && actual !== expected) {
    throw new AppError(
      "La orden fue actualizada por otro usuario. Actualiza la información antes de continuar.",
      409,
    );
  }
};
...
const assertTransition = async (
  ...
  if (
    !isProductionWorkflowTransitionAllowed(
      from as ProductionBoardJob["workflowStatus"],
      input.toStatus,
    )
  ) {
  ...
  if (input.toStatus === "COMPLETED") {
    ...
    const pendingQuality = job.qualityChecks.some((check) =>
      ["PENDING", "FAILED", "REWORK_REQUIRED"].includes(check.status),
    );
    if (pendingQuality)
  ...
  if (
    ...
    job.materialConsumptions.some(
      (entry) => entry.consumptionType === "PLANNED",
    ) &&
    !job.materialConsumptions.some(
      (entry) => entry.consumptionType === "ACTUAL",
    )
  ) {
    throw new AppError(
      "No existe suficiente material preparado para iniciar la orden.",
      422,
    );
  }
};
...
const mapBlock = (
  block: Prisma.ProductionBlockGetPayload<{
    include: {
      createdBy: { select: typeof userSelect };
      assignedTo: { select: typeof userSelect };
...
const blockInclude = {
  ...
  productionJob: { select: { id: true, code: true } },
  productionTask: { select: { id: true, title: true } },
} satisfies Prisma.ProductionBlockInclude;
...
export const productionBoardService = {
  async getBoard(
    ...
    })) as ProductionBoardResponse["columns"];
    const inScope = mapped.filter(
      (job) => !["CANCELLED", "COMPLETED"].includes(job.workflowStatus),
    );
    const plannedMinutes = mapped.reduce(
      (sum, job) => sum + job.estimatedMinutes,
      0,
    );
    ...
    return {
      ...
      metrics: {
        ...
        inProgressOrders: inScope.filter((job) =>
          ["IN_PROGRESS", "PAUSED"].includes(job.workflowStatus),
        ).length,
        overdueOrders: mapped.filter((job) => job.overdue).length,
        blockedOrders: mapped.filter(
          (job) => job.workflowStatus === "BLOCKED" || job.openBlockCount > 0,
        ).length,
        pendingQualityTasks: mapped.filter(
          (job) => job.workflowStatus === "PENDING_QUALITY",
        ).length,
        capacityUtilizationPercent: capacity._sum.capacityDailyMinutes
          ? Math.min(
              999,
              Math.round(
                (plannedMinutes / Number(capacity._sum.capacityDailyMinutes)) *
                  100,
              ),
            )
          : 0,
        estimatedWasteAreaM2: Number(
          (
            await prisma.productionWasteReport.aggregate({
  ...
  async listWorkCenters(
    date = new Date(),
  ): Promise<ProductionWorkCenterRecord[]> {
    ...
    return centers.map((center) => {
      const scheduledMinutes = center.tasks.reduce(
        (sum, task) => sum + task.estimatedMinutes,
        0,
      );
      const availableMinutes =
        center.capacityDailyMinutes * Math.max(1, center.workstationCount);
      return {
        id: center.id,
        code: center.code,
        name: center.name,
        ...
        scheduledMinutes,
        availableMinutes,
        utilizationPercent: calculateCapacityUtilization(
          scheduledMinutes,
          availableMinutes,
        ),
      };
  ...
  async listCalendar(query: ProductionBoardQuery) {
  ...
  async listBlocks(
    ...
    return blocks.map((block) =>
      mapBlock(
        block as Prisma.ProductionBlockGetPayload<{
  ...
  async transitionJob(
    ...
    return prisma.$transaction(async (db) => {
      ...
      if (!job) throw new AppError("La orden de producción no existe.", 404);
      ...
      return mapBoardJob(updated as BoardJobEntity);
    });
  },
  ...
  async scheduleJob(
    ...
    return prisma.$transaction(async (db) => {
      ...
      if (
        ...
        throw new AppError(
          "Solo se pueden programar órdenes pendientes de producción.",
          422,
        );
      const nextStatus =
  ...
  async assignTask(
    taskId: string,
    input: ProductionTaskAssignmentInput,
    actorUserId: string | null,
  ) {
    return prisma.$transaction(async (db) => {
      ...
      if (!task) throw new AppError("La tarea de producción no existe.", 404);
  ...
  async transitionTask(
    taskId: string,
    toStatus: "IN_PROGRESS" | "PAUSED" | "BLOCKED" | "COMPLETED" | "PENDING",
    reason: string | null,
    actorUserId: string | null,
  ) {
    return prisma.$transaction(async (db) => {
      ...
      const eventMap: Record<
        string,
        "START" | "PAUSE" | "RESUME" | "BLOCK" | "UNBLOCK" | "FINISH"
      > = {
  ...
  async createBlock(
    input: ProductionBlockCreateInput,
    actorUserId: string | null,
  ) {
    return prisma.$transaction(async (db) => {
  ...
  async resolveBlock(
    blockId: string,
    input: ProductionBlockResolveInput,
    actorUserId: string | null,
  ) {
    return prisma.$transaction(async (db) => {
      ...
      if (!block) throw new AppError("El bloqueo no existe.", 404);
  ...
  async createWasteEntry(
    input: ProductionWasteEntryInput,
    actorUserId: string | null,
  ) {
  ...
  async exportBoard(query: ProductionBoardQuery, format: "csv" = "csv") {
    ...
    const csv = [
      header,
      ...rows.map((row) =>
        header.map((key) => String(row[key as keyof typeof row] ?? "")),
      ),
    ]
      .map((row) =>
