...
import type { JsonLike, MaterialUnit } from "../materials/materials.behavior.js";
...
import type {
  ...
  ProductionCuttingPlanSummary,
  ProductionJobDetailRecord,
  ProductionJobItemRecord,
  ProductionJobListItem,
  ProductionJobStatus,
  ProductionStatusHistoryRecord,
  ProductionTaskRecord,
  ProductionWasteReportRecord,
  ProductionWarehouseSummary,
  QualityCheckMutationInput,
  QualityCheckRecord,
...
type MaterialForConversion = {
  code: string;
  deletedAt: Date | null;
  id: string;
  isRemnantEligible: boolean;
  materialType: "LINEAR" | "PACKAGE" | "SERVICE" | "SHEET" | "UNIT";
  minimumReusableHeightMm: PrismaNamespace.Decimal | null;
  minimumReusableLengthMm: PrismaNamespace.Decimal | null;
  minimumReusableWidthMm: PrismaNamespace.Decimal | null;
  name: string;
...
const cuttingPlanSummarySelect = {
  ...
  wastePercent: true,
} satisfies Prisma.CuttingPlanSelect;
...
} satisfies Prisma.ProductionJobItemInclude;
...
} satisfies Prisma.MaterialConsumptionInclude;
...
} satisfies Prisma.ProductionStatusHistoryInclude;

const productionWasteReportInclude = {
  cuttingPlan: {
    select: cuttingPlanSummarySelect,
  },
} satisfies Prisma.ProductionWasteReportInclude;
...
type ProductionJobListEntity = Prisma.ProductionJobGetPayload<{
  include: typeof productionJobListInclude;
}>;

type ProductionJobDetailEntity = Prisma.ProductionJobGetPayload<{
  include: typeof productionJobDetailInclude;
}>;

type ProductionTaskEntity = Prisma.ProductionTaskGetPayload<{
  include: typeof productionTaskInclude;
}>;

type ProductionJobItemEntity = Prisma.ProductionJobItemGetPayload<{
  include: typeof productionJobItemInclude;
}>;

type MaterialConsumptionEntity = Prisma.MaterialConsumptionGetPayload<{
  include: typeof materialConsumptionInclude;
}>;

type QualityCheckEntity = Prisma.QualityCheckGetPayload<{
  include: typeof qualityCheckInclude;
}>;

type ProductionStatusHistoryEntity = Prisma.ProductionStatusHistoryGetPayload<{
  include: typeof productionStatusHistoryInclude;
}>;

type ProductionWasteReportEntity = Prisma.ProductionWasteReportGetPayload<{
  include: typeof productionWasteReportInclude;
}>;
...
const mapCuttingPlanSummary = (
  plan:
    | {
        ...
        sheetCount: number;
        status: string;
        wastePercent: PrismaNamespace.Decimal;
      }
    | null,
): ProductionCuttingPlanSummary => {
  return plan
...
const mapInventoryStockSummary = (
  stock:
    | Prisma.InventoryStockGetPayload<{
        select: typeof inventoryStockSummarySelect;
      }>
    | null,
) => {
  return stock
...
const mapRemnantSummary = (
  remnant:
    | Prisma.RemnantPieceGetPayload<{
        select: typeof remnantSummarySelect;
      }>
    | null,
) => {
  return remnant
...
const mapProductionJobItem = (item: ProductionJobItemEntity): ProductionJobItemRecord => {
  return {
...
const mapProductionTask = (task: ProductionTaskEntity): ProductionTaskRecord => {
  return {
...
const mapQualityCheck = (check: QualityCheckEntity): QualityCheckRecord => {
  return {
...
const mapProductionStatusHistory = (
  entry: ProductionStatusHistoryEntity,
): ProductionStatusHistoryRecord => {
  return {
...
const mapWasteReport = (
  report: ProductionWasteReportEntity | null,
): ProductionWasteReportRecord | null => {
  if (!report) {
    return null;
  }
...
const mapProductionJobListItem = (job: ProductionJobListEntity): ProductionJobListItem => {
  const completedTaskCount = job.tasks.filter((task) => task.status === "COMPLETED").length;
  const pendingTaskCount = job.tasks.filter((task) =>
    ["BLOCKED", "IN_PROGRESS", "PENDING"].includes(task.status),
  ).length;
...
const mapProductionJobDetail = (job: ProductionJobDetailEntity): ProductionJobDetailRecord => {
...
const materialBehaviorFromRecord = (material: MaterialForConversion) => {
  return {
    code: material.code,
    materialType: material.materialType,
    name: material.name,
    unitConversionJson: (material.unitConversionJson as JsonLike | null) ?? null,
  };
};

const convertQuantity = (
  material: MaterialForConversion,
  quantity: number,
...
const buildProductionJobsWhereClause = (
...
const auditProductionAction = async (
  input: {
    action: string;
...
const generateProductionJobCode = async (db: DbClient, value = new Date()) => {
...
const getMaterialOrThrow = async (db: DbClient, materialId: string) => {
...
const getInventoryStockOrThrow = async (db: DbClient, inventoryStockId: string) => {
...
const getRemnantOrThrow = async (db: DbClient, remnantPieceId: string) => {
...
const getProductionJobByIdOrThrow = async (
  db: DbClient,
  productionJobId: string,
): Promise<ProductionJobDetailEntity> => {
...
const getProductionTaskByIdOrThrow = async (
  db: DbClient,
...
const assertSourceIsAvailable = (job: { status: ProductionJobStatus }) => {
  if (["CANCELLED", "COMPLETED"].includes(job.status)) {
    throw new AppError(
      "Cannot consume material for a completed or cancelled production job.",
      400,
    );
  }
};
...
const closeInventoryStock = async (
  db: DbClient,
  stockId: string,
  condition:
    | "AVAILABLE"
...
const updateRemnantStatus = async (
  db: DbClient,
  remnantId: string,
...
const createProductionRemnantOutput = async (
  db: DbClient,
  input: {
    code?: string | null;
  ...
  if (
    input.material.minimumReusableLengthMm !== null &&
    input.lengthMm !== null &&
    input.lengthMm < Number(input.material.minimumReusableLengthMm)
  ) {
    throw new AppError(
      "Remnant length is below the minimum reusable length configured for this material.",
      400,
    );
  }

  if (
    input.material.minimumReusableWidthMm !== null &&
    input.widthMm !== null &&
    input.widthMm < Number(input.material.minimumReusableWidthMm)
  ) {
    throw new AppError(
      "Remnant width is below the minimum reusable width configured for this material.",
      400,
    );
  }
...
const syncJobItemStatuses = async (db: DbClient, productionJobId: string) => {
  ...
  for (const item of items) {
    ...
    let nextStatus: "CANCELLED" | "COMPLETED" | "IN_PROGRESS" | "PENDING" =
      "PENDING";
...
const assertNoSourceConflict = async (
  db: DbClient,
  input: {
    ...
    where: {
      ...
      OR: [
        ...
        input.cuttingPlanId
          ...
          : undefined,
      ].filter(Boolean) as Prisma.ProductionJobWhereInput[],
    },
  });

  if (existing) {
    throw new AppError(
      `Production job ${existing.code} already exists for this source record.`,
      409,
    );
  }
};

const buildProfilePlanSourceMarker = (profileCuttingPlanId: string): string => {
  return `[PROFILE_CUTTING_PLAN_ID=${profileCuttingPlanId}]`;
};

const assertNoProfilePlanConflict = async (
  db: DbClient,
  ...
  if (existing) {
    throw new AppError(
      `Production job ${existing.code} already exists for this profile cutting plan.`,
      409,
    );
  }
};

const createJobWithUniqueCode = async (run: (db: DbClient, code: string) => Promise<string>) => {
...
const createPlannedConsumptionsFromQuotation = async (
  db: DbClient,
  input: {
    createdByUserId: string | null;
    productionJobId: string;
      ...
      await db.materialConsumption.create({
        data: {
          ...
          quantity: roundQuantity(
            Number(materialLine.requiredQuantity) * Number(item.quantity),
          ),
          sourceType: "MANUAL",
          unit: materialLine.unit as MaterialUnit,
        },
      });
    }
  }
...
const createPlannedConsumptionsFromCuttingPlan = async (
  db: DbClient,
  input: {
    createdByUserId: string | null;
    materialId: string;
    ...
    sheets: Array<{
      ...
      sheetAreaM2: PrismaNamespace.Decimal;
...
const createPlannedConsumptionsFromProfileCuttingPlan = async (
  db: DbClient,
  input: {
    createdByUserId: string | null;
    material: {
    ...
    bars: Array<{
      inventoryStockId: string | null;
      originalLengthMm: PrismaNamespace.Decimal;
      remnantPieceId: string | null;
  ...
  for (const bar of input.bars) {
    const sourceType =
      bar.sourceType === "REMNANT"
        ? "REMNANT"
        : bar.sourceType === "INVENTORY_BAR"
          ? "INVENTORY_STOCK"
          : "MANUAL";
    const plannedQuantity =
      input.material.stockUnit === "UNIT"
        ? 1
        : roundQuantity(
            convertMaterialUnit({
              fromUnit: "MM",
              material: {
                ...
                unitConversionJson: input.material.unitConversionJson as JsonLike,
                ...(input.material.code
                  ? {
...
export const productionService = {
  async listProductionJobs(query: ListProductionJobsQuery) {
  ...
  async getProductionJobById(productionJobId: string): Promise<ProductionJobDetailRecord> {
    const job = await getProductionJobByIdOrThrow(prisma, productionJobId);
    return mapProductionJobDetail(job);
  },

  async createProductionJobManually(
    ...
    const productionJobId = await createJobWithUniqueCode(async (db, code) => {
        const job = await db.productionJob.create({
          data: {
            ...
            quotationId: input.quotationId,
            status:
              input.items.length > 0 || input.tasks.length > 0 || input.cuttingPlanId
                ? "READY"
                : "DRAFT",
          },
          select: {
  ...
  async createProductionJobFromQuotation(
    quotationId: string,
    userId: string | null,
  ): Promise<ProductionJobDetailRecord> {
    await assertNoSourceConflict(prisma, {
    ...
    const productionJobId = await createJobWithUniqueCode(async (db, code) => {
        ...
        if (quotation.status !== "ACCEPTED") {
          throw new AppError(
            "Production jobs can only be created from accepted quotations.",
            400,
          );
        }
        ...
        for (const item of quotation.items.filter((entry) =>
          !["DISCOUNT", "NOTE"].includes(entry.itemType),
        )) {
          await db.productionJobItem.create({
            data: {
              description: item.description,
              materialId:
                item.materials.length === 1 ? item.materials[0]?.materialId ?? null : null,
  ...
  async createProductionJobFromCuttingPlan(
  ...
  async deleteProductionJob(productionJobId: string, userId: string | null) {
    return prisma.$transaction(async (db) => {
  ...
  async completeProductionJob(
    ...
    return prisma.$transaction(async (db) => {
      ...
      if (
        ...
        throw new AppError(
          "Cannot complete the production job while required tasks are still pending.",
          400,
        );
      }
  ...
  async assignProductionJob(
    productionJobId: string,
    assignedToUserId: AssignProductionJobInput["assignedToUserId"],
    userId: string | null,
  ): Promise<ProductionJobDetailRecord> {
    return prisma.$transaction(async (db) => {
      const job = await getProductionJobByIdOrThrow(db, productionJobId);
  ...
  async generateProductionTasks(
    productionJobId: string,
    userId: string | null,
    input: GenerateProductionTasksInput,
    ...
    return prisma.$transaction(async (db) => {
      ...
      if (job.tasks.length > 0 && !input.replaceExisting) {
        throw new AppError(
          "Production tasks already exist for this job. Set replaceExisting to regenerate them.",
          400,
        );
      }
      ...
      if (
        ...
        throw new AppError(
          "Tasks with completed work or material consumption cannot be regenerated.",
          400,
        );
      }
  ...
  async listProductionJobTasks(productionJobId: string): Promise<ProductionTaskRecord[]> {
    const job = await getProductionJobByIdOrThrow(prisma, productionJobId);
    return job.tasks.map(mapProductionTask);
  },

  async updateProductionTask(
    productionTaskId: string,
    ...
    return prisma.$transaction(async (db) => {
      ...
      if (
        ...
        if (!latestQualityCheck || latestQualityCheck.status !== "PASSED") {
          throw new AppError(
            "A passed quality check is required before completing the quality task.",
            400,
          );
        }
      }
  ...
  async consumeMaterialForTask(
    ...
    return prisma.$transaction(async (db) => {
      ...
      if (!["COMPLETED", "IN_PROGRESS"].includes(task.status)) {
        throw new AppError(
          "Start the production task before consuming material.",
          400,
        );
      }
      ...
      let sourceStockForRemnantOutput: string | null = null;
      let sourceMaterial: MaterialForConversion | null = null;
      ...
      if (input.remnantPieceId) {
        ...
        const quantityInRemnantUnit = convertQuantity(
          material,
          input.quantity,
          input.unit,
          remnant.unit,
      ...
      const createdConsumptions: MaterialConsumptionEntity[] = [];
      ...
      let remnantOutput: { code: string; id: string } | null = null;
  ...
  async listQualityChecks(productionJobId: string): Promise<QualityCheckRecord[]> {
    const job = await getProductionJobByIdOrThrow(prisma, productionJobId);
  ...
  async recordQualityCheck(
    productionJobId: string,
    input: QualityCheckMutationInput,
    userId: string | null,
  ...
  async getWasteReport(productionJobId: string): Promise<ProductionWasteReportRecord | null> {
    ...
    const plannedInputAreaM2 = roundQuantity(
      job.materialConsumptions
  ...
  async calculateProductionWaste(
    productionJobId: string,
    userId: string | null,
  ): Promise<ProductionWasteReportRecord> {
    return prisma.$transaction(async (db) => {
      const job = await getProductionJobByIdOrThrow(db, productionJobId);

      const actualWasteAreaM2 = roundQuantity(
        job.materialConsumptions
          .filter((entry) =>
            ["SCRAP", "WASTE"].includes(entry.consumptionType) && entry.unit === "M2",
          )
          .reduce((sum, entry) => sum + Number(entry.quantity), 0),
      );
      const plannedInputAreaM2 = roundQuantity(
      ...
      const theoreticalWasteAreaM2 = job.cuttingPlan
        ? Number(job.cuttingPlan.totalWasteAreaM2)
        : roundQuantity((plannedInputAreaM2 * theoreticalWastePercent) / 100);
      const actualWastePercent =
        plannedInputAreaM2 > 0
      ...
      return mapWasteReport(report) as ProductionWasteReportRecord;
    });
  },
};
