...
const nullableDateSchema = (message: string) =>
  z
    ...
    .transform((value) => {
      ...
      return parseDateValue(value as string | Date);
    })
...
export const postventaCaseStatusChangeSchema = z.enum(
  POSTVENTA_CASE_STATUSES.filter((status) => status !== "CERRADO"),
);
