...
import {
  buildPostventaEvidenceUrl,
  postventaEvidenceUploadsDir,
...
import type {
  ...
  ListPostventaCasesQuery,
  ListProductWarrantiesQuery,
  PostventaActivityRecord,
  PostventaCaseDetailRecord,
  PostventaCaseListItem,
  PostventaCaseStatus,
  PostventaClientSummary,
  PostventaCostCategory,
  PostventaCostRecord,
  PostventaCostOrigin,
  PostventaEvidenceMetadataInput,
  PostventaEvidenceRecord,
  PostventaInstallationSummary,
  PostventaPriority,
  PostventaProjectSummary,
  PostventaQuotationSummary,
  PostventaReservationRecord,
  PostventaStatusHistoryRecord,
  PostventaUserSummary,
  ProductWarrantyRecord,
  UpdatePostventaActivityInput,
  UpdatePostventaCaseInput,
  UpdateProductWarrantyInput,
} from "./postventa.types.js";
import {
  POSTVENTA_STATUS_TRANSITIONS,
...
} satisfies Prisma.ProductWarrantyInclude;

const activityInclude = {
  responsibleUser: {
    select: userSummarySelect,
  },
} satisfies Prisma.PostventaActivityInclude;

const evidenceInclude = {
  uploadedByUser: {
    select: userSummarySelect,
  },
} satisfies Prisma.PostventaEvidenceInclude;

const statusHistoryInclude = {
  changedByUser: {
    select: userSummarySelect,
  },
} satisfies Prisma.PostventaStatusHistoryInclude;
...
} satisfies Prisma.PostventaInventoryReservationInclude;
...
} satisfies Prisma.PostventaCaseInclude;
...
type ProductWarrantyEntity = Prisma.ProductWarrantyGetPayload<{
  include: typeof productWarrantyInclude;
}>;

type PostventaActivityEntity = Prisma.PostventaActivityGetPayload<{
  include: typeof activityInclude;
}>;

type PostventaEvidenceEntity = Prisma.PostventaEvidenceGetPayload<{
  include: typeof evidenceInclude;
}>;

type PostventaStatusHistoryEntity = Prisma.PostventaStatusHistoryGetPayload<{
  include: typeof statusHistoryInclude;
}>;

type PostventaReservationLinkEntity = Prisma.PostventaInventoryReservationGetPayload<{
  include: typeof reservationLinkInclude;
}>;

type PostventaCaseListEntity = Prisma.PostventaCaseGetPayload<{
  include: typeof postventaCaseListInclude;
}>;

type PostventaCaseDetailEntity = Prisma.PostventaCaseGetPayload<{
  include: typeof postventaCaseDetailInclude;
}>;
...
const getWarrantyEffectiveStatus = (
  warranty: Pick<ProductWarrantyEntity, "endDate" | "startDate" | "status">,
): ProductWarrantyRecord["status"] => {
...
const isWarrantyCurrent = (
  ...
  return (
    referenceDate >= warranty.startDate &&
    referenceDate <= warranty.endDate
  );
};

const mapWarrantyRecord = (
  warranty: ProductWarrantyEntity | null,
): ProductWarrantyRecord | null => {
  if (!warranty) {
    return null;
  }
...
const mapActivityRecord = (
  activity: PostventaActivityEntity,
): PostventaActivityRecord => ({
...
const mapEvidenceRecord = (
  evidence: PostventaEvidenceEntity,
): PostventaEvidenceRecord => ({
...
const mapStatusHistoryRecord = (
  entry: PostventaStatusHistoryEntity,
): PostventaStatusHistoryRecord => ({
...
const mapReservationRecord = (
  link: PostventaReservationLinkEntity,
): PostventaReservationRecord => ({
...
const summarizeCostBucket = (
  caseType: PostventaCaseListEntity["caseType"],
  category: PostventaCostCategory,
): "GARANTIA" | "RECLAMO" | "REPOSICION" | null => {
...
const buildFinancialImpact = async (
  record: PostventaCaseDetailEntity,
): Promise<PostventaCaseDetailRecord["financialImpact"]> => {
  const costs = record.costs.map(mapCostRecord);
  const costTotal = roundTo(costs.reduce((sum, cost) => sum + cost.amount, 0), 4);

  const costByBucket = costs.reduce(
    (summary, cost) => {
  ...
  let ventaProyecto: number | null = null;
  let utilidadProyecto: number | null = null;

  if (record.projectId) {
    try {
      const detail =
        await projectProfitabilityService.getProjectProfitabilityByProjectId(
          record.projectId,
        );
  ...
  return {
    ...
    porcentajeSobreUtilidad:
      utilidadProyecto && utilidadProyecto !== 0
        ? roundTo((costTotal / utilidadProyecto) * 100, 4)
        : null,
    porcentajeSobreVenta:
      ventaProyecto && ventaProyecto !== 0
...
const mapCaseListItem = (
  record: PostventaCaseListEntity,
): PostventaCaseListItem => {
  const totalCost = roundTo(
    record.costs.reduce((sum, cost) => sum + Number(cost.amount), 0),
    4,
  );

  return {
    activityPendingCount: record.activities.filter(
      (activity) => activity.status !== "EJECUTADA" && activity.status !== "CANCELADA",
    ).length,
    ...
    priority: record.priority as PostventaPriority,
...
const normalizeWarrantyStatus = (
  status: "ANULADA" | "VENCIDA" | "VIGENTE",
  endDate: Date,
): "ANULADA" | "VENCIDA" | "VIGENTE" => {
  if (status === "ANULADA") {
    return "ANULADA";
  }
...
const generatePostventaCaseCode = async (
  db: DbClient = prisma,
  value = new Date(),
): Promise<string> => {
...
const getPostventaCaseOrThrow = async (
  db: DbClient,
  caseId: string,
): Promise<PostventaCaseDetailEntity> => {
...
const getProductWarrantyOrThrow = async (
  db: DbClient,
  warrantyId: string,
): Promise<ProductWarrantyEntity> => {
...
const getPostventaActivityOrThrow = async (
  db: DbClient,
  activityId: string,
): Promise<PostventaActivityEntity> => {
...
const getPostventaReservationLinkOrThrow = async (
  db: DbClient,
  reservationLinkId: string,
): Promise<PostventaReservationLinkEntity> => {
...
const ensureClientExists = async (db: DbClient, clientId: string) => {
...
const ensureProjectExists = async (db: DbClient, projectId: string) => {
...
const ensureQuotationExists = async (db: DbClient, quotationId: string) => {
...
const ensureInstallationExists = async (
  db: DbClient,
...
const findApplicableWarranty = async (
  db: DbClient,
  input: {
    clientId: string;
...
const resolveCaseContext = async (
  db: DbClient,
  input: {
    ...
    reportedAt: Date;
    type: CreatePostventaCaseInput["type"];
  ...
  if (!derivedClientId) {
    throw new AppError(
      "El caso postventa debe estar asociado al menos a un cliente valido.",
      400,
    );
  }
  ...
  if (quotation && quotation.clientId !== derivedClientId) {
    throw new AppError(
      "La cotizacion seleccionada no pertenece al cliente indicado.",
      400,
    );
  }
  ...
  if (installation && project && installation.projectId && installation.projectId !== project.id) {
    throw new AppError(
      "La instalacion seleccionada no corresponde al proyecto indicado.",
      400,
    );
  }
  ...
  if (
    ...
    throw new AppError(
      "La instalacion seleccionada no corresponde a la cotizacion indicada.",
      400,
    );
  }
  ...
  if (selectedWarranty) {
    if (selectedWarranty.client.id !== derivedClientId) {
      throw new AppError(
        "La garantia seleccionada no pertenece al cliente indicado.",
        400,
      );
    }
    ...
    if (project && selectedWarranty.project?.id !== project.id) {
      throw new AppError(
        "La garantia seleccionada no corresponde al proyecto indicado.",
        400,
      );
    }
  }
  ...
  if (
    ...
    throw new AppError(
      "No existe una garantia vigente para la fecha reportada. Marca el caso como fuera de garantia si corresponde.",
      400,
    );
  }
  ...
  if (
    ...
    throw new AppError(
      "La garantia asociada ya no se encuentra vigente para la fecha reportada.",
      400,
    );
  }
  ...
  return {
    ...
    quotationId:
      quotation?.id ?? installation?.quotationId ?? input.quotationId ?? null,
    warrantyId: selectedWarranty?.id ?? null,
  };
};

const updateCaseStatusWithinTransaction = async (
  db: DbClient,
  input: {
    caseId: string;
  ...
  const allowedTransitions =
    POSTVENTA_STATUS_TRANSITIONS[currentCase.status] ?? [];

  if (!allowedTransitions.includes(input.status)) {
    throw new AppError(
      "El cambio de estado solicitado no esta permitido para este caso.",
      400,
    );
  }
...
const getAutoStatusFromActivity = (
  input: {
    executedAt: Date | null;
    scheduledAt: Date | null;
    type: CreatePostventaActivityInput["type"];
  },
): PostventaCaseStatus | null => {
...
const buildCasesWhereClause = (
  query: ListPostventaCasesQuery,
): Prisma.PostventaCaseWhereInput => {
...
const buildWarrantiesWhereClause = (
  query: ListProductWarrantiesQuery,
): Prisma.ProductWarrantyWhereInput => {
  const now = new Date();
...
export const postventaService = {
  async listCases(query: ListPostventaCasesQuery) {
  ...
  async getWarrantyById(warrantyId: string): Promise<ProductWarrantyRecord> {
    const record = await getProductWarrantyOrThrow(prisma, warrantyId);
    return mapWarrantyRecord(record)!;
  },

  async getActivityById(activityId: string): Promise<PostventaActivityRecord> {
  ...
  async createCase(
    ...
    if (!createdCaseId) {
      throw new AppError(
        "No se pudo generar un codigo unico para el caso postventa.",
        500,
      );
    }
  ...
  async updateCase(
    ...
    if (input.warrantyId) {
      ...
      if (warranty.client.id !== currentCase.client.id) {
        throw new AppError(
          "La garantia seleccionada no pertenece al cliente del caso.",
          400,
        );
      }
      ...
      if (currentCase.project && warranty.project?.id !== currentCase.project.id) {
        throw new AppError(
          "La garantia seleccionada no corresponde al proyecto del caso.",
          400,
        );
      }
    }
  ...
  async listWarranties(query: ListProductWarrantiesQuery) {
  ...
  async createWarranty(
    ...
    if (clientId !== project.clientId) {
      throw new AppError(
        "La garantia debe asociarse al mismo cliente del proyecto.",
        400,
      );
    }
    ...
    const updated = await prisma.productWarranty.update({
      data: {
        ...
        ...(input.startDate !== undefined
          ...
          : {}),
        status: normalizeWarrantyStatus(
  ...
  async updateActivity(
    ...
    await ensureUserExists(
      prisma,
      input.responsibleId ?? current.responsibleUser?.id ?? null,
      "la actividad postventa",
    );
  ...
  async createEvidence(
    caseId: string,
    metadata: PostventaEvidenceMetadataInput,
    file: UploadFile,
    userId: string | null,
  ): Promise<PostventaCaseDetailRecord> {
    const currentCase = await getPostventaCaseOrThrow(prisma, caseId);
    ...
    if (metadata.activityId) {
      ...
      if (activity.postventaCaseId !== caseId) {
        throw new AppError(
          "La actividad seleccionada no pertenece al caso postventa indicado.",
          400,
        );
      }
    }
  ...
  async createInventoryReservation(
    ...
    if (!currentCase.project && !currentCase.quotation) {
      throw new AppError(
        "Para reservar repuestos debes asociar el caso a un proyecto o una cotizacion.",
        400,
      );
    }
    ...
    try {
      ...
      if (error instanceof AppError) {
        throw new AppError(
          "No se pudo reservar el repuesto solicitado. Verifica disponibilidad, almacen y cantidad.",
          error.statusCode,
        );
      }
  ...
  async consumeInventoryReservation(
    ...
    try {
      ...
      if (error instanceof AppError) {
        throw new AppError(
          "No se pudo registrar el consumo de la reserva vinculada al caso.",
          error.statusCode,
        );
      }
  ...
  async releaseInventoryReservation(
    ...
    try {
      ...
      if (error instanceof AppError) {
        throw new AppError(
          "No se pudo liberar la reserva vinculada al caso postventa.",
          error.statusCode,
        );
      }
